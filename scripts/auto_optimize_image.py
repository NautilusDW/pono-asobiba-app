...
最適化ポリシー (assets/images/story/README.md の既存慣行に準拠):
  ...
  - alpha 無し (RGB): JPEG に変換。幅 1600px 上限、quality=88, progressive, optimize。
    ただしフック経由では既存拡張子を温存 (png → 同パス png で上書き保存)。
    --scan / 手動呼び出しのみ拡張子変更 (.png → .jpg) を許可。
...
def run_manual_mode(paths: Iterable[str]) -> int:
    cache = _load_cache()
    for pstr in paths:
        p = Path(pstr)
        if not p.is_absolute():
            p = (ROOT / p).resolve()
        r = optimize_one(p, allow_rename=True, verbose=True)
