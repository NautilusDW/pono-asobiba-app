...
assert.match(html, /styles\.css\?v=20260716-1319/, "the current Nazonazo scenery and stage games must bypass stale CSS caches");
assert.match(html, /js\/game\.js\?v=20260716-1319/, "the current Nazonazo runtime must bypass stale game-script caches");
assert.ok(
...
assert.match(game, /addEventListener\("pagehide",\(\)=>\{spaceChaseState\.frameAt=0;cancelSpaceChaseRescuePointer\(true\);closeGameSettings\(\);hideWeatherNotice\(\);pauseSeaInput\(\);pauseFutureCraneInput\(\);clearTimeout\(rainParticleResizeTimer\);safeSuspend\(\);\}\)/, "pagehide must cancel rescue input, pause the space chase, close settings, and clear the weather notice, sea and crane input, and pending resize callback");
