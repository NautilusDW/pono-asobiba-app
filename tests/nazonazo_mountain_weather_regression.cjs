...
assert.match(html, /styles\.css\?v=20260713-1282/, "the current Nazonazo scenery and stage games must bypass stale CSS caches");
assert.match(html, /js\/game\.js\?v=20260713-1282/, "the current Nazonazo runtime must bypass stale game-script caches");
assert.ok(
...
assert.match(game, /addEventListener\("pagehide",\(\)=>\{hideWeatherNotice\(\);pauseSeaInput\(\);clearTimeout\(rainParticleResizeTimer\);safeSuspend\(\);\}\)/, "pagehide must clear the weather notice, sea input, and pending resize callback");
