...
assert.match(game, /function startJourneyAt\(s\)\{\s*hideWeatherNotice\(\);\s*resetNumberCargoGame\(\);\s*clearRareEvent\(\);/, "new journeys must clear delayed and flying rare friends after resetting the number cargo activity");
assert.match(game, /function openMap\(msg\)\{\s*hideWeatherNotice\(\);\s*resetNumberCargoGame\(\);\s*clearRareEvent\(\);/, "opening the map must clear delayed and flying rare friends after resetting the number cargo activity");
