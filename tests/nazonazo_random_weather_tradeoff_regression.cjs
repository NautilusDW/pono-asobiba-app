...
assert.match(game, /function startJourneyAt\(s\)\{\s*hideWeatherNotice\(\);\s*resetNumberJumpGame\(\);\s*clearRareEvent\(\);/, "new journeys must clear delayed and flying rare friends after resetting the number activity");
assert.match(game, /function openMap\(msg\)\{\s*hideWeatherNotice\(\);\s*resetNumberJumpGame\(\);\s*clearRareEvent\(\);/, "opening the map must clear delayed and flying rare friends after resetting the number activity");
