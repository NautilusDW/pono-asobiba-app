...
assert.match(game, /function startJourneyAt\(s\)\{\s*hideWeatherNotice\(\);\s*resetNumberCargoGame\(\);\s*clearFutureRailGame\(\);\s*clearRareEvent\(\);/, "new journeys must clear delayed and flying rare friends after resetting stage-specific answer activities");
assert.match(game, /function openMap\(msg\)\{\s*hideWeatherNotice\(\);\s*resetNumberCargoGame\(\);\s*clearFutureRailGame\(\);\s*clearRareEvent\(\);/, "opening the map must clear delayed and flying rare friends after resetting stage-specific answer activities");
