...
assert.match(showQuiz, /if\(isNumberCargoQuestion\(\)\)renderNumberCargoGame\(\);else if\(isSeaStage\(\)\)renderSeaBubbleGame\(\);else if\(isFutureStage\(\)\)renderFutureCapsuleGame\(\);else if\(isSpaceStage\(\)\)renderSpaceRepairGame\(\);else renderChoiceCards\(\);/, "number, sea, future, and space questions must keep their own answer games while other stages use cards");
