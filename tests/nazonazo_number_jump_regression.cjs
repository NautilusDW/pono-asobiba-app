...
assert.match(showQuiz, /if\(isNumberCargoQuestion\(\)\)renderNumberCargoGame\(\);else if\(isSeaStage\(\)\)renderSeaBubbleGame\(\);else renderChoiceCards\(\);/, "only number questions should render the cargo activity while sea keeps its own answer game");
