...
test('30秒完走ごとに6→5→4か所へ直接進み、3地点の進行と場所別記録を保存する', async ({ page }) => {
  ...
  await expectHolesDisabled(page, true);
  await expect(page.locator('#stage')).toHaveCSS('background-image', /bg_world_komorebi_lowangle_20260724\.png/);
  ...
  await expect(page.locator('#stage')).toHaveCSS('background-image', /bg_world_donguri_overlook_20260724\.png/);
  await runToResult(page);
  await expect(page.locator('#result-title')).toHaveText('さいごまで あそべた！');
  ...
  await expect(page.locator('#stage')).toHaveCSS('background-image', /bg_world_mizube_waterline_v2_20260724\.png/);
  await runToResult(page);
