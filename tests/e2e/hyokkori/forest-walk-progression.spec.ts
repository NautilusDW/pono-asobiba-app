...
test('30秒完走ごとに6→5→4か所へ直接進み、3地点の進行と場所別記録を保存する', async ({ page }) => {
  ...
  await expect(page.locator('#stage')).toHaveCSS('background-image', /bg_donguri_path_autumn_20260723\.png/);
  await runToResult(page);
  await expect(page.locator('#result-title')).toHaveText('さいごまで あそべた！');
  ...
  await expect(page.locator('#stage')).toHaveCSS('background-image', /bg_mizube_cool_20260723\.png/);
  await runToResult(page);
