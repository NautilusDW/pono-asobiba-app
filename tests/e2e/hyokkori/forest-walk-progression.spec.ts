...
const GAME_URL = '/hyokkori-hightouch/index.html?e2e=score-gate-1469';
const LOCATIONS = [
  {
...
async function advanceToAwake(page, budgetMs = 8_000) {
...
async function runToResult(page, scoreGoal = 0) {
  ...
  while (Number((await page.locator('#hud-score').textContent())?.match(/\d+/)?.[0] || 0) < scoreGoal) {
    const hole = await advanceToAwake(page);
    await hole.click({ force: true });
...
async function continueTo(page, location) {
  ...
  for (let elapsed = 0; elapsed < 2_000; elapsed += 50) {
    ...
    const ready = await page.evaluate((id) => (
      ...
      && document.getElementById('countdown-screen')?.classList.contains('show')
    ), location.id);
    if (ready) break;
  }
...
async function expectLocationPresentation(page, location, index) {
  ...
  await expect(page.locator('#start-goal')).toHaveText(`${location.scoreGoal}てんで つぎへ`);
...
test('50点未満は同じ面、50点以上だけ6→5→4→5→4か所を進み、最終達成で月の花から1面へ戻る', async ({ page }) => {
  ...
  await runToResult(page, 0);

  await expect(page.locator('#result-card')).toHaveClass(/is-goal-missed/);
  await expect(page.locator('#result-card')).not.toHaveClass(/is-final-bloom/);
  await expect(page.locator('#result-title')).toHaveText('おしい！');
  await expect(page.locator('#result-goal-status')).toHaveText('あと 50てんで つぎへ');
  await expect(page.locator('#result-next-location')).toHaveText('おなじ ばしょで もういちど');
  await expect(page.locator('#retry-btn')).toHaveText('もういちど');
  await expectWalkDots(page, 0, 0);
  ...
  expect(saved.routeCompletedRuns).toBe(0);
  expect(saved.completedLocationIds).toEqual([]);
  ...
  await runToResult(page, LOCATIONS[0].scoreGoal);
  await expect(page.locator('#result-card')).not.toHaveClass(/is-goal-missed/);
  await expect(page.locator('#result-title')).toHaveText('ナイス ハイタッチ！');
  await expect(page.locator('#result-goal-status')).toHaveText('50てん クリア！');
  ...
  expect(saved.locationRecords.komorebi_clearing.plays).toBe(2);
  expect(saved.locationRecords.komorebi_clearing.bestScore).toBeGreaterThanOrEqual(50);

  await continueTo(page, LOCATIONS[1]);
  await expectLocationPresentation(page, LOCATIONS[1], 1);
  await runToResult(page, LOCATIONS[1].scoreGoal);
  await expect(page.locator('#result-title')).toHaveText('ナイス ハイタッチ！');
  await expect(page.locator('#result-goal-status')).toHaveText('50てん クリア！');
  ...
  await runToResult(page, LOCATIONS[2].scoreGoal);
  await expect(page.locator('#result-title')).toHaveText('ナイス ハイタッチ！');
  ...
  await runToResult(page, LOCATIONS[3].scoreGoal);
  await expect(page.locator('#result-title')).toHaveText('ナイス ハイタッチ！');
  ...
  await expect(page.locator('#result-card')).not.toHaveClass(/is-final-bloom/);
  await expect(page.locator('#result-title')).toHaveText('おしい！');
  await expect(page.locator('#result-visual')).not.toHaveClass(/is-moon-flower/);
  await expect(page.locator('#retry-btn')).toHaveText('もういちど');
  await expectWalkDots(page, 4, 4);
  ...
  await continueTo(page, LOCATIONS[4]);
  await runToResult(page, LOCATIONS[4].scoreGoal);
  ...
  expect(saved.locationRecords.moonlight_forest.plays).toBe(2);
