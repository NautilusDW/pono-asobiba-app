...
test('中央コンボレーンが可変配置の遠景・近景と4画面サイズで重ならない', async ({ page }) => {
  ...
  // idle中の穴は意図的にinert/disabled。実プレイ中のタップ面積を測るため開始する。
  ...
  for (const viewport of [
    ...
    const geometry = await page.evaluate(() => {
      ...
      const holeRows = holes.map((hole) => ({
        hole,
        window: hole.querySelector('.hh-window').getBoundingClientRect(),
      ...
      const farRows = holeRows.filter(({ slotY }) => slotY < 50);
      const nearRows = holeRows.filter(({ slotY }) => slotY >= 50);
      ...
      const hitTargets = holes.map((hole) => {
        ...
        return {
          ...
          centerHitsHole: document.elementFromPoint(points[0][0], points[0][1])?.closest('.hh-hole') === hole,
          hitPointCount: points.filter(
            ([x, y]) => document.elementFromPoint(x, y)?.closest('.hh-hole') === hole,
          ).length,
        };
      });
      ...
      const farHole = farRows[0].hole;
      const nearHole = nearRows[0].hole;
      farHole.classList.add('is-pressed');
      nearHole.classList.add('is-pressed');
      const pressedScales = {
        farBase: scaleX(farHole.querySelector('.hh-hideout-base')),
        farForeground: scaleX(farHole.querySelector('.hh-hideout-foreground')),
        nearBase: scaleX(nearHole.querySelector('.hh-hideout-base')),
        nearForeground: scaleX(nearHole.querySelector('.hh-hideout-foreground')),
      };
      farHole.classList.remove('is-pressed');
      nearHole.classList.remove('is-pressed');
      // パースで列が斜めになっても、中央コンボ本体と各動物窓が2次元で交差しない。
      const windowClearances = holeRows.map(({ window }) => ({
        horizontal: Math.max(window.left - core.right, core.left - window.right),
        vertical: Math.max(window.top - core.bottom, core.top - window.bottom),
      }));
      return {
        ...
        windowClearances,
        farWindowWidth: farRows.reduce((sum, { window }) => sum + window.width, 0) / farRows.length,
        nearWindowWidth: nearRows.reduce((sum, { window }) => sum + window.width, 0) / nearRows.length,
        farDepthScale: farRows[0].depthScale,
        nearDepthScale: nearRows[0].depthScale,
        hitTargets,
        pressedScales,
        impactScale,
    ...
    expect(
      geometry.windowClearances.every(({ horizontal, vertical }) => Math.max(horizontal, vertical) >= 2),
      `${viewport.width}x${viewport.height}: コンボと全動物窓の2次元のすきま`,
    ).toBe(true);
    expect(geometry.impactScale).toBeGreaterThan(1);
    expect(geometry.impactScale).toBeLessThanOrEqual(1.14);
    expect(geometry.nearWindowWidth / geometry.farWindowWidth).toBeGreaterThan(1.18);
    expect(geometry.nearWindowWidth / geometry.farWindowWidth).toBeLessThan(1.34);
    expect(geometry.farDepthScale).toBeLessThan(0.9);
    expect(geometry.nearDepthScale).toBeGreaterThanOrEqual(0.9);
    expect(geometry.hitTargets.every((target) => target.tagName === 'BUTTON' && target.type === 'button')).toBe(true);
    ...
    expect(geometry.hitTargets.every((target) => target.centerHitsHole)).toBe(true);
    expect(geometry.hitTargets.every((target) => target.hitPointCount >= 4)).toBe(true);
    expect(geometry.pressedScales.farBase).toBeCloseTo(geometry.farDepthScale * 0.96, 3);
    expect(geometry.pressedScales.farForeground).toBeCloseTo(geometry.farDepthScale * 0.96, 3);
    expect(geometry.pressedScales.nearBase).toBeCloseTo(geometry.nearDepthScale * 0.96, 3);
    expect(geometry.pressedScales.nearForeground).toBeCloseTo(geometry.nearDepthScale * 0.96, 3);
