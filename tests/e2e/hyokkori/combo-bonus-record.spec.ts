...
test('中央コンボレーンが可変配置の遠景・近景と4画面サイズで重ならない', async ({ page }) => {
  ...
  for (const viewport of [
    ...
    const geometry = await page.evaluate(() => {
      ...
      const farRows = holeRows.filter(({ hideoutVariant }) => hideoutVariant === 'far');
      const nearRows = holeRows.filter(({ hideoutVariant }) => hideoutVariant === 'near');
      ...
      document.querySelectorAll('.hh-ground-stack').forEach((element) => {
      ...
      const pressedScales = {
        farStack: scaleX(farHole.querySelector('.hh-ground-stack')),
        nearStack: scaleX(nearHole.querySelector('.hh-ground-stack')),
      };
    ...
    expect(geometry.pressedScales.farStack).toBeCloseTo(geometry.farDepthScale * 0.96, 3);
    expect(geometry.pressedScales.nearStack).toBeCloseTo(geometry.nearDepthScale * 0.96, 3);
