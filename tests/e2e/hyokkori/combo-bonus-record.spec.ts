...
test('中央コンボレーンが4画面サイズで上下のキャラ窓と重ならない', async ({ page }) => {
  ...
  for (const viewport of [
    ...
    const geometry = await page.evaluate(() => {
      ...
      // .hh-window の下側は茂み前景に合わせてclipされる。
      // DOM矩形の不可視部分ではなく、キャラが実際に見える下端との間隔を測る。
      const windowVisibleRatio = stage.height <= 430 ? 0.68 : 0.645;
      const topBottom = Math.max(...windows.slice(0, 3).map(
        (rect) => rect.top + rect.height * windowVisibleRatio,
      ));
    ...
    expect(
      geometry.coreTop - geometry.topBottom,
      `${viewport.width}x${viewport.height}: 上段とコンボのすきま`,
    ).toBeGreaterThanOrEqual(2);
    expect(
      geometry.bottomTop - geometry.coreBottom,
      `${viewport.width}x${viewport.height}: コンボと下段のすきま`,
    ).toBeGreaterThanOrEqual(2);
    ...
    expect(geometry.topDepthScale).toBeCloseTo(0.88, 3);
