...
test('3場所×4画面で可変button・遠近・下側マスク・中央コンボ余白を保つ', async ({ page }) => {
  ...
  for (const viewport of viewports) {
    ...
    for (let locationIndex = 0; locationIndex < LOCATIONS.length; locationIndex += 1) {
        ...
        return {
          ...
          holes: holeElements.map((hole) => {
            ...
            return {
              ...
              hideoutCenterError: Math.hypot(baseCenterX - windowCenterX, baseCenterY - windowCenterY),
              ...
              maskSeamTolerance: windowRect.height * 0.12,
            };
          }),
        };
      });
      ...
      for (const [index, hole] of geometry.holes.entries()) {
        ...
        expect(
          hole.hideoutCenterError,
          `${location.id} ${viewport.width}x${viewport.height} hole ${index}: 開口画像の中心`,
        ...
        expect(
          ...
          `${location.id} ${viewport.width}x${viewport.height} hole ${index}: 動物窓と手前縁の継ぎ目`,
        ).toBeLessThanOrEqual(hole.maskSeamTolerance);
      }
