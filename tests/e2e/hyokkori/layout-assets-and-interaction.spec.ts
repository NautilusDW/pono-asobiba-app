...
test('5つの絵本世界・場所別の遠近開口・12種の動物を使い、旧世界素材を読み込まない', async ({ page }) => {
  ...
  await expect(page.locator('#start-goal')).toHaveText('50てんで つぎへ');
...
test('直前に成功した場所だけを次から外し、次地点開始で解除する', async ({ page }) => {
  ...
  await first.evaluate((hole) => hole.click());
  ...
  await second.evaluate((hole) => hole.click());
  await expect(page.locator('#board')).toHaveAttribute('data-last-successful-hole', secondIndex || '');

  // 次地点へ進む50点まで、あと3回以上の通常ハイタッチを成功させる。
  while (Number((await page.locator('#hud-score').textContent())?.match(/\d+/)?.[0] || 0) < 50) {
    await page.clock.runFor(400);
    const next = await advanceToAwake(page);
    await next.evaluate((hole) => hole.click());
  }
