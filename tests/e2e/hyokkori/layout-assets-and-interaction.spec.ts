  ...
  for (const viewport of viewports) {
    ...
    for (let locationIndex = 0; locationIndex < LOCATIONS.length; locationIndex += 1) {
      ...
      const geometry = await page.evaluate(() => {
        ...
        return {
          ...
          holes: holeElements.map((hole) => {
            ...
            const charLiftCqh = Number.parseFloat(holeStyle.getPropertyValue('--char-ground-lift'));
            ...
            return {
              ...
              maskGapTolerance: windowRect.height * 0.12,
              maskOverlapTolerance: windowRect.height * 0.25,
            };
          }),
        };
      });
      ...
      for (const [index, hole] of geometry.holes.entries()) {
        ...
        expect(hole.charLiftCqh).toBe(location.hideoutLayouts[slot.hideout].charLiftCqh);
        expect(
        ...
        expect(
          hole.maskSeamDelta,
          `${location.id} ${viewport.width}x${viewport.height} hole ${index}: 動物窓と手前縁のすきま`,
        ).toBeLessThanOrEqual(hole.maskGapTolerance);
        expect(
          hole.maskSeamDelta,
          `${location.id} ${viewport.width}x${viewport.height} hole ${index}: 手前縁が動物を隠しすぎない`,
        ).toBeGreaterThanOrEqual(-hole.maskOverlapTolerance);
      }
