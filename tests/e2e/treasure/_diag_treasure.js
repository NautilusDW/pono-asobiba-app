...
(function() {
  ...
  function _clearPendingTimers() {
    console.log('[DIAG] _clearPendingTimers, count=' + _pendingTimers.length);
    _pendingTimers.forEach(function(t) { clearTimeout(t); });
    _pendingTimers = [];
  }
  function _later(fn, ms) {
    var id = setTimeout(fn, ms);
    console.log('[DIAG] _later scheduled id=' + id + ' ms=' + ms + ' pendingCountAfter=' + (_pendingTimers.length+1));
  ...
  function _doClose() {
    console.log('[DIAG] _doClose called, _closing=' + _closing);
    if (_closing) return;
  ...
  function _fallbackCss() {
    console.log('[DIAG] _fallbackCss called, started=' + _fallbackStarted + ' finished=' + _finished);
  ...
  function _runQueue() {
    console.log('[DIAG] _runQueue queueLen=' + _queue.length + ' busy=' + _busy);
  ...
  window.showTreasure = function(options) {
    console.log('[DIAG] showTreasure called label=' + (options && options.label));
    _queue.push(options);
    if (!_busy) _runQueue();
  };
  ...
  function _startTreasure(options) {
    console.log('[DIAG] _startTreasure called, label=' + (options && options.label));
    ...
    tapOverlay.addEventListener('click', function _onTapOpen() {
      console.log('[DIAG] _onTapOpen FIRED');
      ...
      var softTimeoutId = _later(function() {
        console.log('[DIAG] softTimeout fired, videoMatch=' + (_video === capturedVideo) + ' readyFired=' + readyFired);
