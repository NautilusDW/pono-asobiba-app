...
namespace Pono.HideSeekCreatures.Gameplay
{
    ...
    {
        private const float InitialFogAmount = 0.92f;
        ...
        private void StartSession(bool showIntro)
        {
            ...
            _ink.SetReduceMotion(_reduceMotion);
            _ink.SetFogAmount(InitialFogAmount);
            _ink.ResetInk();
            SeedInitialClues();
            SeedAmbientInk();
            StartCoroutine(AmbientInkRoutine());
        ...
        private IEnumerator CompleteRoutine()
        {
            ...
            PlayerPrefs.Save();
            yield return FadeFogForCompletion(_reduceMotion ? 0.65f : 1.25f);
        ...
        private void SeedAmbientInk()
        {
            // These broad, low-density splats make the stage feel alive before
            // the first touch without changing the CPU discovery mask.
            _ink.Inject(
                new Vector2(0.08f, 0.22f),
                new Vector2(0.12f, 0.05f),
                new Color(0.34f, 0.76f, 0.74f, 0.09f),
                0.19f);
            _ink.Inject(
                new Vector2(0.92f, 0.78f),
                new Vector2(-0.11f, -0.04f),
                new Color(0.42f, 0.62f, 0.91f, 0.08f),
                0.18f);
            _ink.Inject(
                new Vector2(0.52f, 0.93f),
                new Vector2(-0.03f, -0.12f),
                new Color(0.91f, 0.71f, 0.32f, 0.07f),
                0.17f);
            _ink.Inject(
                new Vector2(0.46f, 0.08f),
                new Vector2(0.05f, 0.11f),
                new Color(0.58f, 0.45f, 0.85f, 0.07f),
                0.16f);
        }

        private IEnumerator AmbientInkRoutine()
        {
            // A few late wisps keep the fluid readable through the opening
            // title. They remain visual-only and stop before idle hints begin.
            for (var cycle = 0; cycle < 4; cycle++)
            {
                yield return WaitForUnpausedSeconds(0.72f);
                if (State == HideSeekStageState.Complete
                    || State == HideSeekStageState.CompleteCelebration)
                {
                    yield break;
                }

                var progress = cycle / 3f;
                var phase = cycle * 1.65f;
                _ink.Inject(
                    new Vector2(Mathf.Lerp(0.13f, 0.87f, progress), 0.88f + Mathf.Sin(phase) * 0.025f),
                    new Vector2(0.08f, -0.10f + Mathf.Cos(phase) * 0.025f),
                    new Color(0.20f, 0.78f, 0.82f, 0.12f),
                    0.15f);
                _ink.Inject(
                    new Vector2(Mathf.Lerp(0.86f, 0.14f, progress), 0.12f + Mathf.Cos(phase) * 0.022f),
                    new Vector2(-0.07f, 0.10f + Mathf.Sin(phase) * 0.025f),
                    new Color(0.88f, 0.48f, 0.82f, 0.10f),
                    0.14f);
            }
        }
        ...
        private IEnumerator FadeFogForCompletion(float duration)
        {
            var elapsed = 0f;
            while (elapsed < duration)
            {
                ...
                {
                    elapsed += Time.unscaledDeltaTime;
                    var amount = Mathf.SmoothStep(InitialFogAmount, 0f, Mathf.Clamp01(elapsed / duration));
                    _ink.SetFogAmount(amount);
                }
                yield return null;
            }
            _ink.SetFogAmount(0f);
        }
