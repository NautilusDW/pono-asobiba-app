...
namespace Pono.HideSeekCreatures.Bootstrap
{
    ...
    public sealed class HideSeekRuntimeBootstrap : MonoBehaviour
    {
        ...
        private void BuildGame()
        {
            ...
            var forestSprite = RuntimeAssetLoader.LoadSprite("HideSeekCreatures/km01/forest_background");
            var backdrop = UiFactory.CreateImage(
                "Forest Backdrop",
                root,
                forestSprite,
                new Color(0.78f, 0.80f, 0.68f, 1f));
            ...
            var backdropVeil = UiFactory.CreateImage(
                "Backdrop Veil",
                root,
                null,
                new Color(0.95f, 0.94f, 0.84f, 0.42f));
            UiFactory.Fill(backdropVeil.rectTransform);

            var contentFrame = UiFactory.CreateImage(
                "Forest Content Frame",
                root,
                forestSprite,
                Color.white);
            ...
            var creatureLayer = UiFactory.CreateRect("Creatures", contentFrame.rectTransform);
            UiFactory.Fill(creatureLayer);
            var foundCreatureLayer = UiFactory.CreateRect("Found Creatures", contentFrame.rectTransform);
            UiFactory.Fill(foundCreatureLayer);
            ...
            for (var i = 0; i < creatures.Length; i++)
            {
                ...
                view.Initialize(HideSeekStageCatalog.Creatures[i], creatureLayer, foundCreatureLayer);
                creatures[i] = view;
            }

            var inkImage = UiFactory.CreateRawImage("FluidInk Surface", contentFrame.rectTransform, Color.white);
            ...
            foundCreatureLayer.SetAsLastSibling();

            var effectsLayer = UiFactory.CreateRect("Celebration Effects", contentFrame.rectTransform);
            ...
            var tutorialInstruction = UiFactory.CreateText(
                ...
                TextAnchor.MiddleCenter,
                TextBrown);
            SetAnchor(tutorialInstruction.rectTransform, new Vector2(0.5f, 0f), new Vector2(0f, 96f), new Vector2(1350f, 104f));
            AddSoftShadow(tutorialInstruction, new Color(1f, 0.98f, 0.86f, 0.96f), new Vector2(3f, -3f));
            ...
            RuntimeQaCapture.AttachIfRequested(gameObject);
        }
