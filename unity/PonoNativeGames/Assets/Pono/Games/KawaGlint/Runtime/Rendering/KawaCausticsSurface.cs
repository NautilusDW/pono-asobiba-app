...
{
    /// <summary>
    /// World-space additive quad that renders animated Voronoi-filament
    /// caustics for the KawaGlint river-cutaway spike. This is a same-shape
    /// port of <c>Pono.AquaLumina.Rendering.AquaCausticsSurface</c> (see that
    /// file for the technique's original derivation and comments -- it is a
    /// reference only and is never modified by this file's existence). It is
    /// drawn by the URP 2D Renderer's own transparency sort (via the
    /// SRPDefaultUnlit pass, see KawaCaustics.shader) at a sorting order that
    /// places it above the riverbed/plants/fish-shadow content and below the
    /// motes/surface-band/shore layers, so light appears to pass through
    /// water before it distorts the scene via the later refraction post-pass
    /// -- the physically plausible layering order (see
    /// KawaGlintBootstrap.CausticsSortingOrder).
    ///
    /// Fully procedural: the quad Mesh and its Material are both built in
    /// code here, there is no .mat/.prefab asset backing this component.
    /// </summary>
    [DisallowMultipleComponent]
    public sealed class KawaCausticsSurface : MonoBehaviour
    {
        ...
        /// </summary>
        public static KawaCausticsSurface Create(Transform parent, Rect worldRect, int sortingOrder, float intensity)
        {
            var shader = Resources.Load<Shader>("KawaGlint/Rendering/KawaCaustics");
            if (shader == null)
            {
                shader = Shader.Find("Pono/KawaGlint/Caustics");
            }
            if (shader == null || !shader.isSupported)
            {
                if (!_missingShaderWarningLogged)
                {
                    Debug.LogWarning("KawaCausticsSurface: KawaCaustics shader is missing or unsupported on this platform; caustics will be skipped.");
                    _missingShaderWarningLogged = true;
                }
                return null;
            }

            var go = new GameObject("KawaCausticsSurface");
            go.transform.SetParent(parent, worldPositionStays: false);

            var surface = go.AddComponent<KawaCausticsSurface>();
        ...
        private void Initialize(Shader shader, Transform parent, Rect worldRect, int sortingOrder, float intensity)
        {
            ...
            {
                name = "KawaCaustics (Runtime)",
                hideFlags = HideFlags.DontSave
            };
            _material.SetColor(TintId, new Color(0.72f, 0.94f, 1.0f, 1f));
            _material.SetFloat(ScaleId, 2.2f);
            _material.SetFloat(SpeedId, 0.5f);
            _material.SetVector(FadeParamsId, new Vector4(0.08f, 0.80f, 0.30f, 0f));
            // _RectExtent is part of this material's documented uniform contract
            // (Intensity/Tint/Scale/Speed/FadeParams/RectExtent/KawaTime --
            // seven total, see KawaCaustics.shader's Properties block for the
            // full pinned list): it carries the quad's own UV extents
            // (uMax = aspect, vMax = 1) so the shader's box-edge soft mask can
            // stay proportionally even on all four sides regardless of
            // worldRect's aspect ratio, without needing a per-frame CPU
            // parameter (set once here, static for the lifetime of the
            // surface). Required precisely because the box-edge soft mask is
            // itself a requirement (no additive quad may show a hard
            // rectangle silhouette against the background) -- any future
            // reader enumerating "the caustics parameters" must treat this as
            // a non-optional entry, not an implementation-private extra.
        ...
        // which would desync this layer from module 4's KawaWave-driven crest
        // line/bobber -- see KawaSurfaceBand.Update for the same pattern).
        private void Update()
        {
            if (_material != null)
            {
