...
-ponoSpikeCaptureMode baseline|full|refraction|surface|actors|caustics
-ponoSpikeCaptureDelay <秒数, 0-10, 既定 1.5>
-ponoQuitAfterCapture
```

| モード | 屈折/疑似反射 | 水面バンド | アクター (魚影/ウキ) | Bloom | コースティックス |
| --- | --- | --- | --- | --- | --- |
| `baseline` | off | off | off | off | off |
| `full` | on | on | on | on | on |
| `refraction` | on | off | off | off | off |
| `surface` | off | on | off | on | off |
| `actors` | off | off | on | off | off |
| `caustics` | off | off | off | off | on |

撮影後、同名 `.txt` に `mode=` / `delay=` / `renderer=` / `refraction=on|off|unavailable` /
`surface=` / `actors=` / `bloom=` / `caustics=on|off|unavailable` / `avgFrameMs=` を書き出します
(該当する Volume コンポーネント/モジュールが見つからない場合は `unavailable` になります)。

## 水のきらめき (コースティックス)

姉妹スパイク AquaLumina (`Pono.AquaLumina.Rendering.AquaCausticsSurface` / `AquaCaustics.shader`) の
「二層アニメーション Voronoi-F1 フィラメント」技法を KawaGlint 専用の新規ファイル
(`Runtime/Rendering/KawaCausticsSurface.cs` / `Content/Resources/KawaGlint/Rendering/KawaCaustics.shader`,
シェーダー名 `Pono/KawaGlint/Caustics`) へ移植したものです。AquaLumina 側のファイルは参照のみで
変更していません。KawaGlint の水中コンテンツ (リバーベッド order10 / 水草 order12 / 魚影 order14) の
上、モート/水面バンド/岸辺 (order18/30/40) の下、sorting order **16** で加算合成描画されます
(`KawaGlintBootstrap.CausticsSortingOrder`)。時間軸は KawaGlint 共通の pinned contract
(`Time.time` 駆動の `_KawaTime`、`KawaSurfaceBand` と同じ) に揃えてあり、レベルロード相対の
`_Time.y` は使いません。特徴点の明滅周期は約12.6秒 (3Hz 点滅なし) で、児童向けの過度な点滅演出
にはなりません。
