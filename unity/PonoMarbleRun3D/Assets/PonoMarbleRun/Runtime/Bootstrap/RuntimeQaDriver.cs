...
namespace Pono.MarbleRun3D.Bootstrap
{
    ...
    public sealed class RuntimeQaDriver : MonoBehaviour
    {
        ...
        private IEnumerator WaitForSampleGoal(string modeId)
        {
            ...
            _controller.StartRun();
            var startedAt = Time.realtimeSinceStartup;
            var timeout = modeId == "starter" ? 55f
                : modeId == "sample6" ? 45f
                : modeId == "sample5" ? 42f
                : 24f;
        ...
        private void WriteQaReport()
        {
            ...
            var json = "{\n"
                ...
                + "  \"camera_yaw\": "
                + _controller.OrbitCamera.Yaw.ToString("0.00", CultureInfo.InvariantCulture) + ",\n"
                + "  \"camera_pitch\": "
                + _controller.OrbitCamera.Pitch.ToString("0.00", CultureInfo.InvariantCulture) + ",\n"
                + "  \"camera_distance\": "
