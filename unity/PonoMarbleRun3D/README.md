...
- チャレンジ １: まっすぐ・カーブ・くだりざかで自由な経路を作ります。
- チャレンジ ２: トンネルとシーソーを含むコースを作ります。
- チャレンジ ３: ぶんき・じょうご・ドミノを含む複数経路を試します。
- じゆうに つくる: 全10部品を個数制限なしで使えます。スタートとゴールも移動できます。

各チャレンジは接続グラフと部品数だけを条件にし、固定の正解配置を持ちません。テストでは各チャレンジに対して少なくとも2種類の接続方法を受理することを確認しています。

## 部品

スタート、ゴール、まっすぐ、カーブ、くだりざか、ぶんき、トンネル、じょうご、シーソー、ドミノを実装しています。木製玩具を意識した明るい色、木の台、接続ペグとリング、配置前の半透明ゴースト、配置不可時の赤い×表示を使用します。
...
- 玉は `ContinuousDynamic` と補間を使用し、最大速度14 m/s、初速5.8 m/s。
- ソルバー反復10回、速度反復4回。玉だけは12回・5回へ増強。
- 木製レールと玉に低反発・低摩擦の `PhysicsMaterial` を使用。
- くだりざかは約16.70度で、1段（0.90 m）の高低差へ正確につながります。高い接続点へ置いた平面部品は自動で上段へスナップします。
...
モードごとに1コースを保存します。macOSのサンドボックス保存先は次です。
