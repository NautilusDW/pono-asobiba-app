...
- 部品を置く: 左の部品カードは、実際の3D部品を同じ向きから撮った絵が主役です。絵を盤面までドラッグします。カードを一度押してから盤面を押す方法も使えます。部品名と残り個数は14pxの補助表示です。
...
- 回転・削除: 置いた部品を一度押して選び、画面下の回転アイコンまたは削除アイコンを押します。短いかな表示も添えています。回転は右へ90度単位です。さかみち・かいだん・のぼるみちは2回押して180度回すと、のぼり向きとくだり向きが入れ替わります。
- Undo・全消去: 戻すアイコンと全消去アイコンを使います。全消去は誤操作防止のため2回押します。
- 試走: 開始の絵＋「ころがす」で6色の玉が時間差で流れます。落ちた玉だけが自動でスタートへ戻り、「たまを もどす」で全ての玉を一緒に戻せます。最初の玉が着いたあとも少し待って、ほかの玉の動きを見せてからゴール演出へ進みます。
- 保存: 保存の絵＋「ほぞん」で現在のコースを保存し、読込の絵＋「よむ」で再編集します。
- カメラ: 編集中はコース全体と高さへ自動で合わせます。空いている盤面のドラッグで回転、マウスホイールでズームします。タッチでは2本指の移動で回転、ピンチでズームします。全景の絵＋「うえ」と回転の絵＋「ななめ」で視点を切り替えられます。
- 試走カメラ: 開始の絵＋「ころがす」を押すと6色の玉へ近づき、進む向きに合わせて見やすい斜め後ろへゆっくり回り込みながら自動で追いかけます。試走を止めずに盤面ドラッグ／1本指ドラッグで回転、ホイール／ピンチでズームできます。手動操作のあとは3.5秒その視点を保ち、その後なめらかに自動追従へ戻ります。「みわたす」で全景へ戻り、「おいかける」で自動追従へすぐ戻せます。一時停止中もカメラだけは動かせます。
- 一時停止: 停止の絵＋「とめる」で時間と3D物理を完全に止め、開始の絵＋「つづける」で戻します。
...
UIは紙色の明るい背景を基調に、空色 `#A9DCF2`、ミント `#B8E2C1`、さくら `#F4B6B0`、ラベンダー `#D4C3EC` の4色だけで操作の種類を分けています。影と縁取りのある絵本カード、7つのメニューバッジ、選択マークを使い、色数を抑えながら子どもが押す場所を見分けやすくしています。子ども向けUIの書体は丸みのあるKosugi Maruで統一し、利用できない環境ではNoto Sans JPへフォールバックします。

左パレットには、`WoodenPieceFactory` が作る実際の3D形状からUnity Editorで1回だけ生成した、透明背景の部品画像19種を使います。完成プレイヤーはPNGを読むだけで、実行時のサムネイル用CameraやRenderTextureを作りません。操作ボタンは、GPT Image 2で生成した12操作の4×3アトラスを724×543 px／562,866 bytesへ最適化し、回転、削除、戻す、全消去、保存、読込、カメラ回転、全景、玉リセット、開始、一時停止、編集を絵主体で示します。意味を補う場合だけ短いかなを添えます。配置前の半透明ゴーストと、配置不可時の赤い×表示も維持しています。生成時の生素材は `tmp/alpha_pending/1175g-marble-run-pastel-camera/` に残しています。
...
- `Gameplay/OrbitCameraController.cs`: 編集時の全景操作、試走時の玉群追従、コース先読み、遮蔽を避ける自動回り込み、手動操作からの復帰を管理。
- `UI/MarbleRunUi.cs`: マウス・タッチ共通のuGUI、絵本カード、画像中心の部品パレットと操作ボタン、選択状態、Safe Area対応。
- `UI/ActionIconAtlas.cs`: 4×3操作アトラスを12セルへ分け、必要時に1度だけキャッシュ。
- `Editor/PieceThumbnailAssetGenerator.cs`: `WoodenPieceFactory` の実形状から透明背景の部品PNG 19種をEditor上で生成。プレイヤー実行時には使いません。
...
試走時の追従カメラは玉との距離を11.5〜16、ピッチを36〜40度に保ちます。複数の玉は位置の中央値を中心に追い、離れすぎた玉を一時的に追跡対象から外すため、落下やリセットで画面が急に引かれません。玉群の速度は1.2秒、進行方向は1.5秒かけて平滑化し、螺旋で玉が素早く向きを変えても一周ごとの動きを追わず、全体をゆっくり追います。向きの判断は現在速度だけに頼らず、先に続くコースを78%の比率で取り入れます。

自動回転は通常10度／秒、必要時でも最大12度／秒です。0.20秒間隔で正面と左右35度・70度の5候補へSphereCastし、壁やトンネルで玉が隠れそうな時は、次の進路が見える側へ回り込みます。左右を頻繁に行き来しないよう、選んだ側を1.35秒保つヒステリシスも設けています。マウスやタッチでカメラを動かしたあとは、自動追従を3.5秒保留します。
