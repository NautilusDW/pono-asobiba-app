...
## Auto-Extracted Best Practices (Updated: 2026-03-16)

以下のルールは過去のタスク実績から自動抽出されたベストプラクティスです。

1. **エラー発生後に別のアプローチに切り替えた** (検出回数: 208, 例: テスト: LP素材画像修正とカラーパレット動作確認, PDF INVALID_ARGUMENT修正: サイズベース動的ページ制限 + 重複エラーメッセージ解消 + ユーザー向けエラー改善, 素材から動画を作成機能 + モジュール分離骨組み実装)
2. **編集前にファイルを読んで理解した** (検出回数: 202, 例: テスト: LP素材画像修正とカラーパレット動作確認, PDF INVALID_ARGUMENT修正: サイズベース動的ページ制限 + 重複エラーメッセージ解消 + ユーザー向けエラー改善, 素材から動画を作成機能 + モジュール分離骨組み実装)
3. **小さな単位で検証しながら進めた** (検出回数: 201, 例: テスト: LP素材画像修正とカラーパレット動作確認, PDF INVALID_ARGUMENT修正: サイズベース動的ページ制限 + 重複エラーメッセージ解消 + ユーザー向けエラー改善, 素材から動画を作成機能 + モジュール分離骨組み実装)
4. **実装前にコードベースを探索した** (検出回数: 112, 例: PDF INVALID_ARGUMENT修正: サイズベース動的ページ制限 + 重複エラーメッセージ解消 + ユーザー向けエラー改善, 素材から動画機能実装+sys.exit安全化+コンテキスト永続化+分析結果表示調査, 素材から動画Phase2: URL深掘り+複数ファイル+ワンショット統合分析)
5. **テストを先に書いてから実装した (TDD)** (検出回数: 20, 例: 素材から動画を作成機能 + モジュール分離骨組み実装, スリップ編集機能: ソースモニターでvideoSourceIn指定+使用区間インジケーター+ここから使用ボタン, A/B/C: ローカル動画延長・非破壊テイク管理・SRT字幕インポート実装)
