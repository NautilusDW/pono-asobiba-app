...
## Auto-Extracted Best Practices (Updated: 2026-02-26)

以下のルールは過去のタスク実績から自動抽出されたベストプラクティスです。

1. **小さな単位で検証しながら進めた** (検出回数: 11, 例: 5歳向けなぞり書きWebアプリのプロトタイプ作成（HTML/CSS/JS単一ファイル）, 書き順ガイドのSVGパス精度改善（座標自動検出+Catmull-Romスプライン+データ全面書き直し）, Hanzi Writerライブラリ導入による書き順ガイド・なぞり判定の全面刷新)
2. **エラー発生後に別のアプローチに切り替えた** (検出回数: 11, 例: 5歳向けなぞり書きWebアプリのプロトタイプ作成（HTML/CSS/JS単一ファイル）, 書き順ガイドのSVGパス精度改善（座標自動検出+Catmull-Romスプライン+データ全面書き直し）, Hanzi Writerライブラリ導入による書き順ガイド・なぞり判定の全面刷新)
3. **実装前にコードベースを探索した** (検出回数: 11, 例: 5歳向けなぞり書きWebアプリのプロトタイプ作成（HTML/CSS/JS単一ファイル）, 書き順ガイドのSVGパス精度改善（座標自動検出+Catmull-Romスプライン+データ全面書き直し）, Hanzi Writerライブラリ導入による書き順ガイド・なぞり判定の全面刷新)
4. **編集前にファイルを読んで理解した** (検出回数: 10, 例: 書き順ガイドのSVGパス精度改善（座標自動検出+Catmull-Romスプライン+データ全面書き直し）, Hanzi Writerライブラリ導入による書き順ガイド・なぞり判定の全面刷新, AnimCJK教科書体SVGデータを表示・判定の単一ソースとする設計への全面刷新)
