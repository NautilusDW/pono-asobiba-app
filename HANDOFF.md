...
#### 1. 生成する画像 (全 16 種、 検証用最小セット)

これは **本番用ではなくレイアウト検証 (画像が指定解像度で正しく表示されるか) のみが目的**。
チップ / 色マル / ドットなど複数バリエーションが必要な要素も、 検証段階では **1 種類を流用** で OK。

現在のレイアウト寸法は `quizland/preview/full/saved-layout.json` を参照。 saved-layout に明示寸法が無いものは `getBoundingClientRect` で実寸計測。

##### ヘッダー左ピル (5 種)
| # | 要素 | 寸法 (px) | ファイル名 |
|---|---|---|---|
| 1 | ピル背景フレーム | 856×142 | `hdr-pill.png` |
| 2 | 博士アイコン | 127×127 | `owl-icon.png` |
| 3 | タイトル札 (文字込みで OK) | 296×128 | `title-card.png` |
| 4 | 1/5 ボックス | 119×63 | `progress-num.png` |
| 5 | 進捗ドット (1 種、 5 個に流用) | 28×28 | `dot.png` |

##### ヘッダー右ボタン (2 種)
| # | 要素 | 寸法 (px) | ファイル名 |
|---|---|---|---|
| 6 | ctrl-btn おしらせ | 120×120 | `ctrl-btn-news.png` |
| 7 | ctrl-btn せってい | 120×120 | `ctrl-btn-settings.png` |

##### 左カラム (3 種)
| # | 要素 | 寸法 (px) | ファイル名 |
|---|---|---|---|
| 8 | 問題文カード | 実寸 (約 1000×170) | `q-text-card.png` |
| 9 | 音声ボタン | 80×80 | `audio.png` |
| 10 | 問題ボード | 856×512 | `board.png` |

##### 右カラム (3 種、 各 1 種を 4 個に流用)
| # | 要素 | 寸法 (px) | ファイル名 |
|---|---|---|---|
| 11 | 答えトレイ枠 | 704×512 | `answer-tray.png` |
| 12 | 選択肢チップ (1 種、 4 個に流用) | 322×220 | `chip.png` |
| 13 | 色マル (1 種、 4 個に流用) | 110×110 | `circle.png` |

##### ヒント + キャラ (2 種、 右カラム下部にあるが「下段」という旧名称ではない)
| # | 要素 | 寸法 (px) | ファイル名 |
|---|---|---|---|
| 14 | ヒント帯 | 491×122 | `hint.png` |
| 15 | キャラクター (フクロウ博士) | 199×122 | `character.png` |

##### スロット (1 種、 動的生成のテンプレ)
| # | 要素 | 寸法 (px) | ファイル名 |
|---|---|---|---|
| 16 | ItemSlot (1 種、 サイズ変えて使い回し) | 240×240 | `item-slot.png` |

→ **計 16 種**。 全部マスト。 検証成功後、 必要な色違い / バリエーション画像 (チップ赤青黄緑、 ドット active/inactive、 スロット用途別 等) は別フェーズで追加生成する。
