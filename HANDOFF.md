...
- 2026-04-30 ・Codex が `quizland/index.html` のフクロウ博士問題画面フレームを再調整。`border-image-repeat` で葉・どんぐり装飾がタイル状に繰り返され、装飾サイズと密度がばらついていたため、`ui_*_frame.png` の9-slice流用をやめ、ヘッダー/HUD/問題枠/回答枠を同じ木目風CSSフレームに統一。問題チップが左下へ落ちる副作用も修正し、横画面では問題チップを上段左、問題文を上段中央、問題ボードを下段全幅に固定。未プッシュ。(by Codex)
