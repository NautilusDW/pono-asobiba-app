...
### 🔁 Codex 依頼: Quizland パーツ — 装飾を外周マージンに収める再生成 (2026-05-02)

**前回の再生成の評価**: 透過と寸法は OK だが、 **装飾 (葉/どんぐり/花/ツル/羽根ペン) と フレーム枠線が画像の内側を大きく侵食** していて、 実際にコンテンツを置ける内側面積が窮屈。 加えていくつか個別の問題あり。

bbox 外側に装飾を逃がす案も検討したが、 上下端で切れたり隣接フレーム同士でぶつかる懸念があるため、 **「装飾は画像内に留めつつ、 外周マージンに押し込めて、 内側を広く取り直す」 方向で再生成希望**。

#### 共通ルール

1. **装飾 (葉/どんぐり/花/ツル/羽根ペン等) は画像の外周 10% マージン内に収める**。 中心の 80% × 80% は **クリーンな content area** として、 文字/中身を乗せても何にも被らないように。
2. **フレーム線は細めに**。 厚すぎる木枠で内側を侵食しない。 木の質感はそのまま保って良いが幅は控えめ。
3. **「サブフレームの焼き込み」 禁止**。 例えば `hdr-pill.png` の中に 「1/5」 のオーバル枠が焼き込まれてしまっているが、 これは `progress-num.png` が別個に用意されているので、 hdr-pill 側は **空のピル枠だけ** にすること。 同様に他の 「親に子フレームが焼き込み」 は剥がすこと。
4. **寸法 (縦横比) は前回と同じ表で**。 RGBA 透過も維持。

#### 個別の問題と希望

| ファイル | 現状の問題 | 希望 |
|---|---|---|
| `hdr-pill.png` | 中に 「1/5」 オーバル枠が焼き込まれている (progress-num と二重化) | 空のピル枠だけ。 中身要素 (owl-icon / title-card / progress-num / dot) は別画像で乗せる |
| `q-text-card.png` | 左上の羽根ペン + 右側の謎の丸 + 角の葉っぱで、 中央の文字を置ける帯が 60% 程度に狭まっている。 また右側の丸は audio.png (スピーカー) のはずなので二重化 | 装飾は外周 10% 以内に。 audio 用の丸は焼き込まない (audio.png が別個) |
| `board.png` | 木枠 + 角の葉/どんぐり/花が全周 25〜30% 食い込んで、 中心のパッチメント (4 スロット領域) が窮屈 | 外周マージンに装飾を押し込めて、 内側 80% を 4 スロット用にクリーンに確保 |
| `answer-tray.png` | 中に描かれた土台が **真四角の見た目** になっており、 横長 (704×512 ≈ 1.375:1) のトレイ形状に見えない | キャンバス比 (1.375:1) に合わせた **横長の土台** を描くこと。 4 チップが 2×2 でゆったり乗る幅 |
| `chip.png` | 外枠 + 装飾で、 中の circle 領域が小さい | 装飾控えめ、 内側に circle.png + 文字が乗る空きを大きく |

#### 確認方法

- `quizland/preview/full/index.html` のヘッダー右側 **🔁 グリッド比較** トグルで、 旧グリッド (色付き正方形) と新画像を切替えて寸法/内側広さを目視確認できる。 旧グリッドのベタ塗り面積 ≒ 望ましい content area の広さ。
- 仕上がった PNG は `python scripts/auto_optimize_image.py <files...>` を必ず通す。 透過維持。
