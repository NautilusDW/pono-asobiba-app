...
### 5.1 画像生成 ✕ 後処理 の役割分担 (2026-05-06 改訂版 — 2026-05-06b)

| 工程 | 担当 |
|---|---|
| **生成** (GPT Image 2 等で raw 画像生成) | Codex |
| **アルファ抜き** (白/単色背景の透過化) | **Codex** (実験中 — チーム内クロスレビューで品質担保) |
| **切り抜き** (split / crop / 個別オブジェクト分離) | **Codex** (実験中 — 同上) |
...
- **2026-05-06b (現行)**: 再び Codex に alpha 抜き + crop を任せる。 ただし**チーム編成 + クロスレビュー**で品質担保 (= 単一エージェント生成の当時とは違う体制)。 一定期間運用してダメなら 2026-05-06a に戻す。

納品先: `tmp/alpha_pending/alpha/<NN>/` 配下に**透過 PNG / 個別ファイル切り出し済**で置く。 ユーザーが「< NN/ 入れた」と伝える → Claude が `assets/` に配置 + 反映。
