...
### 1.1 ★2系統 staging と共通コード同期ルール (2026-06-16 追加)
...
- OtoTouch / QuizLand / Maze / Bento / 共通UI / `play.html` / `sw.js` など、 本版とアプリ版の両方に存在する機能を直した場合は、 **同じ修正を `develop-app` と `develop` の両方へ入れる前提**で作業する。
...
- 片方のブランチだけに入れてよいのは、 以下のいずれかを満たす場合のみ:
  ...
  - 本版だけの緊急修正で、 ユーザーが片側のみ反映を明示した変更
  - デプロイ/検証用の一時差分で、 HANDOFF.md に片側限定である理由と後続同期予定を書いた変更

**作業完了時の必須確認**:
- 変更ファイルが共通領域かどうかを判断する。
- 共通領域なら完了報告に「`develop-app` / `develop` の同期方針」を明記する。
...
- `wrangler.toml` 行 43-49: `[env.staging]` (LP staging, worker name `pono-asobiba-staging`)
- `wrangler.toml` 行 57-72: `[env.staging-app]` (App staging, worker name `pono-asobiba-app-staging`, `APP_BUILD = "1"`)
