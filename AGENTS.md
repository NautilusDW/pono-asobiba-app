...
7. **画像生成モデルは GPT Image 2 以外使わない。** (Codex 側の仕事。 DALL-E / Stable Diffusion / Imagen / Flux / Nano Banana 等は使用禁止) 既存アセットがすべて GPT Image 2 で生成されているため、 別モデルを混ぜると画風が揺らぐ。これは「モデル」の固定ルールであり、Codex 内蔵 ImageGen / SpriteCook のどちらを制作パイプラインとして使うかとは分けて扱う。SpriteCook 経由でも必ず `model=gpt-image-2` を明示し、SpriteCook のデフォルトモデルへ任せない。§5.1.0 と整合。
   ...
   - **制作パイプラインの既定値は Codex 内蔵 ImageGen。** ただし、オートタイル、スプライトシート、フレームアニメーション、透明背景のゲーム素材、参照画像による量産、Unity / Godot 向け書き出しなど、SpriteCook の専用機能が品質・整合性・実装効率に明確に有利な場合、エージェントは生成前に「今回は SpriteCook が適している理由」「使用予定モデルが GPT Image 2 であること」「予想消費クレジット」をユーザーへ提示し、どちらを使うか確認する。
   - ユーザーが依頼内で SpriteCook を明示指定した場合は、その指定を制作パイプラインの承認として扱う。別モデルの使用許可とは扱わず、`model=gpt-image-2` 固定は維持する。複数枚・アニメーション・タイルセット等のバッチ開始前には残高と予想消費クレジットを知らせる。
   - SpriteCook 生成物の背景除去・余白トリミング・縮小は、画質を保てる単純な素材ならローカルの決定的な後処理を優先し、追加クレジットを消費しない。髪・半透明・発光・アニメーションなどローカル処理で破綻しやすい素材だけ、予想クレジットを提示して承認を得てから SpriteCook の専用処理を使う。
   - SpriteCook を無断で選択して課金クレジットを消費しない。接続不能・残高不足・GPT Image 2 選択不能の場合も別モデルへ自動フォールバックせず、Codex 内蔵 ImageGen へ戻すかユーザーへ確認する。
...
>
> **画像生成モデルはすべて GPT Image 2 を使用することを厳格に守ってください。Codex 内蔵 ImageGen と、`model=gpt-image-2` を明示した SpriteCook のどちらも利用可能ですが、制作パイプラインの選択は §2 ルール 7 に従ってください。**
>
> **ユーザー指示に「生成」「生成して」「作って」等があり、対象が見た目素材なら、SVG / Canvas / CSS / PIL / 手書きベクター等で代用せず、GPT Image 2 で画像アセットを生成してください。**
>
> **Codex 内蔵 ImageGen ルートでは、アルファチャンネル抜きと切り抜きは行わないでください。** 生成 API のレスポンスをそのまま raw 画像として保存。 後処理 (alpha 透過化、 個別 PNG 切り出し) はユーザーがローカルの Photoshop で手作業します (2026-05-06c ポリシー)。ただし、ユーザー承認済みの SpriteCook ルートでは、ローカルの決定的な後処理を第一候補として背景除去・トリミング・縮小を行ってよい。SpriteCook の有料背景除去・スライス等は、ローカル処理で品質維持が難しい場合に限り、予想クレジットを提示して承認後に利用する。元の生成 asset ID と raw URL / raw 保存物を追跡可能な形で残す。
>
...
これは Codex に**オーケストレーター専任モード**で動いてもらうための指示。 Codex が単独で全部やろうとせず、 サブエージェントを組成して並列実装 + クロスレビューする体制を作るための前置き。後処理は上記の制作パイプライン別ルールに従う。

※ クロスレビュー義務の一般ルールは §4.8 を正とする。 本節は画像生成依頼時の追加テンプレであり、 クロスレビュー対象を画像生成に限定するものではない。

**画像生成モデル統一**: GPT Image 2 以外のモデル (DALL-E / Stable Diffusion / Imagen / Flux / Nano Banana 等) を混ぜると画風が揺らぐ。 既存 quizland アセットは全て GPT Image 2 で生成されているので、新規も同モデルで揃える (§2 ルール 7)。SpriteCook は別の画像モデルではなく、`model=gpt-image-2` 固定でのみ使えるゲーム素材制作パイプラインとして扱う。また、ユーザーが「生成」と言った見た目素材を SVG / Canvas / CSS / PIL などのコード生成で置き換えない。
...
このコスト無制限ポリシーは SpriteCook クレジットには適用しない。SpriteCook を使う場合は、開始前に `get_credit_balance` と現行モデル料金を確認し、ユーザー承認済みの範囲だけ実行する。
