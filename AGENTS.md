...
>
> **Codex 内蔵 ImageGen ルートでは、アルファチャンネル抜きと切り抜きは行わないでください。** 生成 API のレスポンスをそのまま raw 画像として保存。 後処理 (alpha 透過化、 個別 PNG 切り出し) はユーザーがローカルの Photoshop で手作業します (2026-05-06c ポリシー)。ただし、ユーザー承認済みの SpriteCook ルートでは、同サービスの目的である透明背景化、背景除去、スライス、スプライトシート / タイルセット / engine-ready export を利用してよい。元の生成 asset ID と raw URL / raw 保存物を追跡可能な形で残す。
>
