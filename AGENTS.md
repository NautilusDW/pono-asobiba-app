...
- tier/APP_BUILD で出し分ける変更をしたら、 完了報告に「LP staging (free/book) と App staging (sub) でそれぞれどう見えるか」を明記する。
...
| --- | --- | --- | --- | --- |
| `develop-app` | **App staging (アプリ版)** | `https://pono-asobiba-app-staging.ndw.workers.dev/` | GitHub Actions が `wrangler deploy --env staging-app` 実行 (push で LP staging と直列)。 APP_BUILD=1 が HTML に注入されて `window.__APP_BUILD__=1` 化、 sub tier 全機能解放 | `wrangler deploy --env staging-app` |
