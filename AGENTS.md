...
- **ブランチ運用**:
  - `develop` → push で **本版 staging に自動デプロイ** (`https://pono-asobiba-staging.ndw.workers.dev/`) — free + book tier の従来版テスター動線
  - `develop-app` → push で **アプリ版 staging に自動デプロイ** (`https://pono-asobiba-app-staging.ndw.workers.dev/`) — sub tier ベースの新ミニゲーム搭載版。 本版テスターには見せない
  - `master` → push で **production に自動デプロイ** (`https://pono.kodama-no-mori.com/`、 ユーザー明示指示時のみ)
- **手動デプロイ**: `wrangler deploy --env staging` (本版 staging) / `wrangler deploy --env staging-app` (アプリ版 staging) / `wrangler deploy` (production)
- **アプリ版限定機能の gating**: `env.APP_BUILD === "1"` のとき worker.js が HTML に `<script>window.__APP_BUILD__=1;</script>` を注入する。 各 JS は `if (window.__APP_BUILD__) { ... }` で分岐すること。 develop-app のみ APP_BUILD=1、 develop / master は未設定 (= falsy)。

### 1.1 ★2系統 staging と共通コード同期ルール (2026-06-16 追加 / 2026-07-10 改訂)

> **【2026-07-10 決定】単一トランク統合が承認済み**（計画: [`docs/branch-unification-plan.md`](docs/branch-unification-plan.md)、**実装は未着手**）。`develop` / `develop-app` の 2 ブランチ並行開発は将来的に `develop-app` 単一トランク + 環境フラグ/tier 出し分けへ統合される予定だが、統合完了（計画書 Phase 5 まで完了）まではこのセクションの現行ルールがそのまま有効。実装 Phase の進捗は計画書冒頭のチェックリストで確認すること。この注記だけを見て「もう統合済み」と誤解しないこと。

このリポジトリには **配信先としての2系統** があるが、 **共通ゲーム/共通UIを2本のブランチで別々に育ててはいけない**。

- `develop-app` はアプリ版 staging の開発正本。
- `develop` は本版/絵本購入版 staging の配信用ブランチ。
- `master` は production。 ユーザー明示指示時のみ。
- OtoTouch / QuizLand / Maze / Bento / 共通UI / `play.html` / `sw.js` など、 本版とアプリ版の両方に存在する機能を直した場合は、 **同じ修正を `develop-app` と `develop` の両方へ入れる**というのが 2026-06-16〜2026-07-10 の原則だった。**【2026-07-10 更新: interim ルール（移行完了まで有効）】** 単一トランク統合計画の承認に伴い、この「両ブランチへ同期」は **移行完了までの経過措置であり、原則停止**する。現行の interim ルールは以下のとおり（詳細は `docs/branch-unification-plan.md` の D8 を正とする）:
  - `develop` への手動 sync は原則停止（統合待ち）。
  - LP 緊急修正のみ `develop` へ直接コミットしてよい。その内容は統合完了時に取り込む。
  - 新規共通機能はすべて `develop-app` で開発する（`develop` への同時投入は行わない）。
- アプリ版だけに見せたい差分は、 ブランチ差分ではなく `window.__APP_BUILD__` / tier 判定 / feature flag で隠す。 共通ファイルをアプリ版だけ別挙動にしたい場合も、 まず flag 分岐で実装する。
- 片方のブランチだけに入れてよいのは、 以下のいずれかを満たす場合のみ:
  - アプリ版限定の新ミニゲーム/未公開機能で、 本版に見せないことが目的の変更
  - 本版だけの緊急修正で、 ユーザーが片側のみ反映を明示した変更（= 上記 interim ルールの LP 緊急修正に相当）
  - デプロイ/検証用の一時差分で、 HANDOFF.md に片側限定である理由と後続同期予定を書いた変更

**作業完了時の必須確認**:
- 変更ファイルが共通領域かどうかを判断する。
- 共通領域なら完了報告に「`develop-app` / `develop` の同期方針」を明記する（2026-07-10 以降は interim ルールにより「同期不要・develop-app のみ」が原則になる点に注意）。
- まだ片側ブランチにしか入っていない場合は、 完了扱いにせず、 「ローカル確認のみ」「app staging のみ」「develop 同期待ち」など状態を明確に書く。
- staging で確認してほしい時は、 どのURLに反映済みかを必ず書く。 未push/未デプロイの場合は、 staging URL を案内せず、 ローカルプレビューURLを案内する。
...
| --- | --- | --- | --- | --- |
| `develop-app` | **App staging (アプリ版)** | `https://pono-asobiba-app-staging.ndw.workers.dev/` | GitHub Actions が `wrangler deploy --env staging-app` 実行。 APP_BUILD=1 が HTML に注入されて `window.__APP_BUILD__=1` 化、 sub tier 全機能解放 | `wrangler deploy --env staging-app` |
| `develop` | **LP staging (本版/絵本販売)** | `https://pono-asobiba-staging.ndw.workers.dev/` | GitHub Actions が `wrangler deploy --env staging` 実行。 APP_BUILD 未設定 (= falsy)、 free/book tier 体験 | `wrangler deploy --env staging` |
...
**配信実装の参照箇所 (2026-07-10 行番号ズレ修正済み):**
- `wrangler.toml` 行 14-18: top-level `routes` (production の `pono.kodama-no-mori.com` custom_domain)
- `wrangler.toml` 行 67-73: `[env.staging]` (LP staging, worker name `pono-asobiba-staging`)
- `wrangler.toml` 行 81-116: `[env.staging-app]` (App staging, worker name `pono-asobiba-app-staging`, `APP_BUILD = "1"` は行 85-86)
- `.github/workflows/deploy.yml` 行 49: `command: ${{ github.ref == 'refs/heads/master' && 'deploy' || (github.ref == 'refs/heads/develop-app' && 'deploy --env staging-app' || 'deploy --env staging') }}`

**運用ルール:**
- ユーザーに staging URL を提示する前に、 必ずどちらの env (App or LP) を指しているか確認する
- 本プロジェクトの app タスク (`play.html` 以降のゲーム本体、 `bento/`, `puzzle/`, `maze/`, `quizland/`, `oto/`, `bubble/`, `aquarium/` 等) は **App staging (`pono-asobiba-app-staging.ndw.workers.dev`)** が正解。 `develop-app` ブランチで作業しているなら自動的にこれ
- LP (`index.html`、 `message/`、 ランディング素材) タスクのときだけ LP staging (`pono-asobiba-staging.ndw.workers.dev`)
- §1.1 の **2系統 staging と共通コード同期ルール** と整合: 共通領域 (OtoTouch / QuizLand / Maze / Bento / `play.html` / `sw.js`) を触ったら**両 staging への反映状況**を完了報告に明記
- DEPLOY-FACT system reminder にある URL は LP staging を指している場合があるが、 文脈 (どの branch で作業中か、 タスクが LP か app か) で判断。 機械的にコピペしない
- 「staging」 とユーザーが言ったとき、 ambiguity があれば 「App staging (`...-app-staging...`) ですか、 LP staging (`pono-asobiba-staging...`) ですか?」 と 1 行で確認してから進める
- ★ **間違えやすいポイント**: `pono-asobiba-staging.ndw.workers.dev` (LP) と `pono-asobiba-app-staging.ndw.workers.dev` (App) は `-app-` の差だけで見た目が酷似。 URL を貼るときは差分箇所を強調 (太字 / バッククォート) して目視確認しやすくする

#### 1.2.1 【参考・未実装】移行後の姿 (単一トランク統合完了後)

> ⚠️ **この節は `docs/branch-unification-plan.md` の Phase 5 まで完了した後にのみ有効になる「将来像」であり、2026-07-10 時点ではまだ何も実装されていない。** 上の表・運用ルールが依然として現行の正本。両者を混同しないこと。

統合完了後は `develop` は凍結され、`develop-app` への 1 回の push で以下の 2 env に **同一コミットが同時に反映される**予定 (`.github/workflows/deploy.yml` を 2 step 直列デプロイに変更、D2)。

| ブランチ | 環境 | URL | 備考 |
| --- | --- | --- | --- |
| `develop-app` | App staging | `https://pono-asobiba-app-staging.ndw.workers.dev/` | `--env staging-app` (APP_BUILD=1 → sub tier) |
| `develop-app` (同一 push) | LP staging | `https://pono-asobiba-staging.ndw.workers.dev/` | `--env staging` (APP_BUILD 未設定 → free/book tier) |
| `develop` | — (凍結) | — | 移行完了時に凍結。以後 push しない |
| `master` | production | `https://pono.kodama-no-mori.com/` + `https://pono-asobiba-app.ndw.workers.dev/` | 変更なし。ユーザー明示指示時のみ |

移行後は §1.1 の「2系統 staging と共通コード同期ルール」自体が不要になる (同期対象のブランチが 1 本になるため)。詳細・設計決定 D1-D10・実装 Phase の進捗は必ず [`docs/branch-unification-plan.md`](docs/branch-unification-plan.md) を参照すること。
...
- **3 ブランチ共通の単一カウンタ運用**: `develop` / `develop-app` / `master` で CACHE_VERSION は**同一の連番**を共有する。 ブランチ別に独立カウントしない (merge 時に番号衝突 → 古いキャッシュ掴みっぱなしの事故を回避するため)。 develop-app で +1 して push したら、 次に develop を触る人は **その値からさらに +1** する。
...
4. **シークレットウィンドウ** で staging URL を開いて**実際に動作確認**:
   - 本版 (develop): `https://pono-asobiba-staging.ndw.workers.dev/`
   - アプリ版 (develop-app): `https://pono-asobiba-app-staging.ndw.workers.dev/`
...
deploy   : git push origin develop      (本版 staging 自動)
            git push origin develop-app  (アプリ版 staging 自動)
...
gating   : window.__APP_BUILD__ === 1 (develop-app のみ true)
画像最適化: python scripts/auto_optimize_image.py <path>
SW バンプ : sw.js 4行目 CACHE_VERSION を +1 (develop/develop-app/master 共通カウンタ)
```
