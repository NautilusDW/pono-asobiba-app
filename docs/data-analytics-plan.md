...
| `rating_submitted` | `game_id`, `star_score`(1-5) | `common/rating-modal.js:215-255` `postStarToAppsScript`成功パスに相乗り | **既存GAS**(client_id/session_id同梱化のみ追加) | ゲーム別満足度スコア |
| `survey_submitted` | `game_id`, `star_score`(自由記述等は既存フローのまま) | `survey.html:305-328` | **既存GAS**(同上) | 年齢帯・価格受容感・つまずき(既存運用の拡張) |
...
- 生成: アプリ起動(タブロード)ごとに`sessionStorage`へ新規ランダムID発行。タブクローズ、または30分無操作でセッション終了とみなす。
...
| **Phase 1** | Web収集開始。`/api/e`(`src/api/events.js`新設)+WAE binding、`common/telemetry.js`+`common/device-id.js`新設、`pono_client_id`(`crypto.randomUUID`、プロフィールと物理分離)、P0イベント8個実装、オプトアウトトグル+告知バナー、プライバシーポリシー記載(§5-4/§7-3のCOPPAゲート)、staging-app先行導入+Playwright/admin除外。CORSは`src/api/savedata.js`の`allowedOrigins()`を共有。 | 2〜3日 |
