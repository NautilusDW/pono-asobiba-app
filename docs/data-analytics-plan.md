...
| B-6 | 「お子さんの 個人識別情報は 一切保管しません」 | `parent_flyer.html:233`(保育園掲示用チラシ)。当初はC表で`consent_form.html:198`の列挙限定型と同一行に混在していたが、実文言が異なるため分離(2026-07-13) | 「個人識別情報」の射程が広く無限定の断定であり、COPPA(16 CFR 312.2 item 7)が永続識別子(`client_id`)を個人情報として扱う整理とぶつかり得る。列挙限定型(お名前・顔写真・声・位置情報)とは別物 | **推奨**: β配布前に「お名前・お顔・お声・位置情報などを保管しません」型の列挙限定へ文言修正する(β文書は未配布のため修正コストゼロ) |
