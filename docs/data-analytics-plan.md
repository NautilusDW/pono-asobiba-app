...
> **本計画の正本。実装時は本書を参照すること。** 現状棚卸し・マーケ活用分析・技術基盤設計・ネイティブアプリ対応の4系統の調査/分析ワークフロー成果物を、オーケストレーター(Fable)が確定した設計方針に統合したもの。個別分析の詳細根拠(file:line)は本文中に残しているが、ワークフロー生ログの正本パスは [§10 出典](#10-出典) を参照。
...
| メールアドレス(あそびの記録そのものには含めない) | あそびの記録(client_idで自動的につく行動の記録)そのものに、メールアドレスを含めることはありません(§5-5)。**ただし1点だけ例外があります**: 感想アンケート(survey.html)で保護者が任意で書かれたメールアドレスは、記録用の符号(client_id)と同じ送り先(スプレッドシート)に届くことがあります。この場合に限り、Google側であそびの記録とメールアドレスが同じ場所に置かれる可能性があります。くわしくは§10-3 FAQ Q2をご覧ください |
...
| 名前と結びつかない「符号」(ランダムな番号) | `pono_client_id`。`crypto.randomUUID()`で自動生成、氏名・年齢・性別とは別の場所に保存(§5-3)。**ただし感想アンケートに任意でメールを書いた場合は例外があります(上表「メールアドレス」行参照)** |
...
**掲載時の注記文言案**: 「ポノのあそびばは、あそびの記録(とうけい)を、名前と結びつかない符号だけで自動的に記録しています。この記録は、あそびばをもっと楽しくするためだけに使い、広告やお子さまへの連絡には使いません。なお、感想アンケートに任意でメールアドレスを書いていただいた場合のみ、その回答と符号が同じ場所に届くことがあります。」
...
**Q2. 名前と結びつく?**
A. あそびの記録(自動で送られるデータ)そのものには、お名前・生まれた日・メールアドレスは含まれません。記録に使う符号(client_id)は、お名前などとは別の場所に保存されています(この設計は合言葉クラウド同期でお名前などを常に取り除く仕組みと同じ考え方です、§5-3)。

ただし1つだけ例外があります。感想アンケート(survey.html)でメールアドレスを任意で書いていただいた場合、そのメールは記録用の符号(client_id)と同じ問い合わせ窓口(Googleの表)に届く仕組みになっています。そのため、アンケートにメールを書かれたご家庭に限り、あそびの記録とメールアドレスがGoogle側の同じ場所で結びつく可能性があります。メールアドレス欄への入力は今までどおり完全に任意で、書かなければこの結びつきは起きません(この点は現在の設計上の課題として認識しており、記録専用の送り先を分ける形で解消できないか検討しています)。
