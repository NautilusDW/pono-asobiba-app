# TIER POLICY — 無料 / 絵本購入者 / アプリ (= sub) の境界 (v3.1)

ユーザー層（tier）の判定ルールと、各層に解放される範囲の正式仕様。
実装の一次ソースは `common/tier.js`。本ドキュメントは人間向けの参照用。
不整合が生じた場合は `common/tier.js` を正とし、本ドキュメントを更新する。

> **v3.1（2026-07-08）で maze / puzzle だけ book > free を復帰。**
> quizland / bento / oto は v3 の **free = book（機能差ゼロ）** を維持する。
...
v3.1 の核となる原則は以下の 4 つ。

1. **maze / puzzle は book で追加抜粋を解放する。** free は入口として年齢幅のある抜粋、book は free に数ステージを足す。sub は全ステージ解放。
2. **quizland / bento / oto は free = book を維持する。** これらは v3 のまま本編コンテンツ差を作らず、book の差は付録中心に留める。
...
旧仕様（book が free よりステージ数や食材数で少し勝る設計）は、合言葉が SNS 等で拡散した場合に本編コンテンツの価値がそのまま流出するリスクを抱えていた。v3 では一度 book の解放範囲を free と同一にしたが、v3.1 ではユーザー判断により maze / puzzle のみ「絵本を買ってくれた人への一晩の解放」として追加抜粋を復帰した。詳細は §6「合言葉流出耐性の構造的防御」を参照。

---

## 3. ゲーム別解放範囲

以下は free / book / sub の解放範囲。**maze / puzzle だけ book が free より広い。**
...
v3.1 の設計は、絵本奥付の合言葉が SNS 等で拡散されるリスクを前提に、以下の三重の構造的防御を持つ。
...
v3.1 実装では `common/tier.js` の各ゲーム判定を **集合（Set/allowlist）方式**に統一する。maze / puzzle は `FREE_*` と `BOOK_*` の allowlist を分け、quizland / bento / oto は v3 の free = book 配列を維持する（`isXxxUnlocked()` は `indexOf` / `Set.has` ベースで統一）。
...
- quizland / bento / oto は v3 の free = book を維持。
