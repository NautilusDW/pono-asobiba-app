...
> 2026-07-08: maze / puzzle は連番ロックではなく allowlist 方式へ変更。
> free / book の具体ステージは本ドキュメント §2.1 B / D と `common/tier.js` を正とする。
...
#### A. quizland（フクロウはかせのなぞなぞ） — 実問題数 **全 180 問**

実態: order_color / count_total / shape_name / weather / opposite / trivia / body / number_sequence の 8 カテゴリ、Lv1-3 合計 180 問。

| 層 | 解放範囲 | 問題数 | 全体比 |
|---|---|---|---|
| **free = book** | Lv1=10 / Lv2=10 / Lv3=6 のおすすめ固定プール。難易度選択なし | **26 問** | 14% |
| **sub** | 全カテゴリ全問 + 難易度選択 | **180 問** | 100% |

free/book は `はじめる` から直接おすすめ5問に入る。1ラウンドは **Lv1 2問 / Lv2 2問 / Lv3 1問** で組み、単純ランダムによる難度偏りを避ける。book は本編問題数では free と差を付けず、付録や記念要素で差別化する。

#### B. maze（ポノとランタンのめいろ） — 全 10 ステージへ拡張予定

| 層 | 解放ステージ | ステージ数 | 全体比 |
|---|---|---|---|
| **free** | Stage 1 / 3 / 6 | 3 / 10 | 30% |
| **book** | Stage 1 / 2 / 3 / 4 / 6 | 5 / 10 | 50% |
| **sub** | Stage 1-10 | 10 / 10 | 100% |

差別化のキモ: free は導入 / 中盤 / ボス手前を抜粋し、book は Stage 2 / 4 を追加して序盤から中盤を厚くする。Stage 7（最終ボス）は book でも除外し、アプリ (= sub) のクライマックスとして温存する。
...
| 層 | 解放ステージ | ピース数 | ポノ特別枠 |
|---|---|---|---|
| **free** | Stage 1 / 4 / 7 / 13 | 4 / 6 / 9 / 16 ピース | なし |
| **book** | Stage 1 / 4 / 5 / 7 / 11 / 13 / 17 | 4 / 6 / 特別枠 / 9 / 12 / 16 / 20 ピース、特別枠 #5 のみ | 1 枚 |
| **sub** | Stage 1-20 | 4-20 ピース、特別枠 #5 / #10 / #15 / #20 全部 | 4 枚 |

差別化のキモ: free は 4 / 6 / 9 / 16 ピースで難易度を散らす。book は Stage 5 のポノ特別枠と Stage 11 / 17 を追加する。ポノ特別枠（5 / 10 / 15 / 20）は 4 枚あるが、book には 1 枚（#5）だけ味見、残り 3 枚（#10 / #15 / #20）はアプリ (= sub) 専用。
...
|---|---|---|
| **quizland** | free/book の26問プール外の **154 問** + 難易度選択 | 全問題・全難易度に触れられるのはアプリ (= sub) だけ |
| **maze** | Stage 5 / 7 / 8 / 9 / 10（5 ステージ） | Stage 7 のクライマックスと高難度のご褒美 |
| **oto** | リズム曲 きらきらぼし / よろこびのうた + 音色 blip / taiko + コードモード一部 | 「子供が知ってる定番曲」を演奏できるのはアプリ (= sub) だけ |
| **puzzle** | book 未解放ステージ（Stage 2 / 3 / 6 / 8 / 9 / 10 / 12 / 14 / 15 / 16 / 18 / 19 / 20）+ ポノ特別枠 #10 / #15 / #20（3 枚） | 完成達成感と「ポノ絵本コレクション」がアプリ (= sub) に集中 |
