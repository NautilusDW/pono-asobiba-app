# TIER POLICY — 無料 / 絵本購入者 / アプリ (= app) の境界 (v3.2)
...
| tier | 累積数 | 内訳 | 追加テーマ |
|---|---:|---|---|
| **free** | **4** | boy 系 2 + girl 系 2 | `boy` / `shinobi` / `girl` / `sakura` |
| **book** | **7** | boy 系 3 + girl 系 3 + 共通 1 | free + `hero` / `idol` / `book_buyer_edition` |
...
- 子ども向け UI は、現在の tier で使えるテーマと次 tier のテーマだけを表示する。free は 4 解放 + book 3 ロック、book は 7 解放 + app 4 ロック、app は 11 解放を表示し、ロックの壁を作らない。
...
| シールちょうテーマ | 累積 11 種（free 4 + book 3 + app 4。book の共通「とくべつ」1 種を含む） |
