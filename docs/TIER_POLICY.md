# TIER POLICY — 無料 / 絵本購入者 / アプリ (= app) の境界 (v3.3)

> **2026-07-11: 課金方針が買い切り (アプリ買い切り) に確定したため、tier 名 `'sub'` を `'app'` にリネームした（挙動不変の純リネーム）。** 本ドキュメント中の `app` は旧 `sub` と同一の tier を指す。localStorage キー (`pono_sub_active` / `pono_sub_promo_*`) は既存ユーザー状態維持のため旧名のまま。
...
> book の本編追加は限定的な抜粋で、app の全解放とは明確に差を残す。
> 変更の経緯は末尾「## 履歴」を参照。
...
| **app** | `'app'` | `APP_BUILD=1`（capture override 含む） | アプリ購入者。`APP_BUILD=1` 環境では自動的に app 扱い。※旧記述の `localStorage.pono_sub_active === '1'` は旧設計・廃止済み（`getTier()` は読まない） |

判定の優先順位: `app` > `book` > `free`（`common/tier.js`）。

公開 API:
- `window.PonoTier.getTier()` → `'free' | 'book' | 'app'`
- `window.PonoTier.isFree() / isBook() / isApp()`（`isSub` は @deprecated エイリアスとして残置）

---

## 2. 原則:「free は体験、book は追加抜粋、app は全解放」

v3.2 の核となる原則は以下の 4 つ。

1. **maze / puzzle は book で追加抜粋を解放する。** free は入口として年齢幅のある抜粋、book は free に数ステージを足す。app は全ステージ解放。
...
4. **app は全 content + 全付録 + 蓄積系。** app のみが全ゲーム・全ステージ・全食材・app 専用ゲーム 7 本・app 専用シール・デイリーガチャ全ページを持つ。app と free/book の差は「量」であり、量の差こそが継続契約の動機になる。
...
以下は free / book / app の解放範囲。**maze / puzzle / oto / bento は book が free より広い。**
...
- Stage 8-10 の新規制作は **優先度を下げる**（§7 参照）。追加時は初期状態で app 専用とする方針は維持。
...
| **app** | Stage 1-20 全部 | 4-20 ピース |

- free は 4 / 6 / 9 / 16 ピースで簡単・普通・難しいを散らす。
- book は Stage 5 のポノ特別枠と、Stage 11 / 17 を追加する。Stage 10 / 15 / 20 のポノ特別枠は app 専用のまま維持。
...
- app のみ難易度選択 UI を表示し、全 180 問・全カテゴリへアクセスできる。
...
| **book** | free + taiko / marimba / blip（計 6 種） | メジャー + ペンタ | じゆう + リズム | ちょうちょうまで / 和音 ON |
| **app** | 全音色 + 将来追加 | 全スケール | 全モード | 全曲 + 将来追加曲 |

- free は基本音色とリズム体験を残し、book で追加音色・ペンタ・和音に加えてネジマエストロ初登場の「ちょうちょう」までを解放する。app は「かえるのうた / メリーさん / きらきらぼし / ちょうちょう / ロンドンばし / よろこびのうた / せいじゃの こうしん」まで遊べる。
...
book は maze / puzzle の追加抜粋とは別に、以下の付録を持つ。free には付与しない。app は全て含む。
...
- `Prototypes/StickerBookThreeJS/main.js` の `BOOK_VARIANTS` に専用エントリを持つ。
- `coverFront`: 絵本表紙の再構成 + 右下に「絵本を読んでくれたあなたへ」。
- `coverInside`: 名前差し込み欄（`localStorage.pono_player_profile_v1` の `name` を Canvas で動的描画）。
- 旧 `book_bonus` 表紙と画像・役割が重複していたため、表示上は `book_buyer_edition`（「とくべつ」）1 種へ統合する。旧 URL / 旧保存値の `book_bonus` は互換 alias として `book_buyer_edition` へ移行する。
- この特別表紙は男女どちらにも使える共通枠として、下記の book / app 累積数に含める。プロフィールの性別で選択肢を制限しない。

### 4.2.1 シールちょうテーマの tier 配分（2026-07-13 確定）

| tier | 累積数 | 内訳 | 追加テーマ |
|---|---:|---|---|
| **free** | **2** | boy 系 1 + girl 系 1 | `boy` / `sakura` |
| **book** | **7** | boy 系 3 + girl 系 3 + 共通 1 | free + `girl` / `shinobi` / `hero` / `idol` / `book_buyer_edition` |
| **app** | **11** | boy 系 5 + girl 系 5 + 共通 1 | book + `robot` / `game_center` / `space_live` / `rainbow_dream` |

- boy / girl はカタログ在庫のバランス確認用分類であり、利用者プロフィールに対する性別ゲートではない。解放済みテーマは誰でも選べる。
- 残り 14 種（`kaiju` / `mascot` / `parade` / `secret_base` / `anime_studio` / `neon_city` / `candy_pop` / `manga_action` / `cyber_grid` / `midnight_magic` / `pop_art` / `festival_night` / `sweet_gothic` / `sports_power`）は、将来のどんぐり交換・イベント・追加特典用の在庫として保持する。正式な取得経路ができるまでは通常 UI に表示しない。
- 子ども向け UI は、現在の tier で使えるテーマと次 tier のテーマだけを表示する。free は 2 解放 + book 5 ロック、book は 7 解放 + app 4 ロック、app は 11 解放を表示する。
- 選んだテーマは `localStorage.pono_sticker_book_theme_v1` に保存する。URL の `book` 指定がある場合はその表示を優先し、指定がない再訪時は保存値を使う。
...
- book / app 共通。頻度は `localStorage` で制御。
...
### 4.6 プロフィールアバター 40 種（2026-07-14 追加）

`play.html` のプロフィール編集（アバター作成・編集）で選べる 40 種のアバタープリセットは、他の書付録と異なり **book 限定の一括贈呈ではなく free/book/app の 3 段階 allowlist** で解放する（maze/puzzle/oto/bento と同じ集合方式）。

| 層 | 解放数 | 内訳 |
|---|---:|---|
| **free** | **4** | `pono_kko` / `sky_kko` / `berry_kko` / `niji_kko`（男女 2:2 のバランス） |
| **book** | **10** | free 4 種 + `fox` / `rabbit` / `bear` / `cat` / `robot_blue` / `star_mon` |
| **app** | **40**（全解放） | 上記 10 種を含む全プリセット |

- 判定関数: `PonoTier.isAvatarPresetUnlocked(id)`（`common/tier.js`）。`gameLocksEnabled()` によるゲートは付けない — `play.html` は常駐ページで `PONO_TIER_GAME_LOCKS_ENABLED` を立てていないため、ゲート付きにすると常にフェイルオープンになってしまう。`isRoomItemUnlocked` / `isKatakanaUnlocked` と同じ「play.html 常駐機能はゲート無し」パターンに揃える。
- book は `FREE_AVATAR_PRESET_IDS.concat([...])` で定義し、free ⊂ book のスーパーセット関係をコード上でも保証する。
- ロック UI は `.tier-locked`（グレーアウト + 中央 🔒、×印なし）を使う。oto/quizland の同名クラスと定義を完全一致させる（§13 の意匠統一ルール）。タップ自体は受け付け、`showBookLimitPromo('avatar')` / `showTierLockPromo()` の柔らかいモーダルへ誘導する（`btn.disabled` にはしない）。
- 誘導コピー: 「えほんの あいことばで、えらべる すがたが ふえるよ」（`getBookLimitPromoCopy` の `id === 'avatar'` 分岐）。
- 既存に保存済みの `profile.avatar.preset` はロック対象に含まれていても読み込み時にリセットしない（グランドファザー。回帰テスト: `tests/profile_direct_edit_regression.cjs`）。

---

## 5. app 維持（新規 content 追加なし）

app は v3 移行後も以下を維持し、**新規本編コンテンツの追加は行わない**（free/book の付録拡張とは別軸）。

| 項目 | 内容 |
|---|---|
| 全 content | maze 1-10 / puzzle 1-20 / quizland 180 問 / bento 30 食材 / oto 全音色・全曲 |
| app 専用ゲーム | starparodier / undersea-cave / sea-album / writing-mori / zukan / cooking / wordmatch（計 7 本） |
| app 専用シール | 10-12 枚（既存の余剰選別 7-9 枚 + Brand Kit 新規 2-3 枚） |
| シールちょうテーマ | 累積 11 種（free 2 + book 5 + app 4。book の共通「とくべつ」1 種を含む） |
...
2. **app の物量差が圧倒的。** app は全 content（maze 全 10 / puzzle 全 20 / quizland 全 180 問 / bento 全 30 食材 / oto 全曲）+ app 専用ゲーム 7 本 + app 専用シール 10-12 枚 + デイリーガチャ全 10 ページを持つ。book の抜粋と付録がどれだけ拡散されても、app との体験量の差は埋まらないため、「合言葉さえあれば app は要らない」という状況は構造的に発生しない。
3. **APP_BUILD ゲート。** アプリ (= app) 版ビルド（`APP_BUILD=1`）は合言葉入力を経由せず自動的に app 判定される。合言葉は book 判定専用の入口であり、app 判定とは独立した経路のため、合言葉の流出が app 側のセキュリティに影響しない。
...
| **アプリで あそべる** | app 専用 7 ゲーム（starparodier / undersea-cave / sea-album / writing-mori / zukan / cooking / wordmatch） |
...
- 現時点ではストアフロント未実装。※旧記述「`localStorage.pono_sub_active = '1'` を直接セットすれば app 相当になる」は旧設計・廃止済み（`getTier()` はこのキーを読まない。開発時は staging の debug-mode + capture override か `APP_BUILD=1` 環境を使う）。
- `APP_BUILD=1` 環境（Web アプリ版ビルド）では自動的に app 扱いになる。
- 将来: 決済フロー導入後、ここを正規のアプリ (= app) 管理に差し替える。
- TODO: アプリ (= app) 解錠 UI の正式実装は将来課題（play.html 内に解錠モーダル新設）。
...
PonoTier.isAvatarPresetUnlocked(id)   // プロフィールアバタープリセットが解放されているか（§4.6）
```
...
- `assets/data/game-stickers.json` — `tier: book_exclusive / app_exclusive` の schema 拡張、8 ページ目「絵本特典」新設
...
### v3.3（2026-07-14、プロフィールアバター tier gating 追加）

- プロフィール編集のアバタープリセット 40 種に free 4 / book 10 / app 40 の 3 段階 allowlist ロックを新設（§4.6）。他ゲームと同じ `FREE_*`/`BOOK_*` allowlist + `isXxxUnlocked()` 集合方式。
- `common/tier.js`: `FREE_AVATAR_PRESET_IDS` / `BOOK_AVATAR_PRESET_IDS` / `isAvatarPresetUnlocked` を追加、`hasBookUpgradeContent` / `getBookLimitPromoCopy` に `avatar` 分岐を追加。
- `play.html`: `.tier-locked` CSS（oto/quizland と同一定義）、プリセットボタンのロック判定、クリックガードを追加。

### v3.2（2026-07-08、oto / bento book 追加復帰）

- oto を free 3 音色 + かえるのうた / book 6 音色 + ペンタ + 和音 ON + ちょうちょうまでに変更。
