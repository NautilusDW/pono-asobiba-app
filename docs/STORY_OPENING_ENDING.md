...
### Beat 2 — ザガンの手下 (ノクス / 仮面の魔人) 妨害

**視覚:** `_showMaskedManInterference(onDone)` がフルスクリーン紫闇オーバーレイを生成、`kagerou_recurse.jpg`（ノクスがカゲロウに再呪印を刻む場面）を中央表示。タイトルテキストは撤去し、画像で語る方針 (v275)。
