...
## イベント: ④  わ行バトル中盤〜クライマックス (v277 系で大幅再構成、v277k 時点)

> **注意:** 本セクションは v277k 時点のコード実態に追従して全面リライト済み。歴代の v277b〜v277j の change-log は本文の上 (イベント①欄) に積層保存。

### 全体フロー (なぞりターン = 1、紙芝居は 0 ターン換算)

| # | アクション | ターン | 実装関数 |
|---|----------|-------|---------|
| 1 | わ なぞり (剣) | 1 | 通常 quiz、カゲロウ反撃→ヒノカ累積被弾 |
| 2 | を なぞり (剣) | 2 | 同上 |
| — | (step==2 で `_rowPracticeNextChar` が中断) | — | mini-cut → episode の 2 段挿入 |
| — | **`_playWaMidHitCut`** (v277h): ヒノカ軽傷 mini-cut | 0 | カゲロウ「グ……まだ おわらん……！」→ ヒノカ被弾 (`_hinokaMidHitEffect`) → ヒノカ「だいじょうぶ……！まだ、 たたかえる……！」 |
| — | **`_playWaRowEpisode`** | 0 | ヒノカ訴え → 回想 → `_playKagerouClimax` |
| — | ┗ メモリフラッシュバック (4 枚絵) | 0 | `_showMemoryFlashback` |
| — | ┗ `battle_last_01.jpg` 1 枚絵 | 0 | `_showClimaxEventImage` |
| — | ┗ ヒノカ訴え「カゲロウ を とりもどせる はず…」「『ほむらの うた』を、おもいだして！」 | 0 | bustup + dialog |
| — | ┗ **Beat 0** (v277d): セリナ氷無効 → ヒノカ正義の炎が通る | 0 | `_playKagerouClimax` 内、Beat 1 前 |
| — | ┗ **Beat 1**: カゲロウ「…ほむら…？」(覚醒の兆し) | 0 | `_playKagerouClimaxMain`、`_pulseKagerouAwaken` |
| — | ┗ **Ritual 1** (v277): ヒノカ promo → `_showChantRitual ['ほ','む','ら']` → カゲロウ「…ほ…むら…？」 | 0 | awakening theme |
| — | ┗ **Beat 2**: ノクス再呪印 (`_showMaskedManInterference`、`nokus_reappear.jpg`) | 0 | 「て こずって いる ようだな」「ザガンさま の のろい は…」 |
| — | ┗ **Beat 3**: カゲロウ激昂 + ヒノカ被弾 (`_hinokaFallEffect`) | 0 | rage attack |
| — | ┗ **Beat 4**: ヒノカ最後の託し「星の印 を…」 | 0 | bustup + 弱々しいセリフ |
| — | ┗ **Beat 5**: `_showStarMarkAwakening` (預言+星の印) | 0 | climax episode はここで終了 |
| 3 | **ん なぞり** (剣、通常 quiz) | 3 | ひらがな完成、HP→0 finisher トリガ |
| — | **`_playKagerouFinalCombo`** (v277i+k): ん 完了 → finisher 連続技 | — | 火山ボス専用、battleVictory の前 |
| — | ┗ Step 1: 「ひらがな かんせい！れんぞくわざ かいほう！」バナー (1.6s + Web Audio チャイム) | 0 | |
| — | ┗ Step 1.5 (v277k): **妖精合流** `_showFairyMergeIntro` (リーファ+セリナ「ゆうしゃさま！やっと おいついた わ！」「ヒノカ も ぼろぼろ ね…」「カゲロウ は とても つよい——でも、 みんな で たちむかえば！」) | 0 | |
| — | ┗ Step 2: プロンプト「よげん の ちから が…」「カゲロウ の ほんとう の なまえ を、よびかけよう！」 | 0 | |
| 4 | ┗ Step 3: **連続技 `_showChantRitual ['か','げ','ろ','う']`** (starmark theme + shoutout「カゲロウ！」) | 4 | finisher 入力 |
| — | ┗ Step 4: `_crackKagerouMask` (仮面割れ + 白フラッシュ + Web Audio ノイズバースト) | 0 | |
| — | ┗ Step 5: `battle_last_02.jpg` (浄化遷移シーン) | 0 | |
| — | → `battleVictory('fire', _chainReward)` → bow + leave + ascending → celebration | — | |
