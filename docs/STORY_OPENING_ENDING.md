...
> おそわれた むら には、  
> こわれた いえ と、 なきじゃくる ひとびと だけ が のこりました。  
> まおう ザガン は、 みんな の 「ありがとう」 の こころ を うばって いた の です。
...
**Voice (TTS):** `voice/S5c.mp3` (S5c_a と S5c_b で共用・連続再生) / `voice/S5d.mp3` 統合済み。**S5c_c.mp3 は未生成** (画像と合わせて後日)。画像とセットで AI Studio で Iapetus 収録予定。
