...
**Voice (TTS):** 未収録。`voice/S6d.mp3` を Iapetus で生成予定。
...
**Voice (TTS):** 未収録。`voice/S7a_mid.mp3` を Iapetus で生成予定。ナレーターが「王族だけの秘密の預言」と紹介してから、預言本文を朗読する自然な読み上げ。巻物などのビジュアル演出は作らず、絵はユーザー側で別途検討。
...
**Voice (TTS):** 既存 `voice/S7a.mp3` は旧テキスト (「そして、あなたが えらばれたのです」) のため、v269 新テキストで再収録が必要。
