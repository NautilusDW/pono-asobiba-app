...
**視覚 (v277b で全面刷新):**
- 山道 (`volcano_path_intro.jpg`): `_showClimaxEventImage(src, fn, { cover:true, pan:'up' })` — 画面いっぱいに敷き、translate Y 8% → -8% を 10s linear で縦パン (見上げていくイメージ)
- カゲロウ登場 (`kagerou_entrance.jpg`): `_showClimaxEventImage(src, fn, { cover:true, kenBurns:true })` — 画面いっぱい + 8s ゆっくりズームイン (1.0 → 1.08)
- 認識セリフ中: 既存のバトル画面 (仮面姿のカゲロウが画面右に立っている状態) + ヒノカ右バストアップ
