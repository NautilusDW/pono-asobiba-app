...
## イベント: 炎の妖精との別れ（ら行 最終文字クリア直後、v267 で簡素化）

**発動タイミング:** ら行 lava_step パズルの finisher 完了直後、既存の `_fireFairyFarewell(advance)` 呼び出しで発火。

**v267 変更:** 魔王城の鍵受け渡しは氷の妖精に移管されたため、炎の妖精は純粋な別れ挨拶だけ (3 メッセージ)。
...
2. ほのおの ようせい「これで わたしも
   ようがん に かえれる わ。」

3. ほのおの ようせい「みんなの ぶん まで、
   おひめさま を たすけだして ね！」
```

最後のセリフ完了後、妖精がフェードアウト → `advance()` → エンディング紙芝居。
