...
| 5 | クライマックス interlude (覚醒→**儀式①「かざん」**→ノクス妨害→倒れる→星の印→**儀式②「かげろう」**→仮面割れ) | `_playKagerouClimax` + `_showChantRitual` × 2 + `_crackKagerouMask` | v271 / **v277** |
...
**発動タイミング:** わ行選択 → マップシーン → エンカウント紙芝居 (「かざんのぬし あらわる！」ドラクエ風モーダル) 直後、「もじを なぞって たおそう！」の操作説明が出る**前**。
**実装関数:** `_playHinokaRecognitionIntro(onDone)` — バトルログ `_showBattleDialog` を 5 → 急襲シェイク → 2 の 2 段で流す。
**BGM:** バトル BGM 継続。ヒノカの dialog 中は静かなまま、急襲のシェイクでテンションを付ける。
**視覚:** 既存のバトル画面 (仮面姿のカゲロウが画面右に立っている状態)。急襲時に `_triggerKagerouRageAttack()` で敵スプライトに紫フィルタ + 激しい左右振動 + `.battle-stage` 強シェイク。

**ナラティブ前提:** ヒノカ は ら行の溶岩で救出されるまでの**数日間 ザガン の魔力の鎖で幽閉されていた**。その間に カゲロウ が呪縛された事実は知らない。火山頂上で初対面の際に、**気配**で「もしかして…?」と気づく。

### Phase A: ヒノカ認識セリフ (5 ビート、タップ送り)
...
## イベント: ③  ヒノカ『かざんのうた』呼びかけ + 回想フラッシュバック（v270、v272 で簡素化）
...
**実装関数:** `_showChantRitual({ chars: ['か','ざ','ん'], theme: 'awakening', ... }, onDone)`
...
- **v277 追加**: 儀式①「かざん」(歌の一節 → 思い出を呼び覚ます) と 儀式②「かげろう」(真名 → 呪縛を解く) で、プレイヤー自身が言霊を発する**操作感**を物語のピークに重ねる。紙芝居だけでは得られない関与感を与える
