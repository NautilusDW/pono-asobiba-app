...
> ✅ **完了 (2026-07-05)**: 実装 A (図鑑モード凍結 flag) + B (ミュージアム説明文バッタ見本) は実装・クロスレビュー承認・App staging デプロイ済み (sw v1966、commit 41dbcbb)。§4 の user 判断は確定済み — トーン =「出自 (どこで出てきたか) + 軽い架空設定、入手方法は書かない。例外: 特典系 (book_bonus/アンケート等) のみ短く入手経緯付記」(案 A-D のいずれでもない新基準)。バッタ採用文と実装詳細は memory `feature_zukan_freeze_museum_desc` 参照。本文書は履歴として保存。
