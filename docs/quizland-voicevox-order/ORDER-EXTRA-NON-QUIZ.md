...
> ## ⚠️ MVP スコープ (2026-05-12 ユーザー確定)
>
> 本書 53 件のうち、 **MVP 対象は A くるみ 5 件のみ**。 B 博士 48 件は **MVP 後回し** (将来フェーズで着手)。
>
> - **MVP 内**: A. くるみ担当 (5 件) — 第1〜5問目コール
> - **MVP 後**: B. 博士担当 (48 件) — 関連準備 (漢字混じり化 / 辞書 9 語追加 / 「ナレーター おじいさん」 購入済) は完了、 MVP リリース後に着手
>
> 発注書本文は 53 件すべて記載のまま温存 (将来そのまま使えるよう)。
...
| 話者 | エンジン | 設定 | 担当 |
|---|---|---|---|
| **くるみちゃん** | VOICEPEAK | **「女性4」 プリセット** (2026-05-12 確定、 旧 「女の子」 試聴の結果却下) | A. 問題番号コール 5 件 |
...
- VOICEPEAK 「女性4」 プリセット (2026-05-12 確定、 旧 「女の子」 試聴の結果却下)
- 問題本編 (くるみ女声 907 件、 `ORDER-FULL.md`) と同じ音量・話速で揃える
- 子供向けに早口にならないよう注意

> **【廃案 2026-05-12】 中継案 「女の子」 プリセット (履歴温存)**: 同日内一段階目で 「女の子」 で確定したが 27 ファイル試聴の結果で却下、 「女性4」 へ再差替。 詳細は [memory/feature_quizland_voicepeak_pivot_jyosei4.md](../../memory/feature_quizland_voicepeak_pivot_jyosei4.md) 参照。
...
|---|---|---|---|
| `kurumi_dai1mon.wav` | 第一問 | だいいちもん | 「第一問」 はユーザー辞書登録済 (`voicepeak_user_dict.csv` line 67) |
| `kurumi_dai2mon.wav` | 第二問 | だいにもん | 第二問 = `ダイニモン` 4 平板 (辞書登録済) |
| `kurumi_dai3mon.wav` | 第三問 | だいさんもん | 第三問 = `ダイサンモン` 4 平板 (辞書登録済) |
| `kurumi_dai4mon.wav` | 第四問 | だいよんもん | 第四問 = `ダイヨンモン` 4 平板 (辞書登録済、 「ダイシ」 ではなく「ダイヨン」) |
| `kurumi_dai5mon.wav` | 第五問 | だいごもん | 第五問 = `ダイゴモン` 4 平板 (辞書登録済) |

> **読み方注 (2026-05-12 更新)**: speech は短縮 + 漢数字統一 (旧「第1問目です」 / 「第一問目です」 を「第一問」 〜 「第五問」 に変更)。 既に「第一問」 が辞書登録済 (`ダイイチモン` 4 平板)、 同形の「第二問〜第五問」 4 語も辞書追加済 (`voicepeak_user_dict.csv` 75 → 79 語、 同じ 4 平板パターン)。 ファイル名 `kurumi_dai{N}mon.wav` (アラビア数字) は発注書ファイル名規則として確定済のため維持 → speech (漢数字) と ファイル名 (アラビア数字) の食い違いは意図的。
...
`tools/voicepeak/voicepeak_user_dict.csv` (79 語) と照合し、 本発注書に登場するセリフから抽出した未登録の主要語を以下に列挙する。 アクセント核位置は推定値。 試聴後に必要に応じて辞書追加 → VDC2 再生成 (`tools/voicepeak/Convert-VoicepeakUserDictCsvToVdc2.ps1`) を実施する。

### C-1. 番号コール関連 (高優先) — 全件登録済 (2026-05-12)

| 単語 | 読み | アクセント核位置 | 備考 |
|---|---|---|---|
| 第二問 | ダイニモン | 4 平板 | 既存「第一問」 と同じパターン (辞書 line 77) |
| 第三問 | ダイサンモン | 4 | (辞書 line 78) |
| 第四問 | ダイヨンモン | 4 | 「第四 (ダイシ)」 ではなく「ダイヨン」 と読ませる (辞書 line 79) |
| 第五問 | ダイゴモン | 4 | (辞書 line 80) |
...
| 発注書 | 件数 | 話者 | スコープ |
|---|---|---|---|
| `COWORK-TEST-ORDER.md` | 27 | くるみ (VOICEPEAK 「女性4」) | 問題本編テスト 5 問分 |
| `ORDER-FULL.md` | 907 | くるみ (VOICEPEAK 「女性4」) | 問題本編フル 181 問分 |
| **`ORDER-EXTRA-NON-QUIZ.md` (本書)** | **53** | **くるみ (VOICEPEAK) + 博士 (VOICEPEAK)** | **問題以外: リアクション/リザルト/問題コール** |

- **くるみ全件 VOICEPEAK 「女性4」 統一 (2026-05-12 確定、 旧 「女の子」 試聴の結果却下)、 二重話者化リスクなし**: くるみ全 912 件 (問題本編 907 + 第1〜5問目 5) を VOICEPEAK 「女性4」 プリセットで統一する方針が確定。 VOICEVOX 雨晴はう + 中継案 「女の子」 はいずれも廃案。 これにより問題本編と問題コールでエンジン違いによる音色差が発生する懸念は解消済。 詳細な話者再差替の経緯は [memory/feature_quizland_voicepeak_pivot_jyosei4.md](../../memory/feature_quizland_voicepeak_pivot_jyosei4.md) 参照
