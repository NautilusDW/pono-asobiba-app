...
- MVP リリース後 (`bento`, `maze`, `oto`, `puzzle`, `quizland` 全公開 + `starparodier`, `undersea-cave`, `sea-album` の app tier 隠し公開) を機に、 「ゲーム本編から canvas/DOM を直接キャプチャ」 する内蔵機構へ移行したい。
...
| Admin パネル | `admin/index.html` の「スクショモード」 ボタン → `localStorage.ponoCaptureArmed = '1'` をセットして対象ゲームを別タブで開く | Phase 3。 全ゲーム一括撮影、 app tier 隠しゲーム含む |
...
| 3 | **starparodier** | app | `#gameCanvas` | wave 1 開始後 | 弾幕が画面に出ている戦闘中、 ボス登場カットイン | **易** |
| 4 | **undersea-cave** | app | `#gameCanvas` | 探検開始後 | 洞窟内で生き物に遭遇した瞬間、 図鑑カード獲得演出 | **易** |
| 5 | **sea-album** | app | `#gameCanvas` | アルバム表示後 | 海中シーン (魚が多く泳いでる瞬間)、 アルバムページめくり | **易** |
...
- app tier 隠しゲーム (`starparodier`, `undersea-cave`, `sea-album`) も admin gate 経由でのみ撮影できる。
...
### 7.2 app tier 隠しゲームの撮影権限
- `starparodier`, `undersea-cave`, `sea-album` は現状 `PonoTier` で app 判定がないとゲーム本体が起動しない (tier 判定は `common/tier.js` の `isAppBuild()` = `window.__APP_BUILD__` 経由)。
...
- **APP_BUILD との整合**: capture モード時は `window.__APP_BUILD__` を勝手に書き換えない (Worker が prepend する正規ルートを汚染しない)。 代わりに `PonoTier` に `setCaptureOverride(true)` のような明示 API を追加し、 capture flag が立っているときだけ `PonoTier.getTier()` が `'app'` を返すよう拡張する (= [tier_system_policy memory](feature_puzzle_mori_no_nakayoshi.md) の free/book/app 3 パターン維持と非破壊で両立)。
