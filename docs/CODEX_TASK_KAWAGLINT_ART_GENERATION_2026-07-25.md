...
>
> **アルファチャンネル抜きと切り抜きは行わないでください。** 生成 API / web UI のレスポンスを
...
## 2. なぜ web UI (higgsfield.ai) で生成する必要があるのか

**MCP / API から生成しないでください。web UI をブラウザ操作して生成してください。**

理由 (2026-07-25 に判明した実データ):

- ユーザーは Higgsfield の **ultimate プラン**契約者で、GPT Image 2 / Nano Banana Pro 等の
  **アンリミテッド (無制限) 生成枠**を持っています。
- しかし `show_plans_and_credits` の実データを確認したところ、アンリミテッド枠は全モデルとも
  **"Available ... on web"** と明記されており、**higgsfield.ai のサイト上での生成のみ**が対象。
- **MCP 経由 (Claude Code / API から) の生成にはアンリミテッドが適用されず、通常クレジットを
  消費します** (GPT Image 2 high/2k = 7クレジット/枚、残高 892)。
  本タスクは 70 枚 = 490 クレジット消費となり、残高の 55% を1タスクで焼くことになります。
- MCP 側からアンリミテッドを有効化するパラメータ・フラグは**存在しません**
  (ツール定義・モデル定義・プラン情報すべて確認済み)。

したがって:

1. **第一選択**: あなた (Codex) がブラウザ操作できるなら、higgsfield.ai に
   ログイン → GPT Image 2 を選択 → 本書のプロンプトを1枚ずつ貼って生成 → ダウンロード。
   アンリミテッド枠なのでクレジットは消費されません。**枚数を節約する必要は一切ありません。
   気に入らない結果は何度でもリテイクしてください。**
2. **ブラウザ操作ができない場合**: 本書はそのまま**人間が手作業でコピペして生成できる形式**に
   なっています。§7 の各エントリは「プロンプト」「AR」「クロマキー色」「保存ファイル名」が
   1件ずつ揃っているので、ユーザーに本ファイルを渡して手動生成してもらってください。
   その場合、あなたは「どの順で生成すべきか」「受け取った画像の検品」を担当してください。

> MCP (`mcp__claude_ai_higgsfield_game__generate_image`) を使ってよいのは、**ユーザーが明示的に
> 「MCP で作っていい」と言った場合のみ**です。勝手に MCP で 70 枚生成しないでください。
...
| 項目 | 値 |
| --- | --- |
| モデル | **GPT Image 2** (`gpt_image_2`) — AGENTS.md Hard Rule 7。他モデル絶対禁止 |
| quality | **high** |
| resolution | **2k** (web UI で選べる最高解像度)。最終的に長辺 1024px へ自動縮小されるので、生成は大きく |
...
**完成例 (A-09 `fish_kisu_catch.png` の場合、これが実際に web UI に貼る全文):**
...
- **AR / 解像度**: web UI で選ぶ縦横比。px は最終目安 (スクリプトが長辺 1024px に縮小します)
...
- **形式は PNG**。web UI から落ちてきたものをそのまま置いてください。
...
6. **生成に使った経路** (higgsfield.ai web UI / MCP / 人間手作業のどれか)。
   MCP を使った場合は**消費クレジット数を必ず報告**してください
