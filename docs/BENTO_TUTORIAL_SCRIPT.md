...
- **Phase A (見本のみ)**: 指が置いたのり (例: くち) をぽんとタップ → 編集パネルが開くアニメ → 指が **↻ を 1 回タップ** → のりが 15° 回転する。約 2.5s。パネルは自動で閉じる (or Phase B に持ち越し)。
- **Phase B (任意)**: 指ポインタは「のりOK」ボタンに移動 (ターゲットリング)。**まねっこ強制なし** — パネルを試すもよし、そのまま「のりOK」を押すもよし (light touch)。
...
- **画面状態/前提**: freeGuideStep = tier2Main。おかずパレット (メインのみ)。チュートリアルオーバーライド main = 1。**オーナー方針: おかずは自分のお弁当の主役なので自由**。
...
- **ナレーション音声**: 「つぎは メインの おかず！すきなのを ひとつ、タップして いれてみよう。」 (Phase A→B を 1 本で覆う。「すきなのを」を明示して自由選択を強調 — 変更なし)
- **voice**: `tut2_07_okazu`
