...
- **教える範囲**: 箱は固定 (これでOKのみ) / ごはんタップ / のり配置 + 移動 + 新編集パネル (回転・拡縮、軽く 1 回だけ) / のりOK / メインおかず 1 個 / カップへ / カップ → 自動ピッカー → おかず 1 個 / おかずOK / 完成 + おきにいり。**レイヤー上下 (旧 layer-edit 系) は教えない** (自由発見に任せる)。はっぱ / しきり / ピック タブは tier2Side 中に音声 1 行で「あるよ」と触れるだけ (操作は強制しない)。
...
| 2 | `tut2-box` | 箱かくにん (固定) + 「これでOK」 | button `#free-box-confirm` | tut2_02_box |
...
- **ハイライト対象**: 固定箱 (`box_rect_split` ボタン) にトリム + 指ポインタ → 数秒後 `#free-box-confirm` (「これでOK」ボタン) トリムへ移動。
- **ポノ吹き出し**: 「きょうは これを つかうよ！」 (チュートリアル専用。ゲーム既存の「まずは おべんとうばこを えらぼう！」は固定文脈と矛盾するため suppress — §5 実装メモ参照)
- **ナレーション音声**: 「きょうは この おべんとうばこを つかうよ。「これで オッケー」を おしてね。」
- **voice**: `tut2_02_box`
...
| 2 | `tut2_02_box` | きょうは この おべんとうばこを つかうよ。「これで オッケー」を おしてね。 | 約 5 秒 |
...
01=「おべんとうやさん」, 02=「おべんとうばこ」「オッケー」, 03=「ごはんを よそう」, 04=「おかお」, 05=「うごかして」, 06=「まわしたり」, 07=「メイン」, 08=「カップへ」, 09=「カップ」, 10=「えらんで」, 11=「はっぱ」「ピック」, 12=「できたね」, 13=「ほしマーク」, 14=「アライグマ」「おねがい」, 15=「アライグマ」「よろこんで」。
