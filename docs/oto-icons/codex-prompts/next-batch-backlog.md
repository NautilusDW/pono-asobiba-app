...
> ✅ **このバッチは完了済 (2026-05-14)** — 下記 3 枚はすべて Codex 発注 → 配置完了。本ファイルは履歴として残す (発注プロンプトの参考用)。

前回 Phase1 で不足/弱い 3 枚を Codex に追加発注する。スタイルは既にデプロイ済の 21 枚と完全に揃える: ステッチ風円形 (クリーム背景, 縁に同色破線, 中央に可愛らしいイラスト, 既存21枚と質感統一)

> **最優先**: `btn_ui_loop.png` は **oto/index.html L783 (rec-progress-label) で🔁 絵文字が現在表示中**で、PNG 未配置のため最優先発注対象。

## 不足 3 枚 (全て **完了 2026-05-14**)

| 役割 | 出力ファイル名 | モチーフ案 | 色相 | 備考 |
|---|---|---|---|---|
| ソ (水色音符差替) | btn_note_so.png | 水色のかわいいモチーフ (青空/水玉/水色の鳥/水色の宝石 等、水色がはっきり出るもの) | #3FD0E0 系 | **完了 2026-05-14**: 水色の小鳥で配置済 |
| 🔁 ループ (UI制御) | btn_ui_loop.png | 循環矢印 (リピートマーク) | 鮮やかな単色推奨 | **完了 2026-05-14**: 緑のループ矢印で配置済 (DOM L783 の🔁 絵文字差替) |
| 💾 ほぞん (UI制御) | btn_ui_save.png | フロッピーディスク or 宝箱 or 保存箱 | 茶系 or 紺系 | **完了 2026-05-14**: 宝箱モチーフで配置済 (将来実装予定の予約 UI) |
