...
- `basic_tut_11.mp3`（index 10）は、ヒント実践で発光が見えるタイミング（手本中の発光表示時）に再生する。
- 13ステップ実 UI 導線では、子供が自分でヒントを押して発光させ（`basic-hint-drag-try`）、光った場所へピースを運んではめる。スナップ成功（`onBasicHintDragPracticePieceDropped`）で `showBasicHintDoneNarration()` に入り、`basic_tut_12.mp3`（index 11, 「できたね…」）→ `basic_tut_13.mp3`（index 12, 締めの一言）を順に再生する。締めの音声と自動スナップの両方が完了してから（`maybeFinishBasicPracticeAfterSnap`）基本チュートリアルを終了する。
