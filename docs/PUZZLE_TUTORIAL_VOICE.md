...
2026-06-19 時点の基本チュートリアルは 12 ステップ（`basic_tut_01.mp3` 〜 `basic_tut_12.mp3`）。`D:\ポノのおへや\Puzzle\Tutorial\` 配下の WAV から変換した音声を実装している。
...
| `basic_tut_09.mp3` | 8 | ヒント手本の導入 | 次はヒントだよ。ヒントを押すと場所が光るよ |
| `basic_tut_10.mp3` | 9 | 発光表示時（ヒント実践） | 光った場所へピースを持っていくよ |
| `basic_tut_11.mp3` | 10 | 最後の締め | できたね。わからない時は見るとヒントを使ってね |
| `basic_tut_12.mp3` | 11 | チュートリアル終了時（ステージ1へ進む直前の締め） | これで練習はおしまい。さあ、パズルで遊ぼう。 |
...
- `basic_tut_09.mp3`（index 8）は、ヒント手本の最初（ヒント導入）に再生する。
- `basic_tut_10.mp3`（index 9）は、ヒント実践で発光が見えるタイミングに再生する。
- `basic_tut_11.mp3`（index 10, 最後の締め）は、ヒントボタンのクリック処理で正解位置の発光が実際に描画された後、吹き出し表示と同時に再生する。これでチュートリアルが終了する。
- `basic_tut_10.mp3` 開始後、対象ピースを正解位置へ自動で移動してはめる。音声とアニメーションが両方終わったら基本チュートリアルを自動終了する。
- `basic_tut_11.mp3`（index 10, 「できたね…」）の再生が終わった直後に、続けて `basic_tut_12.mp3`（index 11, 締めの一言）を再生する。締めの音声が終わってからステージ1へ遷移するため、子供は「これで練習はおしまい。さあ、パズルで遊ぼう。」を聞いてからゲーム本編に入る。コーチ吹き出しは「これで れんしゅうは おしまい／さあ、パズルで あそぼう」を表示する。
...
### `tut_basic_09_hint_intro` (index 8)

```text
次はヒントだよ。ヒントを押すと場所が光るよ。
```

### `tut_basic_10_hint_glow` (index 9)

```text
光った場所へピースを持っていくよ。
```

### `tut_basic_11_finish` (index 10)

```text
できたね。わからない時は見るとヒントを使ってね。
```

### `tut_basic_12_closing` (index 11)
...
- `basic_tut_10.mp3`（index 9, `tut_basic_10_hint_glow`）は正解位置が光った後に再生し、約 2.2 秒待って対象ピースを正解位置へ自動移動させる。続けて `basic_tut_11.mp3`（index 10, 締め）→ `basic_tut_12.mp3`（index 11, 締めの一言）を再生し、音声と移動アニメーションの両方が終わってから基本チュートリアルを終了する。
