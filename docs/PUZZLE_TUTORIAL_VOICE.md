...
| `basic_tut_11.mp3` | 10 | ヒント GLOW（子供がヒントを押して発光した直後／demo-less single-try） | 光った場所へピースを持っていくよ |
...
- ヒント節は「見る」ボタンと同じく **手本（お手本ハンドデモ）を廃止した demo-less な単一の「やってみよう」フロー**。`runBasicHintPlaceHandDemo` は呼ばれず（関数定義は残置・未使用）、子供が 選ぶ→押す→動かす を直接行う。`try` バッジ「やってみよう！」は最初の SELECT ステップから表示する。
- `basic_tut_09.mp3`（index 8, SELECT 拍A）は、ヒント節（誘導トライ）の最初に再生する。手本を廃止し、子供が直接ピースを選ぶフローになった。コーチ吹き出しは「やってみよう／ばしょを しりたい ピースを えらんでね」。
- `basic_tut_10.mp3`（index 9, PRESS 拍B）は、子供がピースを選んだ直後（`basic-hint-press-try`）に再生し、「ヒント」ボタンを押すよう促す。コーチ吹き出しは「ヒントを おしてみよう／おすと ばしょが ひかるよ」。
- `basic_tut_11.mp3`（index 10）は、子供が自分でヒントボタンを押して発光させた直後（`basic-hint-drag-try`、`onBasicHintPracticeHintButtonUsed`）に再生し、光った場所へピースを運ぶよう促す。
- 13ステップ実 UI 導線では、子供がヒントを直接使用する。idx8/idx9/idx10 はそれぞれ子供の対応ステップで **1回ずつ** 再生される（手本での重複再生はなし）。子供がピースを選ぶ（idx8）→ヒントボタンを押す（idx9）→発光が見えた時点で idx10 → 光った場所へピースを運んではめる。スナップ成功（`onBasicHintDragPracticePieceDropped`）で `showBasicHintDoneNarration()` に入り、`basic_tut_12.mp3`（index 11, 「できたね…」）→ `basic_tut_13.mp3`（index 12, 締めの一言）を順に再生する。締めの音声と自動スナップの両方が完了してから（`maybeFinishBasicPracticeAfterSnap`）基本チュートリアルを終了する。
