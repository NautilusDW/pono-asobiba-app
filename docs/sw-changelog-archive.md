# sw.js changelog archive

`sw.js` 先頭の changelog コメントのアーカイブ (2026-07-10 移設)。
`sw.js` が ~318KB (約 93% が changelog) に肥大し、 SW は no-store 配信 + 5分毎 update poll で
毎回再ダウンロードされるため、 最新 ~10 エントリのみ sw.js に残し、 それ以前をここへ移動した。
docs/ は `.assetsignore` により deploy 対象外。

運用: sw.js 先頭の changelog が増えてきたら、 古いエントリを下の "Archived entries" の
フェンス先頭 (newest-first を維持) に移すこと。元ファイルの行順のまま保存している。
(元 sw.js ではこの途中、 v2053 の直後に `const CACHE_VERSION = ...;` 行が挟まっていた)
