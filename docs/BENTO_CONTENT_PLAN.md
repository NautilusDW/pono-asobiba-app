...
**作成日:** 2026-06-15
**最終改訂:** 2026-07-06 (tier v3 準拠改訂、詳細は §10 変更履歴)
...
> **⚠️ 2026-07-06 tier v3 更新:** tier 配分の全社的な正本は user memory `feature_tier_v3.md` (tier v3 確定仕様) に移った。 本ドキュメントは bento ゲーム固有の実装事実・アセット touch point・ロードマップの正本として残るが、 free/book/sub の **数量配分は tier v3 の原則 (free = book、機能差ゼロ) に従うよう本改訂で全面更新済み**。 §2 以降の「新方針」表は tier v3 準拠の最新版。 tier v3 以前の議論 (sub 上限 15 vs 20 など) は歴史的記録として §3/§6 に残すが、 実装時は必ず tier v3 の数字を優先すること。
...
## 2. オーナーの新方針 (2026-07-06 tier v3 改訂版、 旧方針は §10 参照)

### 2.1 おかず: **free = book (機能差ゼロ)、 sub は既存 30 エントリそのまま**

tier v3 の大原則「free = book、 差は付録のみ」を bento にも適用。 旧方針 (4〜5 / 10 / 15〜20 の 3 段階、 メイン・小さい別配分) は **廃止**。

| tier | 主菜 (メインのおかず) | 副菜 (小さいおかず) | フルーツ | 合計 |
|---|---|---|---|---|
| **無料 (free)** | **5 種類** | **4 種類** | **3 種類** | **12 種類** |
| **本購入 (book)** | free と同一 12 種類 (book 専用食材枠なし) | | | **12 種類** |
| **アプリ購入 (sub)** | 既存 `FREE_COOKED_OKAZU` の全エントリ (現状 30、 新規アセット不要) | | | **30 種類** |

- book は「食材」では free から一切拡張しない。 book の差別化は tier v3 §3 の付録 (特別シール 8 枚・特別表紙・welcome 演出・月1おかえり) のみで表現する。
- sub 上限は tier v3 で **30 (既存 UI 表示分そのまま)** に確定。 旧 Q1 (15 vs 20) は解消 (§8 参照)、 Phase 2 で予定していた「sub 向け +10 種新規調理済みアセット」も **不要**になった (§6.1 は歴史的記録として保持)。
- free/book に採用する具体的な 12 種 (主菜5・副菜4・フルーツ3) の銘柄選定は Phase 1 実装時に既存 `FREE_MAIN_OKAZU_NAMES` (10) / `FREE_SIDE_OKAZU_NAMES` (10) / フルーツ 9-10 種のプールから絞り込む (年齢バランス・見た目の彩り優先、 選定理由は [docs/AGE_BALANCE_POLICY.md](./AGE_BALANCE_POLICY.md) 側に記録予定)。

### 2.2 ご飯 + 飾り (のり) — tier v3 で「のり 1 種 free 復活」＋「ふりかけ系は sub」の 2 段階に再整理

**2026-07-03 決定 (batch:1056) は維持**: 無料の のり/かざり は目鼻口眉 5 点 (`nori_eye_round` / `nori_nose_bear` / `nori_mouth_smile` / `nori_brow_left` / `nori_brow_right`) + ピック 4 種。 **tier v3 (2026-07-06) 追加変更**: そこに **のりながしかく１ (旧「のりべん」) 1 種のみ** を free に復活させ、 代わりに ふりかけご飯 は book ではなく **sub 専用**に位置づけ直す (親満足度優先の微修正、 [feature_bento_tier_content] との差分は §10 参照)。

| tier | のり/かざり | ふりかけご飯 | ふりかけお絵かき | チャーハン/おにぎり/サンド |
|---|---|---|---|---|
| **無料 (free)** | 目鼻口眉 5 点 + **のりながしかく１ (1 種、 tier v3 で復活)** + ピック 4 種 | なし | なし | なし |
| **本購入 (book)** | free と同一 (顔部品・ながしかく１・ピック) + 顔セット 3 種 (にこ/くま/ねこ)・ほっぺ・うめぼし・既存 book のり形状 (乗り物/あそび系 5 種) | **なし (sub へ移動)** | なし | なし |
| **アプリ購入 (sub)** | あり (book と同一) | **あり (sub 専用に確定)** | **新規実装 (sub 専用)** | **将来追加** (Phase 3) |

無料でも目鼻口眉 5 点 + のりながしかく１ で最低限の顔作り・平面のり装飾が可能 (完全剥奪はしない、 親満足度を優先)。 ワンタップ配置の顔セット / ほっぺ / うめぼし / のり形状 (乗り物・あそび系) は book 以上のまま。 ピック (4 種) は無料継続 (Q3 解決、 2026-07-03、 変更なし)。 ふりかけご飯・ふりかけお絵かきは tier v3 の「sub = 蓄積系」原則に合わせて sub 専用に確定 (旧方針の「本購入にふりかけご飯追加」は撤回)。

### 2.3 NPC (依頼者モード) — 現状方針を継続

tier v3 でも「NPC 依頼者モードは book 以上」の現状方針を維持する。 **book は依頼者モードが使えるが、 依頼を満たすのに使える食材は free と同じ 12 種のまま**（book 専用食材枠がないため）。 sub のみ NPC 3 種追加 (計 6) で、 sub 食材 30 種を使った依頼が発生しうる。

| tier | NPC |
|---|---|
| 無料 (free) | なし |
| 本購入 (book) | risu / inu / shika (3、 wantedFoods は free=book の 12 種の範囲に限定すること) |
| アプリ購入 (sub) | + ahiru / panda / neko (計 6、 sub 食材 30 種を使った依頼も可) |

→ 既存の「NPC `shika` の wantedFoods に sub 食材が混ざる問題」(§7) は、 book 食材枠が free と同一化したことでより重要な修正必須事項になった。 Phase 1 で必ず確認すること。
...
## 4. 新方針の目標表 (tier v3 準拠、 free / book / sub × 主菜 / 副菜 / フルーツ / ご飯 / 飾り)

| 軸 | free | book | sub |
|---|---|---|---|
| **主菜 (メインのおかず)** | 5 | free と同一 (5) | 既存全エントリ (現状 10) |
| **副菜 (小さいおかず)** | 4 | free と同一 (4) | 既存全エントリ (現状 10) |
| **フルーツ** | 3 | free と同一 (3) | 既存全エントリ (現状 9〜10、 Q2 で役割確定) |
| **食材合計** | **12** | **12 (book 専用枠なし)** | **30 (既存 `FREE_COOKED_OKAZU` 全量、 新規アセット不要)** |
| **ご飯ベース** | rice_base_round のみ | + bear / cat 形 | + **ふりかけご飯**・+ **ふりかけお絵かき** (Phase 2、 sub 専用) |
| **飾り (のり 目鼻口眉 5 + のりながしかく１)** | あり (tier v3 でながしかく１ 追加復活) | あり (free と同一) | あり |
...
| **NPC (依頼者モード)** | なし | risu / inu / shika (3、 wantedFoods は free=book の 12 種限定) | + ahiru / panda / neko (計 6、 sub 30 種込みの依頼も可) |

ご飯・のりの扱いを「飾れる/飾れない」で書き換えた点が、 旧 [TIER_POLICY.md §12](./TIER_POLICY.md) との大きな差分 (旧版は「のり剥奪」概念がなかった)。 さらに tier v3 (2026-07-06) で「食材は free=book で機能差ゼロ、 差別化は付録のみ」の原則が確定し、 book 専用食材枠という考え方自体を廃止した。
...
- [ ] **tier v3: 食材を free=book 12 種 (主菜5/副菜4/フルーツ3) に統一**。 `FREE_MAIN_OKAZU_NAMES` (10) / `FREE_SIDE_OKAZU_NAMES` (10) / フルーツプールから 5/4/3 を選定し、 book 専用食材枠 (旧 `BOOK_BENTO_FOOD_NAMES` の free 超過分) を廃止。 sub は既存 30 エントリのまま変更なし。
- [ ] **tier v3: のりながしかく１ を book → free に降格**。 `bookDecorItem` ラッパーを外して free 扱いに変更 (他の顔セット3種・ほっぺ・うめぼし・のり形状5種は book 据え置き)。
...
- [ ] NPC `shika` の wantedFoods を free=book 12 種の範囲に限定する tier ガードを追加 (sub 食材が book 以下の依頼に混ざらないようにする、 §2.3 参照)
...
- [ ] [common/tier.js](../common/tier.js) の bento 定数を tier v3 の集合方式 (indexOf ベース統一) に合わせてリファクタ: `FREE_BENTO_FOOD_NAMES` (8→12)、 `BOOK_BENTO_FOOD_NAMES` は free と同一集合に統合 (別集合として維持する場合も値は free と完全一致させる)、 `ALL_BENTO_FOOD_NAMES` は 30 のまま変更なし
...
- **feature_tier_v3 (2026-07-06)** 「free = book (機能差ゼロ)、 差別化は付録のみ、 sub は物量差で構造的防御」の全社方針を bento にも適用。 旧 feature_bento_tier_content の「のり剥奪」方針と微衝突した箇所 (のりながしかく１) は、 親満足度優先で free 復活する側に倒した (§2.2 / §10)。
...
| のり (のりながしかく１) | book 以上 | **free に復活 (1 種のみ)**、 顔セット3種・ほっぺ・うめぼし・のり形状5種は book 以上のまま |
| ふりかけご飯 | book で新規追加予定 | **sub 専用に変更** (tier v3 の「sub=蓄積系」原則に合わせる) |
| ふりかけお絵かき | sub 限定 (変更なし) | sub 限定 (変更なし) |
| NPC 依頼者モード | book 以上 (変更なし) | book 以上 (変更なし)、 **ただし book NPC の wantedFoods は free=book の 12 種限定であることを明記** (実装時の tier ガード漏れ防止) |
| §3/§6.1 の「sub 向け+10種新規アセット」分析 | Phase 2 の実施予定項目 | **不要化、 歴史的記録として保持** |

理由: tier v3 の全社原則「free = book で機能差ゼロ、 book の価値は付録 (特別シール・特別表紙・welcome演出) のみで表現、 合言葉流出耐性は sub の物量差で構造的防御する」を bento にも一貫適用するため。 旧方針 (メイン/小さいで free 4〜5・book 10・sub 15〜20 という 3 段階配分) は book が食材面でも free と差別化されていて tier v3 の原則と矛盾するため廃止。
