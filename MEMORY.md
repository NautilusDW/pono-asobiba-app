...
- **音タッチ リズムモード Guitar Hero 化** (2026-05-31, sw v703→v708): [memory/feature_oto_rhythm_guitar_hero.md](memory/feature_oto_rhythm_guitar_hero.md) — `oto/index.html` のリズムモードのみ、**画面右65% (left:35%)** に8レーン横一列 + ノート真上から垂直落下 (3.2s) + 鍵盤上端ヒットライン判定方式へ。左35%は既存メニュー(モード切替/ねいろ/ばんそう)用に確保。ボタン left = `35% + (idx+0.5)*8.125%` でノート x 座標と viewport 完全一致。DOM + `transform: translate3d` で 60fps、flashTimers 追跡 / hitYキャッシュ / resize リスナーで cleanup 対称性確保。**v708 で主旋律 startPerfTime を `bandStartPerf + N*beatMs` で拍同期化 (旧実装は主旋律が伴奏ドラムに対し +1.8s ずれていた = RHYTHM_FALL_MS - RHYTHM_RING_LEAD_MS) + 赤ミスフラッシュは `activeNotes.length > 0` のときだけ発火**。ヒットラインは中立白 (赤光と誤認回避)、子供向けに `--gh-sz` メディアクエリ + bottom 28px 浮上
