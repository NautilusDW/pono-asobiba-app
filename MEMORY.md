...
- **Mojikko お世話フローのミニトイ (太鼓 + 積み木) — sw v742** (2026-06-02): [memory/feature_mojikko_care_toys.md](memory/feature_mojikko_care_toys.md) — writing-mori/play.html 内 2 トイ。**(1) リズム遊びを太鼓の達人式に転換**: メロディは BGM として無改修で自動再生、別途 `buildDrumChart()` で拍グリッド駆動のドラム譜面生成 (beat 0/2=don 固定 / 1,3 で melody accent ありなら ka)、新規 `playDon()` (80→50Hz sine + 200Hz triangle attack ~0.16s) + `playKa()` (1700Hz BP noise + 600→400Hz square ~0.08s)、ノートは赤=don/青=ka、ドラム面はシングル surface でタップ時に当たったノートの type を見て自動で対応音、譜面 beat と BGM スケジューラは同じ `miniRoundStartedAt` (perf.now 起点) を共有。**(2) 積み木 (Matter.js mini-stack) を本家 stacking/index.html 設定に完全ポート**: `slop: 0.015` 明示削除 (Matter デフォルト 0.05) ← 小ブロック貫通の主因、gravity 1.05→1.0、friction 1.35→1.4、frictionAir 0.055→0.06、壁厚 68→50px + 中心 ±25 (内側面=ステージ境界)、壁高さ stageH*2→stageH*3、dt clamp 34→16.67ms、positionIter/velocityIter/constraintIter=10/8/4 (本家デフォ上回り、手動 rAF Engine.update のジッタ補償)。**恒久ルール**: (a) 小ブロックに `slop` 明示指定するな (Matter デフォルト 0.05 が ~56px に最適)、(b) 手動 Engine.update の dt clamp は 16.67ms (60FPS) が安全圏、(c) 壁中心は厚みの半分外側に置く (内側面=ステージ境界に揃える)。残課題: `.rattle-tap-cue` 文言が「ドン」固定 (ka 時も「ドン」表示) + `isStackBodyOut` が天井チェック無し (Low)
...
### 2026-06-02T03:02:47Z - Mojikko リズム遊びを太鼓の達人式に変更（メロディBGM継続、ドン/カッ2音ドラム入力）+ 積み木物理を本家stacking/index.html設定にポート（slop除去、gravity1.0、wall 50px、dt clamp 16.67ms）
- **タスク**: Mojikko リズム遊びを太鼓の達人式に変更（メロディBGM継続、ドン/カッ2音ドラム入力）+ 積み木物理を本家stacking/index.html設定にポート（slop除去、gravity1.0、wall 50px、dt clamp 16.67ms）
- **結果**: 成功
- **理由**: N/A
- **総アクション数**: 23
- **エラー数**: 2
...
- **検出された悪いパターン**: 同じエラーを繰り返した
- **有効だったアクション**: テストを先に書いてから実装した (TDD), 編集前にファイルを読んで理解した, エラー発生後に別のアプローチに切り替えた
- **ツール使用統計**: {"Agent": 33, "ToolSearch": 1, "Bash": 3, "Read": 13, "Write": 2, "Edit": 35}
- **サマリ**: 成功タスク: 3個の有効パターンを検出。 改善余地: 1個の非効率パターンあり。


### 2026-05-31T13:50:04Z - oto v718: landscape リズムモードで 8 鍵盤が下端に表示されない問題を #instruments 昇格 + !important 強化で修正
- **タスク**: oto v718: landscape リズムモードで 8 鍵盤が下端に表示されない問題を #instruments 昇格 + !important 強化で修正
- **結果**: 成功
- **理由**: N/A
- **総アクション数**: 76
- **エラー数**: 8
