...
- **【主軸変更 2026-05-12】 VOICEVOX 雨晴はう廃案 + くるみ全件 VOICEPEAK 「女の子」 統一**: [memory/feature_quizland_voicepeak_pivot.md](memory/feature_quizland_voicepeak_pivot.md) — 元々 VOICEVOX 雨晴はうで 907 件の問題本編を発注予定だったが、 試聴で雰囲気が合わず却下。 全 912 件 (= 問題本編 907 + 第1〜5問目 5) を **VOICEPEAK 「女の子」 プリセット** に統一。 博士担当 48 件 = VOICEPEAK 「男性3」 + 年配パラメータ別 Agent 調査中、 ポノ = 音声化保留 (babble のまま)。 VOICEVOX 関連スクリプト・ツール・辞書は当面温存 (削除はユーザー判断待ち)。 今後 「くるみ音声」 「VOICEVOX」 「雨晴はう」 のキーワードが出たら必ず本ファイルを参照して文脈確認
