...
const SPACE_CHASE_RESCUE_CATCH_MS=700;
const SPACE_CHASE_RESCUE_ORBIT_MS=7600;
const SPACE_CHASE_RESCUE_ASSIST_SHOW_MS=6000;
const SPACE_CHASE_RESCUE_AUTO_ASSIST_MS=14000;
...
const SPACE_CHASE_RESCUE_DEMO_SPEED=.35;
...
function createSpaceChaseState(){return {phase:"idle",phaseElapsedMs:0,frameAt:0,raceElapsedMs:0,rocket:null,comet:null,playerChoices:Object.create(null),cometChoices:Object.create(null),activeJunction:"",activeJunctionElapsedMs:0,starFuel:0,boostCharges:0,boostRemainingMs:0,boostReadyElapsedMs:0,collectedBoosts:new Set(),rivalCounterUsed:false,rivalCounterRemainingMs:0,leadSide:"comet",leadChangeCount:0,finalMode:"",finalStartGap:1,finalSprintElapsedMs:0,finalPulseRemainingMs:0,finalTapCount:0,finalAssistActive:false,cinematicKind:"",cinematicRemainingMs:0,rescueElapsedMs:0,rescueOrbitElapsedMs:0,rescueCaptureMs:0,rescueRingX:.34,rescueRingY:.52,rescueAimX:.34,rescueAimY:.52,rescueStarX:.70,rescueStarY:.50,rescuePointerId:null,rescuePointerStartX:0,rescuePointerStartY:0,rescuePointerMoved:false,rescueSuppressClickUntil:0,rescueAssistActive:false,rescueHasInput:false,rescueAimNear:false,rescueNear:false,rescuePerfect:false,completionCommitted:false};}
...
function updateSpaceChaseRescueVisual(){
 if(!spaceChaseRescuePanel)return;const visible=spaceChaseState.phase==="rescue"||spaceChaseState.phase==="victory";
 spaceChaseRescuePanel.hidden=!visible;spaceChaseRescuePanel.setAttribute("aria-hidden",visible?"false":"true");if(visible)spaceChaseRescuePanel.removeAttribute("inert");else spaceChaseRescuePanel.setAttribute("inert","");
 const demo=spaceChaseState.phase==="rescue"&&!spaceChaseState.rescueHasInput&&!spaceChaseState.rescueAssistActive;spaceChaseRescuePanel.classList.toggle("is-victory",spaceChaseState.phase==="victory");spaceChaseRescuePanel.classList.toggle("is-launching",demo&&spaceChaseState.rescueElapsedMs<520);spaceChaseRescuePanel.classList.toggle("is-demo",demo);spaceChaseRescuePanel.classList.toggle("has-input",spaceChaseState.rescueHasInput||spaceChaseState.rescueAssistActive);spaceChaseRescuePanel.classList.toggle("is-aimed",spaceChaseState.rescueAimNear);spaceChaseRescuePanel.classList.toggle("is-near",spaceChaseState.rescueNear);spaceChaseRescuePanel.classList.toggle("is-perfect",spaceChaseState.rescuePerfect);spaceChaseRescuePanel.classList.toggle("is-assisted",spaceChaseState.rescueAssistActive);
 spaceChaseRescuePanel.style.setProperty("--rescue-ring-x",(spaceChaseState.rescueRingX*100).toFixed(2)+"%");spaceChaseRescuePanel.style.setProperty("--rescue-ring-y",(spaceChaseState.rescueRingY*100).toFixed(2)+"%");spaceChaseRescuePanel.style.setProperty("--rescue-aim-x",(spaceChaseState.rescueAimX*100).toFixed(2)+"%");spaceChaseRescuePanel.style.setProperty("--rescue-aim-y",(spaceChaseState.rescueAimY*100).toFixed(2)+"%");spaceChaseRescuePanel.style.setProperty("--rescue-star-x",(spaceChaseState.rescueStarX*100).toFixed(2)+"%");spaceChaseRescuePanel.style.setProperty("--rescue-star-y",(spaceChaseState.rescueStarY*100).toFixed(2)+"%");
 const catchRatio=clamp(spaceChaseState.rescueCaptureMs/SPACE_CHASE_RESCUE_CATCH_MS,0,1),field=spaceChaseRescuePlayfield&&spaceChaseRescuePlayfield.getBoundingClientRect?spaceChaseRescuePlayfield.getBoundingClientRect():{width:window.innerWidth||844,height:window.innerHeight||390},startX=field.width*.28,startY=field.height*.54,ringX=field.width*spaceChaseState.rescueRingX,ringY=field.height*spaceChaseState.rescueRingY,aimX=field.width*spaceChaseState.rescueAimX,aimY=field.height*spaceChaseState.rescueAimY;
 spaceChaseRescuePanel.style.setProperty("--rescue-catch-power",(catchRatio*100).toFixed(1)+"%");spaceChaseRescuePanel.style.setProperty("--rescue-tether-length",Math.hypot(ringX-startX,ringY-startY).toFixed(1)+"px");spaceChaseRescuePanel.style.setProperty("--rescue-tether-angle",(Math.atan2(ringY-startY,ringX-startX)*180/Math.PI).toFixed(2)+"deg");spaceChaseRescuePanel.style.setProperty("--rescue-aim-line-left",ringX.toFixed(1)+"px");spaceChaseRescuePanel.style.setProperty("--rescue-aim-line-top",ringY.toFixed(1)+"px");spaceChaseRescuePanel.style.setProperty("--rescue-aim-line-length",Math.hypot(aimX-ringX,aimY-ringY).toFixed(1)+"px");spaceChaseRescuePanel.style.setProperty("--rescue-aim-line-angle",(Math.atan2(aimY-ringY,aimX-ringX)*180/Math.PI).toFixed(2)+"deg");
 if(spaceChaseRescueProgress){const value=Math.round(catchRatio*100);spaceChaseRescueProgress.setAttribute("aria-valuenow",String(value));spaceChaseRescueProgress.setAttribute("aria-valuetext",value+"パーセント");}
 if(spaceChaseRescueAssist)spaceChaseRescueAssist.hidden=spaceChaseState.phase!=="rescue"||spaceChaseState.rescueElapsedMs<SPACE_CHASE_RESCUE_ASSIST_SHOW_MS||spaceChaseState.rescueAssistActive;
}
...
function resetSpaceChaseRescueStar(){
 spaceChaseState.rescueStarX=.70;spaceChaseState.rescueStarY=.50;spaceChaseState.rescueRingX=.34;spaceChaseState.rescueRingY=.52;spaceChaseState.rescueAimX=.34;spaceChaseState.rescueAimY=.52;spaceChaseState.rescueOrbitElapsedMs=0;spaceChaseState.rescueCaptureMs=0;spaceChaseState.rescueHasInput=false;spaceChaseState.rescueAimNear=false;spaceChaseState.rescueNear=false;spaceChaseState.rescuePerfect=false;if(spaceChaseRescueStar)spaceChaseRescueStar.classList.remove("is-rescued");
}
...
function clearSpaceChaseEncounter(){
 ...
 if(spaceChaseCinematicText)spaceChaseCinematicText.textContent="デッドヒート！";if(spaceChaseRescueStar)spaceChaseRescueStar.classList.remove("is-rescued");if(spaceChaseRescueGuide)spaceChaseRescueGuide.textContent="ほしのこを ゆびで おいかけよう！";if(spaceChaseRescueAssist)spaceChaseRescueAssist.hidden=true;
 if(spaceChaseRescuePanel){spaceChaseRescuePanel.hidden=true;spaceChaseRescuePanel.setAttribute("aria-hidden","true");spaceChaseRescuePanel.setAttribute("inert","");spaceChaseRescuePanel.classList.remove("is-victory","is-launching","is-demo","has-input","is-aimed","is-near","is-perfect","is-assisted");spaceChaseRescuePanel.removeAttribute("style");}
...
function beginSpaceChaseCaught(){
 if(spaceChaseState.phase!=="final")return;spaceChaseState.phase="caught";spaceChaseState.phaseElapsedMs=0;spaceChaseState.finalPulseRemainingMs=0;spaceChaseState.activeJunction="";updateSpaceChaseRouteChoiceControls();startSpaceChaseCinematic("catch","だいぎゃくてん！",SPACE_CHASE_CINEMATIC_MS);setSpaceChaseGuide("おいついた！ しっぽが ほどける！",true);showStamp("だいぎゃくてん！","clear");tone(660,0,.09,"triangle",.055);tone(880,.07,.15,"sine",.05);updateSpaceChaseVisual();
}
function beginSpaceChaseRescue(){
 if(spaceChaseState.phase!=="caught")return;spaceChaseState.phase="rescue";spaceChaseState.phaseElapsedMs=0;spaceChaseState.rescueElapsedMs=0;spaceChaseState.rescueAssistActive=false;cancelSpaceChaseRescuePointer(true);resetSpaceChaseRescueStar();
 if(spaceChaseRescueTitle)spaceChaseRescueTitle.textContent="ほしのこに わを あわせよう！";if(spaceChaseRescueGuide)spaceChaseRescueGuide.textContent="ほしのこを ゆびで おいかけよう！";if(spaceChaseRescueAssist)spaceChaseRescueAssist.hidden=true;setSpaceChaseGuide("レスキューリング はっしゃ！",true);updateSpaceChaseVisual();
 if(spaceChaseRescuePlayfield)try{spaceChaseRescuePlayfield.focus({preventScroll:true});}catch(_){spaceChaseRescuePlayfield.focus();}
}
function acceptSpaceChaseRescueInput(){
 if(!spaceChaseRescueInputActive())return false;if(!spaceChaseState.rescueHasInput){spaceChaseState.rescueHasInput=true;if(spaceChaseRescueGuide)spaceChaseRescueGuide.textContent="ほしのこを ねらおう！";announce("ほしのこを ねらおう！");}return true;
}
function setSpaceChaseRescueAimFromClient(clientX,clientY){
 if(!spaceChaseRescueInputActive()||!spaceChaseRescuePlayfield)return false;const field=spaceChaseRescuePlayfield.getBoundingClientRect();if(!field.width||!field.height)return false;acceptSpaceChaseRescueInput();spaceChaseState.rescueAimX=clamp((clientX-field.left)/field.width,.10,.92);spaceChaseState.rescueAimY=clamp((clientY-field.top)/field.height,.17,.88);return true;
}
function handleSpaceChaseRescuePointerDown(event){
 if(!spaceChaseRescueInputActive()||event.button>0||event.isPrimary===false)return;if(spaceChaseState.rescuePointerId!==null)cancelSpaceChaseRescuePointer(false);ensureAC();spaceChaseState.rescuePointerId=event.pointerId;spaceChaseState.rescuePointerStartX=event.clientX;spaceChaseState.rescuePointerStartY=event.clientY;spaceChaseState.rescuePointerMoved=false;spaceChaseState.rescueSuppressClickUntil=_nowMs()+450;setSpaceChaseRescueAimFromClient(event.clientX,event.clientY);try{spaceChaseRescuePlayfield.setPointerCapture(event.pointerId);}catch(_){}event.preventDefault();
}
function moveSpaceChaseRescuePointer(event){
 if(event.pointerId!==spaceChaseState.rescuePointerId||!spaceChaseRescueInputActive())return;spaceChaseState.rescuePointerMoved=spaceChaseState.rescuePointerMoved||Math.hypot(event.clientX-spaceChaseState.rescuePointerStartX,event.clientY-spaceChaseState.rescuePointerStartY)>8;setSpaceChaseRescueAimFromClient(event.clientX,event.clientY);event.preventDefault();
}
...
function handleSpaceChaseRescueClick(event){
 if(!spaceChaseRescueInputActive())return false;if(_nowMs()<spaceChaseState.rescueSuppressClickUntil){spaceChaseState.rescueSuppressClickUntil=0;event.preventDefault();return false;}const handled=setSpaceChaseRescueAimFromClient(event.clientX,event.clientY);if(handled){ensureAC();event.preventDefault();}return handled;
}
function moveSpaceChaseRescueAimBy(dx,dy){
 if(!spaceChaseRescueInputActive())return false;acceptSpaceChaseRescueInput();spaceChaseState.rescueAimX=clamp(spaceChaseState.rescueAimX+dx,.10,.92);spaceChaseState.rescueAimY=clamp(spaceChaseState.rescueAimY+dy,.17,.88);return true;
}
...
function activateSpaceChaseRescueAssist(){
 if(!spaceChaseRescueInputActive()||spaceChaseState.rescueAssistActive)return false;ensureAC();spaceChaseState.rescueAssistActive=true;spaceChaseState.rescueHasInput=true;if(spaceChaseRescueAssist)spaceChaseRescueAssist.hidden=true;if(spaceChaseRescueGuide)spaceChaseRescueGuide.textContent="じしゃくで いっしょに キャッチ！";announce("じしゃくで いっしょに キャッチ！");tone(620,0,.1,"sine",.05);return true;
}
function updateSpaceChaseRescue(dt){
 if(spaceChaseState.phase!=="rescue")return;spaceChaseState.rescueElapsedMs+=dt;if(spaceChaseState.rescueElapsedMs>=SPACE_CHASE_RESCUE_AUTO_ASSIST_MS)spaceChaseState.rescueAssistActive=true;
 const reduced=futureReducedMotion(),demo=!spaceChaseState.rescueHasInput&&!spaceChaseState.rescueAssistActive;spaceChaseState.rescueOrbitElapsedMs+=dt*(demo?SPACE_CHASE_RESCUE_DEMO_SPEED:1);const period=reduced?11200:SPACE_CHASE_RESCUE_ORBIT_MS,theta=spaceChaseState.rescueOrbitElapsedMs/period*Math.PI*2,shrink=1-clamp((spaceChaseState.rescueElapsedMs-SPACE_CHASE_RESCUE_ASSIST_SHOW_MS)/8000,0,.36),motion=reduced?.58:1;
 spaceChaseState.rescueStarX=.70+Math.sin(theta)*.17*shrink*motion;spaceChaseState.rescueStarY=.50+Math.sin(theta*2)*.23*shrink*motion;
 if(spaceChaseState.rescueAssistActive){spaceChaseState.rescueAimX=spaceChaseState.rescueStarX;spaceChaseState.rescueAimY=spaceChaseState.rescueStarY;}
 ...
 const field=spaceChaseRescuePlayfield&&spaceChaseRescuePlayfield.getBoundingClientRect?spaceChaseRescuePlayfield.getBoundingClientRect():{width:window.innerWidth||844,height:window.innerHeight||390},aspect=Math.max(1,field.width/Math.max(1,field.height)),aimDistance=Math.hypot((spaceChaseState.rescueStarX-spaceChaseState.rescueAimX)*aspect,spaceChaseState.rescueStarY-spaceChaseState.rescueAimY),catchDistance=Math.hypot((spaceChaseState.rescueStarX-spaceChaseState.rescueRingX)*aspect,spaceChaseState.rescueStarY-spaceChaseState.rescueRingY),aimNear=aimDistance<=SPACE_CHASE_RESCUE_AIM_RADIUS,near=catchDistance<=SPACE_CHASE_RESCUE_CATCH_RADIUS,perfect=catchDistance<=SPACE_CHASE_RESCUE_PERFECT_RADIUS,wasAimNear=spaceChaseState.rescueAimNear,wasNear=spaceChaseState.rescueNear,wasPerfect=spaceChaseState.rescuePerfect;spaceChaseState.rescueAimNear=aimNear;spaceChaseState.rescueNear=near;spaceChaseState.rescuePerfect=perfect;
 if(near){spaceChaseState.rescueCaptureMs=Math.min(SPACE_CHASE_RESCUE_CATCH_MS,spaceChaseState.rescueCaptureMs+dt*(perfect?2:1));if(perfect&&!wasPerfect){if(spaceChaseRescueGuide)spaceChaseRescueGuide.textContent="まんなか！ パワー にばい！";tone(920,0,.06,"triangle",.04);}else if(!wasNear){if(spaceChaseRescueGuide)spaceChaseRescueGuide.textContent="ロック！ そのまま！";tone(760,0,.06,"sine",.035);}}
 else if(aimNear&&!wasAimNear){if(spaceChaseRescueGuide)spaceChaseRescueGuide.textContent="ねらいが あった！ わを まとう！";tone(680,0,.055,"sine",.03);}
 else if((wasNear||wasAimNear)&&spaceChaseRescueGuide)spaceChaseRescueGuide.textContent="ほしのこを ねらおう！";
 if(spaceChaseState.rescueCaptureMs>=SPACE_CHASE_RESCUE_CATCH_MS)finishSpaceChaseVictory();
}
function finishSpaceChaseVictory(){
 if(spaceChaseState.phase!=="rescue"||spaceChaseState.completionCommitted)return;cancelSpaceChaseRescuePointer(false);spaceChaseState.phase="victory";spaceChaseState.phaseElapsedMs=0;spaceChaseDefeated=true;if(spaceChaseRescueStar)spaceChaseRescueStar.classList.add("is-rescued");if(spaceChaseRescueGuide)spaceChaseRescueGuide.textContent="ロックオン！ ロケットへ！";if(spaceChaseRescueTitle)spaceChaseRescueTitle.textContent="ほしのこを たすけた！";if(spaceChaseRescueAssist)spaceChaseRescueAssist.hidden=true;setSpaceChaseGuide("ほしのこを たすけた！",true);showStamp("おたすけ だいせいこう！","clear");tone(784,0,.13,"triangle",.075);tone(1047,.1,.2,"sine",.07);confetti(24);updateSpaceChaseVisual();
}
...
function updateSpaceChase(now){
 ...
 else if(spaceChaseState.phase==="victory"&&spaceChaseState.phaseElapsedMs>=(futureReducedMotion()?360:SPACE_CHASE_VICTORY_MS)&&!spaceChaseState.completionCommitted){spaceChaseState.completionCommitted=true;const stageOrigin=origin(stg);clearSpaceChaseEncounter();completeCurrentStage(stageOrigin);return;}
...
if(spaceChaseRescueAssist)spaceChaseRescueAssist.addEventListener("click",activateSpaceChaseRescueAssist);
...
function nazonazoAdminPreviewArm(stageId,previewKind){
 ...
 resetNumberCargoGame();resetSeaInteraction();clearFutureCapsuleGame();clearSpaceRepairGame();resetSpaceSteering();
