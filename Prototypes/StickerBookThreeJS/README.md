...
- As of batch `652`, the free/book-range sticker candidates are collection-first rather than achievement-first. Forced proof/reward items such as medals, final balloons, generic stars, the maze plank, and the rhythm burst background were removed or replaced. `quizland` uses the corrected lion asset `assets/images/quizland/illust/stage/lion_osu.png`; the later canonical choice assets supersede the retired animal word crops. `puzzle` stays on partner characters plus two generated puzzle-piece candidates, with no unrelated RPG/tablet/bridge assets.
...
- As of batch `673`, the turning page back side uses the destination sticker-page texture instead of the old ruled notebook back texture. Empty saved states are seeded with existing stickers across the 12 free sticker pages so staging no longer opens as a blank album.
- As of batch `674`, the page editor is a user-facing feature on staging: clicking a page opens the sticker/drawing editor, while prototype tuning controls remain local-only. The camera framing was tightened so the album reads larger in 16:9 and compact landscape viewports.
