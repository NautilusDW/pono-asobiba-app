...
// v1726: maze AcornModal UI 細部 4 点反映 — (1) capped state 「もう きょうは いっぱい〜」 重複文言の片方を撤去 (重複コピー dedupe)、 (2) close × ボタンを panel 右上の固定座標 (top:clamp + right:clamp + position:absolute) に配置し葉装飾と分離、 (3) 「タップして つぎへ」 tap-hint を panel 内側 (bottom inset + margin-inline auto) に収め panel 外はみ出しを解消 (v1725 の bottom:-72px..-56px 浮かせ方式を撤回し panel 内完結化)、 (4) URL ?dev=1 / dev=true の時のみ daily cap を bypass するための _isMazeDevBypass() helper + localStorage バケットを `pono_maze_acorn_cap_dev_<YYYYMMDD>` に分離 (本番バケット `pono_maze_acorn_cap_<YYYYMMDD>` は無汚染)。 既存 cache strategy / skipWaiting / clients.claim (install/activate で呼ばない) 設計は無改変、 quizland / oto / bento の AcornModal selector scope ([data-game-id="maze"]) で隔離。
