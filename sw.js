...
const CACHE_VERSION = 1562; // v1562: 底部ナビ 4ボタン化を絵柄レベルで完全化 — 鐘 (おしらせ) アイコンを焼き込んだ bottom_nav_group_5_*.png を PIL で編集 (上記 Image2 例外参照) して 4 アイコン等間隔の bottom_nav_group_4_{normal,pressed}.png を生成。 play.html の grid-template-columns を repeat(5,1fr) → repeat(4,1fr)、 background-image 参照を新スプライトに切り替え、 .bn-item flex basis 20% → 25%、 5列向けコメント文言も 4 ボタン基準に更新。 旧 _5_ スプライトは履歴・revert 用に assets/ui/ に残置
