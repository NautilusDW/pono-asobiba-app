...
// Image2 例外: ユーザー明示指示で「既存スプライトの編集 (鐘列を背景で塗り潰し、 4 アイコンを横詰め再配置)」 を PIL で実施。 これは新規視覚素材の『生成』 ではなく既存アセットのレタッチ。 GPT Image 2 で 4 アイコン版を再生成すると木製プレートのトーン/筆致が連続性を失うため、 既存 _5_ スプライトのピクセルを再利用する PIL 経路を採用。
const CACHE_VERSION = 1563; // v1563: シール帳 チュートリアル ナレーション TTS3.1 (Gemini 3.1 Flash TTS Preview) で全 10 本再生成 + 台本ロールバック — v1559 で chirp3 誤読対策に書き換えた 「ぺたっと はろう」 / 「シールアルバム」 をユーザー指示で元の 「すきな ところに はろう」 / 「すきな シールちょうを つくろう」 に戻し、 TTS3.1 ネイティブで生成 (LP production worker /api/ai-tts 経由、 staging-app は GEMINI_API_KEY 未設定で chirp3 fallback してしまうため)。 04 「はろう」 だけ Gemini が英語 Hello で発音する誤読が出たため promptText を 「貼ろう」 漢字 + 「Pure Japanese narration」 に強化した variant4 を採用、 whisper small で 「好きなところに張ろう」 transcript 確認済。 10 本すべて loudnorm I=-14 LRA=11 TP=-1.5 適用 → 24kHz mono mp3 48kbps (既存形式と整合)。 main.js の STICKER_TUTORIAL_STEPS text 2 箇所も同時にロールバック (line 1625, 1697)
// v1562: 底部ナビ 4ボタン化を絵柄レベルで完全化 — 鐘 (おしらせ) アイコンを焼き込んだ bottom_nav_group_5_*.png を PIL で編集 (上記 Image2 例外参照) して 4 アイコン等間隔の bottom_nav_group_4_{normal,pressed}.png を生成。 play.html の grid-template-columns を repeat(5,1fr) → repeat(4,1fr)、 background-image 参照を新スプライトに切り替え、 .bn-item flex basis 20% → 25%、 5列向けコメント文言も 4 ボタン基準に更新。 旧 _5_ スプライトは履歴・revert 用に assets/ui/ に残置
// v1561: play.html フクロウ博士「おしらせ」 ボタン削除 — bottom nav 5→4 ボタン (help/official/stickers/settings)、 #newsModal および 'news' クリックハンドラ撤去、 BOTTOM_NAV_PRESS_AREAS を 25% 均等 4 セルに再分配。 nav sprite (bottom_nav_group_5_*.png) は据え置き (差し替え素材は別タスク)
