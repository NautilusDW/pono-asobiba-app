...
// v1715: maze 旗あげ音声リズム v3 + グローブ z-order + 8 ミニゲームナレーション基盤 (batch:874/875/876 統合)。 (1) _MAZE_VOICE_BUNDLED whitelist (truefalse 3/simon 4/silhouette 4/oddone 3/strength_push 1/kumo 5 = 計 20 keys) を _playMazeVoice / _preloadMazeVoices に通して未配置 12 keys (janken×8/water_bridge×6/strength_push×4/kumo×1) を silent skip 化、 console 404 noise ゼロ。 (2) strength_push retry 時 crackPlayed/almostPlayed flag reset (_failStrengthPushGame)、 二重再生防止。 (3) _stopAllMazeVoices helper 追加 + _closeEncounter 冒頭 invoke、 water_bridge/strength_push 分岐は clearTimers→null 順序へ。 (4) kumo/start.mp3 物理削除 (whisper 不一致音声、 backup scratchpad)。 narration mp3 29 本 (flag 9/truefalse 3/simon 4/silhouette 4/oddone 3/strength_push 1/kumo 5) は runtime cache 任せ (precache list 不変)。 残 21 voice (janken×8/water_bridge×6/strength_push×4/kumo×2/flag×1) は batch:880-quota-retry で TTS quota 復活後追加予定。
