...
- [feature_quizland_voicevox_order.md](feature_quizland_voicevox_order.md) — **VOICEVOX 関連も含めた発注書ワークフロー**: 同じ「発注書 = git 管理 (Claude エージェント整備) / 実行物 = ローカルツール」の役割分担パターン (発注書 `docs/quizland-voicevox-order/` ↔ 生成ツール `tools/voicevox-generator/voicevox-generator.html`) を OP layout (saved-layout.json ↔ tools/op-layout-editor.html) と並列に運用。**2026-05-11 から VOICEVOX アクセント補正も Claude Code 一本化フローを採用** (機能 F でユーザーが `accent_input_xxx.json` を Export → Claude Code が `accent_overrides.json` を生成 → 機能 E で読込 → synthesis): 「ローカルツールで Export → Claude が JSON を反映」 という同じ運用パターンを OP layout と並列に運用 (OP layout 側は Claude が `saved-layout.json` の `__op_layout` をマージ + sw.js bump、 VOICEVOX 側は Claude が `accent_overrides.json` を新規生成、 課金回避のため Anthropic API 直接呼び出しは使わない)
