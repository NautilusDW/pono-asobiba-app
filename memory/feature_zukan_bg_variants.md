...
- 981 → 982: 投資画面エディタ (?edit=1) の userbox 画像差し替え永続化バグ修正。 swapBackgroundImageUndoable が `el.style.backgroundImage` のみ更新し `el.dataset.bgImage` を同期しなかったため、 collectUserboxes が saved-layout.json に空 or 旧画像を書き出していた (= 「部分的にしか反映されない」 症状)。 swap 時に dataset.bgImage を併せて更新し、 undo/redo も before/afterDataset で復元。 _isPersistedReplaceTarget にも .userbox を追加して「プレビューのみ」 警告を抑止。
- 982 → 983: `.le-dropped-img` を saved-layout.json に統合。 これまで localStorage のみだったため、 dirty flag が立たず保存ボタンがオレンジにならない / GitHub に反映されない / 他ブラウザで消える バグを解消。 snapshot に __droppedImages を含め、 applySavedData で復元。 localStorage は fallback として残す。
- 983 → 984: spec 要素 (`.hint-panel` / `.pono-bubble` / `.pono-portrait` / `.pono-guide` / `.field-bg` / `.window-frame` 等 RESIZABLE_SPEC 登録要素) の削除が永続化されない問題を修正。 `__hidden` 配列で永続化されるが、 旧 `deleteSelected` は spec 要素を DOM remove するだけで、 `collectHiddenKeys` (`$$('.user-hidden')`) は DOM から消えた要素を拾えないため `__hidden` に key が乗らず、 reload 時 HTML から fresh に復活する致命バグだった。 `_isSpecRegisteredElement(el)` ヘルパー追加 (state.spec マッチかつ .userbox / .le-dropped-img / .le-added-text 除外) で分岐、 spec 要素は `user-hidden` クラスを当てる hide 経路に倒し snapshot で `__hidden` に乗るように変更。 動的要素 (.userbox / .le-dropped-img / .le-added-text) は従来通り DOM remove で `__userboxes` / `__droppedImages` / `__added_texts` の配列から自然に抜けて永続化される。 Ctrl+Z で hide op の inverse / remove op の inverse でいずれも復活可能 (後方互換)。
