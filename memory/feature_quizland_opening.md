...
## くるみちゃん組み込み (sw v921+ / クロスレビュー反映 v922+ / 13 variants 管理 v923+)
...
- **saved-layout.json** (sw v923+ で 13 variants 拡張): `__op_layout.{B,C,D}.kurumi` に slot + **perVariant 13 entries** を保持。 初期値 B 280×380 / C 380×520 / D 460×630 (aspect 0.73)、 op-layout-editor の 「くるみ側」 タブで variant 別に slot 位置・サイズ調整可能。 全 variant が `kurumi_001` と同値で初期化 (後で editor で個別調整)
...
- **`quizland/saved-layout.json`** (sw v923+ で 13 variants 化): `__op_layout.{B,C,D}.kurumi` に slot + **perVariant 13 entries** を保持 (B 280×380 / C 380×520 / D 460×630、 aspect 0.73、 右寄せ初期値、 全 variant が `kurumi_001` と同値で初期化)
...
## くるみ 13 ポーズ管理機能 (sw v923+): editor 「くるみ側」 タブ + perVariant 13 entries 配信

`tools/op-layout-editor.html` を拡張し、 ポノ / 博士に並ぶ **3 番目のキャラ編集タブ「くるみ側」** を追加。 13 variants × 3 VC (B / C / D) で slot 位置・サイズ・透過オフセット等を **per-variant** で個別編集して saved-layout.json に publish できるようにした。 (キャラ全般のスペック・全 variants の用途表は [feature_quizland_kurumi.md](feature_quizland_kurumi.md) に集約)

### editor 拡張要素

| 要素 | 内容 |
|---|---|
| `KURUMI_VARIANTS` 定数 | 13 variants 名のリスト (`kurumi_001` / `kurumi_hi` / `kurumi_wave` / `kurumi_hooray` / `kurumi_wink` / `kurumi_clasp` / `kurumi_idea` / `kurumi_point` / `kurumi_calm` / `kurumi_pray` / `kurumi_book` / `kurumi_cheer` / `kurumi_greet`) |
...
| `buildRightPane()` | くるみ variant ドロップダウン (13 ポーズから選択 → preview 即時切替) |
...
- 配信フロー: ローカル editor 「くるみ側」 タブで variant 選択 → slot ドラッグ調整 → 「📋 JSON のみクリップボード」 → AI チャット貼付け → orchestrator が saved-layout.json マージ + commit + auto push → develop → staging → 全端末反映
