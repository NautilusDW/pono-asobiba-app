...
| 3 | dialogue  | 同上 | Pono panel 既定 = `dance_hooray.webp`、**hakase line で `pono_001.webp` (聞き役)、pono line で `dance_hooray.webp`**、台詞「きょうも なぞなぞを するかの？」「うん！やりたい！」 (2 行、 くるみは line 注入無しで非表示) |
| 4 | dialogue  | 同上 | Pono panel 既定 = `think_arms_crossed_side.webp`、**hakase line で `pono_001.webp` (聞き役)、pono line で `think_arms_crossed_side.webp`**、台詞「ふむふむ… じしんは あるかな？」「うーん…でも がんばる！」 (2 行、 くるみは line 注入無しで非表示) |
...
## くるみちゃん組み込み (sw v921+ / クロスレビュー反映 v922+ / 13 variants 管理 v923+ / クロスレビュー C HIGH3+MED4 反映 v924+ / 左ペイン Kurumi サムネ + シナリオ speaker 拡張 v925+ / scenario defaults 同期 v926+ / migrate + export 修正 v927+ / scenario auto migration v929+)
...
### 組み込み箇所まとめ (variant 確定値: sw v923+、 editor 拡張は v924/v925 で進化、 v926/v927 で scenario defaults 同期 + migrate/export 修正、 v929 で scenario state version 付き auto migration)

- **dialogue データ** (`quizland/index.html` ~L4869-L4911 = `OP_PANELS` Panel 2 / 5 / 6):
...
- **saved-layout.json** (sw v923+ で 13 variants 拡張): `__op_layout.{B,C,D}.kurumi` に slot + **perVariant 13 entries** を保持。 初期値 B 280×380 / C 380×520 / D 460×630 (aspect 0.73)、 op-layout-editor の 「くるみ側」 タブまたは **左ペイン Kurumi サムネ一覧 (v925+)** で variant 別に slot 位置・サイズ調整可能。 全 variant が `kurumi_001` と同値で初期化 (後で editor で個別調整)
...
くるみが「喋らないが画面に立っている」 panel (= Panel 6) を作るため、 立ち絵 visible 判定は **`speaker === 'kurumi' || d.kurumiImg` の OR 条件** にしてある。 `kurumiImg` を line に注入するだけで、 喋らせずに立ち絵だけ維持できる。 これにより:

- Panel 2 でくるみ初登場 (visible)
- Panel 3-4 で くるみ非表示 (line に kurumiImg なし → `keepKurumiVisible = false`)
- Panel 5 でくるみ再登場 (★行で発話)
- Panel 6 で発話無しのまま立ち絵維持 (両 line に kurumiImg 注入)
- → クイズ本編開始時には くるみが画面に立っていて 「読み手はこの子」 という導入が完結
...
- **`quizland/saved-layout.json`** (sw v923+ で 13 variants 化、 v924 でクロスレビュー C 修正、 v925 で editor 左ペイン Kurumi サムネ + シナリオ speaker 拡張、 v926/v927/v929 はデータ層変更なし — editor の defaults / migrate / export ロジック修正と version 付き auto migration のみ): `__op_layout.{B,C,D}.kurumi` に slot + **perVariant 13 entries** を保持 (B 280×380 / C 380×520 / D 460×630、 aspect 0.73、 右寄せ初期値、 全 variant が `kurumi_001` と同値で初期化)
...
| **`SCENARIO_DATA_VERSION` 定数 (v929 追加、 ~L1238)** | `'v927'` (= 本番 OP_PANELS と同期した defaults のバージョンタグ)。 `defaultScenario()` の構造を変えたら bump する単一の真実 |
| **`loadScenario()` の version check (v929 追加、 ~L1512-L1533)** | localStorage から読んだ state の `parsed.version` が現行 `SCENARIO_DATA_VERSION` と不一致なら、 saved state を破棄して `defaultScenario()` を返す (= 自動 migration)。 console.log で `'<old> → <new>'` を表示 |
| **`saveScenario()` の version embed (v929 追加、 ~L1535-L1545)** | 保存時に `Object.assign({}, state.scenario, { version: SCENARIO_DATA_VERSION })` で必ず最新 version を上書きしてから JSON.stringify。 import 経由などで version 欠落のまま入っても次回 save で揃う |
...
### saved-layout.json 連携

- `__op_layout.{B,C,D}.kurumi.perVariant` を **1 entry → 13 entries** に拡張
- 全 variant が `kurumi_001` と同じ初期値で開始 (後で editor の variant ドロップダウンで個別に微調整可能)
- 配信フロー: ローカル editor の **「対話 (P2-6)」モード** で 「くるみ側」 タブ または **左ペイン Kurumi サムネ一覧** から variant 選択 → slot ドラッグ調整 → 「📋 JSON のみクリップボード」 → AI チャット貼付け → orchestrator が saved-layout.json マージ + commit + auto push → develop → staging → 全端末反映
...
実装は `tools/op-layout-editor.html` 内の以下 4 箇所:

1. **`SCENARIO_DATA_VERSION = 'v927'` 定数** (~L1238): `defaultScenario()` の構造が変わったら bump する単一の真実
2. **`defaultScenario()`** (~L1348-L1424): 戻り値の object に `version: SCENARIO_DATA_VERSION` を埋め込み
3. **`loadScenario()` の version check** (~L1512-L1533): `localStorage[SCENARIO_STORAGE_KEY]` から読んだ object の `parsed.version !== SCENARIO_DATA_VERSION` なら、 saved state を **強制的に捨てて `defaultScenario()` を返す**。 不一致時は console.log で `'<old> → <new>'` を表示。 一致時のみ `migrateScenario(parsed)` を実行
4. **`saveScenario()` の version embed** (~L1535-L1545): 保存時に `Object.assign({}, state.scenario, { version: SCENARIO_DATA_VERSION })` で必ず最新 version を上書きしてから JSON.stringify。 import 経由などで version 欠落のまま state が入った場合も、 1 回保存すれば次回 load で正しく一致判定される
