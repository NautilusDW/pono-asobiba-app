...
| くるみ (`kurumi_001.webp` 439×602, aspect 0.73) | overlay 配置 (saved-layout `__op_layout.{B,C,D}.kurumi` で per-VC 制御、 初期 B 280×380 / C 380×520 / D 460×630 の右寄せ) | 同左 | 同左 | clip-path 不使用 (オーバーレイ全身表示) |
...
| 2 | dialogue  | `OP_BG02.webp` (**博士の家の屋内**、is-static-blur: scale 1.08 + blur 5px) | **入室演出 (2026-05-08 追加)**: 最初の 2 秒は **BG を sharp で表示** (blur 無し / キャラ・Box 非表示) → 2 秒経過時に `is-static-blur` 追加 (filter フェード 0.6s) + sides / dialogueBox を opacity 0→1 で 0.5s フェードイン → 600ms マージン後に typing 開始。Pono panel 既定 = `dance_hi.webp`、博士 = `owl_professor_guide.webp`。 **3 行構成 (sw v921+ で kurumi 追加)**: ① 博士「ほっほっほ…ポノか。よくきたのう」 (pono `pono_001.webp` 聞き役) → ② **★くるみ「こんにちは、ポノさん！」** (pono `pono_001.webp`、 kurumi `kurumi_001.webp` フェードイン) → ③ ポノ「はかせ、くるみちゃん、あそびに きたよ！」 (pono `dance_wave.webp`)。 **「博士」漢字は使わずひらがな「はかせ」に統一** (sw v922+ クロスレビュー反映) |
...
| 5 | dialogue  | 同上 | Pono panel 既定 = `dance_smile.webp`。 **4 行構成 (sw v921+ で kurumi 追加 + sw v922+ で「クイズ」→「なぞなぞ」語彙統一)**: ① 博士「ほっほっほ、それでよい。まちがえても よいのじゃ。**かんがえることが たいせつじゃからの。**」 (pono `pono_001.webp`) → ② ポノ「うん！」 (pono `dance_smile.webp`) → ③ **★博士「くるみよ、いつもどおり なぞなぞを おねがいするぞ」** (pono `pono_001.webp`) → ④ **★くるみ「はーい、まかせて！」** (pono `pono_001.webp`、 kurumi `kurumi_001.webp` フェードイン) |
| 6 | dialogue  | 同上 | Pono panel 既定 = `think_chin_clasp.webp`、**hakase line / pono line とも `think_chin_clasp.webp`** (両者真剣な構え)、台詞「では、いくぞ…」「うん！」 (2 行)。 **両 line に `kurumiImg: kurumi_001.webp` を注入** することで くるみ立ち絵を visible 維持し、 喋らないままクイズ本編に遷移 (sw v922+ 「Panel 6 でくるみ消失」 クロスレビュー HIGH 修正、 表示判定は `speaker === 'kurumi' || d.kurumiImg` の OR 条件) |
...
## くるみちゃん組み込み (sw v921+ / クロスレビュー反映 v922+)

OP に **アシスタント役のリスのくるみちゃん** を 3 番目のキャラとして追加。 キャラ全般のスペック・性格・VOICEVOX 方針は [feature_quizland_kurumi.md](feature_quizland_kurumi.md) に集約、 ここでは **OP cinematic 側の組み込み詳細** をまとめる。

### 組み込み箇所まとめ

- **dialogue データ** (`quizland/index.html` ~L4869-L4911 = `OP_PANELS` Panel 2 / 5 / 6):
  - Panel 2: 3 行 (博士 → くるみ → ポノ)、 くるみ line に `speaker: 'kurumi'` + `kurumiImg`
  - Panel 5: 4 行 (博士 → ポノ → 博士 → くるみ)、 ④ くるみ line に `speaker: 'kurumi'` + `kurumiImg`
  - Panel 6: 2 行 (博士 → ポノ)、 **両 line に `kurumiImg` を注入** して立ち絵を visible 維持 (発話無し)
...
- **saved-layout.json**: `__op_layout.{B,C,D}.kurumi` に slot + perVariant.kurumi_001 を保持。 初期値 B 280×380 / C 380×520 / D 460×630 (aspect 0.73)、 op-layout-editor で位置調整可能

- **アセット**: `assets/images/characters/kurumi/dance/kurumi_001.webp` (439×602 RGBA, 47KB, 透過維持) — 正面立ち姿 1 variant
...
- **アセット (WebP 化済み、PNG はフォールバックで残置)**:
  ...
  - `assets/images/characters/kurumi/dance/kurumi_001.webp` (439×602, 47KB, 透過維持) — くるみ正面立ち姿
  - `OP_NA01.mp3` / `OP_NA02.mp3` — ナレーション per-seg
- **`quizland/saved-layout.json`**: `__op_layout.{B,C,D}.kurumi` に slot + perVariant.kurumi_001 を保持 (B 280×380 / C 380×520 / D 460×630、 aspect 0.73、 右寄せ初期値)
...
`kurumi` は sw v921+ で追加された 5 つ目のオブジェクト。 `slot` (left/top/width/height/aspectLockRatio) と `perVariant.kurumi_001` (現状 1 variant のみ) を持つ。 _opApplyLayoutOverride は v921+ で `applySide('kurumi', layout.kurumi, kurumiVariant)` を追加して inline style 注入経路を拡張済 (~L5486-L5489)。
