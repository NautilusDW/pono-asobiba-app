...
| 2 | dialogue  | `OP_BG02.webp` (**博士の家の屋内**、is-static-blur: scale 1.08 + blur 5px) | **入室演出 (2026-05-08 追加)**: 最初の 2 秒は **BG を sharp で表示** (blur 無し / キャラ・Box 非表示) → 2 秒経過時に `is-static-blur` 追加 (filter フェード 0.6s) + sides / dialogueBox を opacity 0→1 で 0.5s フェードイン → 600ms マージン後に typing 開始。Pono panel 既定 = `dance_hi.webp`、博士 = `owl_professor_guide.webp`。 **3 行構成 (sw v921+ で kurumi 追加 / v923+ で kurumi variant `kurumi_hi.webp` 確定 / v938+ で line 3 にも `kurumiImg` 追加注入)**: ① 博士「ほっほっほ…ポノか。よくきたのう」 (pono `pono_001.webp` 聞き役、 kurumi 登場前のため kurumiImg なし) → ② くるみ「こんにちは、ポノさん！」 (pono `pono_001.webp`、 **kurumi `kurumi_hi.webp` ← 左手大きく挨拶 variant** フェードイン、 既存・初登場) → ③ ポノ「はかせ、くるみちゃん、あそびに きたよ！」 (pono `dance_wave.webp`、 **`kurumiImg: kurumi_hi.webp` を v938 で追加注入** して line 2 → 3 の遷移で kurumi 立ち絵が一瞬消えないように継続表示)。 **「博士」漢字は使わずひらがな「はかせ」に統一** (sw v922+ クロスレビュー反映) |
...
| 5 | dialogue  | 同上 | Pono panel 既定 = `dance_smile.webp`。 **4 行構成 (sw v921+ で kurumi 追加 + sw v922+ で「クイズ」→「なぞなぞ」語彙統一 + sw v923+ で kurumi variant `kurumi_wink.webp` 確定 + sw v938+ で line 1/2/3 にも `kurumiImg: kurumi_clasp.webp` 追加注入)**: ① 博士「ほっほっほ、それでよい。まちがえても よいのじゃ。**かんがえることが たいせつじゃからの。**」 (pono `pono_001.webp`、 **`kurumiImg: kurumi_clasp.webp` を v938 で追加注入** して待機ポーズで継続表示) → ② ポノ「うん！」 (pono `dance_smile.webp`、 **`kurumiImg: kurumi_clasp.webp` を v938 で追加注入**) → ③ 博士「くるみよ、いつもどおり なぞなぞを おねがいするぞ」 (pono `pono_001.webp`、 **`kurumiImg: kurumi_clasp.webp` を v938 で追加注入** して依頼を聞いている表情) → ④ くるみ「はーい、まかせて！」 (pono `pono_001.webp`、 **kurumi `kurumi_wink.webp` ← ウインク + こぶし variant** に切替、 既存)。 line 1-3 → 4 では `kurumi_clasp` → `kurumi_wink` のポーズ切替のみが起こる滑らかな演出 (v937 までは line 4 で kurumi が突然 fade in していた) |
...
- **dialogue データ** (`quizland/index.html` ~L4869-L4911 = `OP_PANELS` Panel 2 / 3 / 4 / 5 / 6 全 5 panel で kurumi visible、 sw v937+ で Panel 3/4 にも kurumiImg 注入、 **sw v938+ で Panel 2 line 3 + Panel 5 line 1/2/3 にも kurumiImg 追加注入で全 12 line カバレッジ完了**):
  - Panel 2: 3 行 (博士 → くるみ → ポノ)、 ② くるみ line に `speaker: 'kurumi'` + **`kurumiImg: 'kurumi_hi.webp'`** (左手大きく挨拶、 既存)、 **③ ポノ line にも `kurumiImg: 'kurumi_hi.webp'` を v938 で追加注入** (line 2 の挨拶ポーズを line 3 でも維持)
  ...
  - Panel 5 (sw v938+ で line 1/2/3 にも追加注入): 4 行 (博士 → ポノ → 博士 → くるみ)、 **① 博士 line + ② ポノ line + ③ 博士 line に `kurumiImg: 'kurumi_clasp.webp'` を v938 で追加注入** して line 1-3 の間も待機ポーズで継続表示、 ④ くるみ line に `speaker: 'kurumi'` + **`kurumiImg: 'kurumi_wink.webp'`** (ウインク + こぶし、 既存) で line 3 → 4 のポーズ切替のみ
  - Panel 6: 2 行 (博士 → ポノ)、 **両 line に `kurumiImg: 'kurumi_clasp.webp'` (両手胸前で組む) を注入** して立ち絵を visible 維持 (発話無し、 待機ポーズ、 既存)
  - **「kurumi は Panel 2 で登場したら Panel 6 まで継続表示」 がプロジェクト方針** (sw v937 で明文化、 v938 で全 12 line × kurumiImg 完全カバレッジで物理実装)。 Panel 3/4 で消すような提案や kurumi 登場以降の line で kurumiImg を省くような提案は今後しない。 ポーズ (variant) はシナリオに合わせて変化させてよい
...
くるみが「喋らないが画面に立っている」 line (= Panel 2 line 3 / Panel 3 全 line / Panel 4 全 line / Panel 5 line 1-3 / Panel 6 全 line) を作るため、 立ち絵 visible 判定は **`speaker === 'kurumi' || d.kurumiImg` の OR 条件** にしてある。 `kurumiImg` を line に注入するだけで、 喋らせずに立ち絵だけ維持できる。 sw v937+ で Panel 3 / 4 にも kurumiImg 注入、 sw v938+ で Panel 2 line 3 + Panel 5 line 1/2/3 にも追加注入した結果、 **Panel 2 line 2 (kurumi 初登場) 〜 Panel 6 末尾までの全 12 line で kurumiImg がカバーされ完全継続表示** が成立 (旧 v921-v936 は Panel 3-4 全体で消失、 v937 はまだ Panel 2 line 3 / Panel 5 line 1-3 で消失していた残課題を v938 で解消):

- Panel 2 line 2 で くるみ初登場 (visible、 `kurumi_hi`)
- **Panel 2 line 3 で くるみ立ち絵維持 (sw v938+、 `kurumiImg: kurumi_hi` 注入で挨拶ポーズ継続)**
- **Panel 3 全 line で くるみ立ち絵維持 (sw v937+、 両 line に `kurumiImg: kurumi_clasp` 注入で待機ポーズ)**
- **Panel 4 全 line で くるみ立ち絵維持 (sw v937+、 両 line に `kurumiImg: kurumi_clasp` 注入で待機ポーズ)**
- **Panel 5 line 1-3 で くるみ立ち絵維持 (sw v938+、 各 line に `kurumiImg: kurumi_clasp` 注入で待機ポーズ)**
- Panel 5 line 4 で くるみ発話 (visible 継続、 `kurumi_wink` にポーズ切替)
- Panel 6 全 line で 発話無しのまま立ち絵維持 (両 line に `kurumiImg: kurumi_clasp` 注入)
- → Panel 2 line 2 で登場したら Panel 6 末尾まで一度も消えず、 クイズ本編開始時にも くるみが画面に立っていて 「読み手はこの子」 という導入が完結

**プロジェクト方針 (v937 で確定 / v938 で完全実装)**: 「くるみは Panel 2 で登場したら Panel 6 まで継続表示」。 「Panel 3/4 で消す」 「特定 panel/line で kurumi を消す」 のような提案は今後しない。 ポーズ変化はシナリオに合わせて自由 (v937/v938 では Panel 3/4 + Panel 5 line 1-3 + Panel 6 全 line を `kurumi_clasp` 待機ポーズに統一、 将来 `book` で本を読む / `idea` でひらめきポーズ等に変えてもよい)。 **「kurumi 登場以降の line には必ず kurumiImg を注入する」 設計原則 (v938 で確定)** — 新しい Panel/line 追加時もこの原則に従わないと line 切替時に立ち絵が一瞬消える。 ユーザー指示原文「くるみはその場所にずっと出てていい。一度最初に出たらずっとそこにいていい。シナリオでポーズも変化させたりはしている」
...
### saved-layout.json 連携 (v923 スキーマ拡張 → v932 B kurumi 実値 publish → v933 runtime CSS アンカー統一 → v934 同値再 export の無変更 publish → v935 C/D 側にも hakase + kurumi 実値 publish 開始 → v936 「全ポーズに反映」 ボタンの初実用 publish (3 VC × 12-13 variants 一括同期、 70 entries diff) → v937 Panel 3/4 継続表示 仕様変更 + B kurumi base box 微調整 (3 entries diff) → **v938 (現行) Panel 2 line 3 + Panel 5 line 1/2/3 にも kurumiImg 追加注入で全 12 line カバレッジ完了、 saved-layout.json は無変更 (= シナリオ JS のみの修正)**)
...
1. **`SCENARIO_DATA_VERSION` 定数** (~L1250): 現行値 **`'v938'`** (v931 で `'v927'` → `'v930'`、 v937 で `'v930'` → `'v937'` に bump、 v938 で `'v937'` → `'v938'` に bump、 v937 では Panel 3 / 4 各 line に `kurumiImg: kurumi_clasp` を注入する defaults 構造変更、 v938 では Panel 2 line 3 + Panel 5 line 1/2/3 にも `kurumiImg` を追加注入する defaults 構造変更のためそれぞれ bump)。 `defaultScenario()` の構造または line に入る可能性のあるフィールド構成が変わったら bump する単一の真実
