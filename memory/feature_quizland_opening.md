...
### saved-layout.json 連携 (v923 スキーマ拡張 → v932 B kurumi 実値 publish → v933 runtime CSS アンカー統一 → v934 同値再 export の無変更 publish → v935 C/D 側にも hakase + kurumi 実値 publish 開始 → **v936 「全ポーズに反映」 ボタンの初実用 publish (3 VC × 12-13 variants 一括同期、 70 entries diff)**)
...
- editor 側のコード変更は v931 で大半完了、 「全ポーズに反映」 ボタンは v936 直前 task で追加 (v932 / v934 / v935 は editor 完成済 Export 経路をそのまま使った publish イベント、 v933 は runtime quizland/index.html の per-VC `.op-char-slot` CSS 3 箇所のみ修正、 v936 は editor の「全ポーズに反映」 ボタンが publish 経路と組み合わさった初実用)
