...
### saved-layout.json 連携 (v923 でスキーマ拡張 → v932 で実値 publish 成立 → v933 で runtime CSS アンカー統一によりその値が editor で見たまま反映)
...
- v932-v933 時点の実値: **B = 13 variants 全部 editor で個別調整済 (slotH 278〜380, slotOffsetX 54〜88、 `kurumi_pray` のみ slotOffsetY=-4) / C = 13 variants 全部 `slotW=280 / slotH=380 / slotOffsetX=0` の初期値温存 / D = `kurumi_001` のみ `slotW=550 / slotH=489 / slotOffsetX=654` で大きく上書き、 他 12 は初期値温存**。 全 variant `slotAspect='fixed_0.73'` / `objectPosition='bottom'` で統一
- 配信フロー (v932 で正式運用化、 v933 で runtime 反映品質も担保): ローカル editor の **「対話 (P2-6)」モード** で 「くるみ側」 タブ または **左ペイン Kurumi サムネ一覧** から variant 選択 → slot ドラッグ調整 → 「📋 JSON のみクリップボード」 → AI チャット貼付け → orchestrator (Claude) が saved-layout.json `__op_layout` のみ merge (他 219 keys 完全温存) + commit + auto push → develop → staging で全端末反映 (v933+ では editor で見たままの位置で再現される) → ユーザー明示時のみ master merge で production
- editor 側のコード変更は v931 で完了 (v932 は editor を一切触らない publish イベント、 v933 は editor も一切触らず runtime quizland/index.html の per-VC `.op-char-slot` CSS 3 箇所のみ修正の最小局所変更)
