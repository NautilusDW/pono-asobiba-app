...
| **`SCENARIO_DATA_VERSION` 定数 (v929 追加、 v931 で bump、 ~L1250)** | **`'v930'` (v931 時点)**。 `defaultScenario()` の構造を変えたら bump する単一の真実。 v931 では defaults 構造自体は未変更だが、 line に hakaseImg / kurumiImg 等の新フィールドが入る余地が広がったため auto migration を発火させる目的で bump |
...
### saved-layout.json 連携 (v923 でスキーマ拡張 → v932 で B の kurumi 実値 publish 成立 → v933 で runtime CSS アンカー統一でその値が editor で見たまま反映 → v934 は同値再 export の無変更 publish → v935 で C/D 側にも hakase + kurumi の実値 publish 開始)

- `__op_layout.{B,C,D}.kurumi.perVariant` は **13 entries** で配信中 (v923 で 1 entry → 13 entries にスキーマ拡張)
- v932 で **B のみ** 13 entries が初期値 (`kurumi_001` 値全コピー) から editor 編集後の個別調整値に更新済: 元データ `D:\ポノのおへや\Dr.owl'quiz\op-layout-2026-05-11-06-18-47.json` (ローカル editor で位置調整 → 「📋 JSON のみクリップボード」 で export) を orchestrator が merge。 `quizland/saved-layout.json` の top-level 220 keys 完全温存 (`q72`, `q83`, `__chip_text_overrides` 等の per-question overrides + `__op_narration` 等は touch せず)
...
- **v935 (現行、 06-49-00 Export を merge) で初めて C / D 側にも実値が流し込まれた** (kurumi flat + perVariant + hakase 含めて 12 entries diff): **B** = `kurumi_clasp` の slotH 354→320 / slotOffsetY 0→19 (両手胸前の立ち姿を少し低く詰める)、 flat の `B.kurumi.slotH` / `slotOffsetY` も同期。 **C** = `hakase.slotOffsetX` 0→-27 (はかせ少し左寄せ) + `kurumi.slotOffsetX` 0→158 (くるみ右寄せ) + `kurumi_001` perVariant も同期。 **D** = `hakase.slotOffsetX` -362→-142 (はかせ大幅右移動) + `kurumi.slotH` 489→413 / `slotOffsetX` 654→158 (くるみを 「画面右端の大サイズ」 から 「中央寄りの中サイズ」 に再配置) + `kurumi_001` perVariant も同期
- v935 時点の実値スナップショット: **B = 13 variants 全部 editor で個別調整済 (slotH 278〜380, slotOffsetX 54〜88、 `kurumi_clasp` のみ slotOffsetY=19 / `kurumi_pray` のみ slotOffsetY=-4) / C = `kurumi_001` のみ `slotOffsetX=158`、 他 12 variants は `slotW=280 / slotH=380 / slotOffsetX=0` の初期値温存、 hakase は `slotOffsetX=-27` 実値化 / D = `kurumi_001` を `slotW=550 / slotH=413 / slotOffsetX=158` で上書き、 他 12 variants は初期値温存、 hakase は `slotOffsetX=-142` 実値化**。 全 variant `slotAspect='fixed_0.73'` / `objectPosition='bottom'` で統一
- 配信フロー (v932 で正式運用化、 v933 で runtime 反映品質も担保、 v935 で C/D も含めた本格運用に): ローカル editor の **「対話 (P2-6)」モード** で 「くるみ側」 タブ または **左ペイン Kurumi サムネ一覧** から variant 選択 → slot ドラッグ調整 → 「📋 JSON のみクリップボード」 → AI チャット貼付け → orchestrator (Claude) が saved-layout.json `__op_layout` のみ merge (他 219 keys 完全温存) + commit + auto push → develop → staging で全端末反映 (v933+ では editor で見たままの位置で再現される) → ユーザー明示時のみ master merge で production
- editor 側のコード変更は v931 で完了 (v932 / v934 / v935 は editor を一切触らない publish イベント、 v933 は editor も一切触らず runtime quizland/index.html の per-VC `.op-char-slot` CSS 3 箇所のみ修正の最小局所変更)
