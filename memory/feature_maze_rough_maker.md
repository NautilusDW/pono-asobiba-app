...
| 難易度 | 道幅 | パネル | キャンバス | グリッド (始→終) | 実効セル数 (始→終) | お邪魔虫 |
|-------|-----|-------|------------|----------------|---------------------|---------|
| 1〜49 | 2タイル(120px) | 1 | 1920×1080 | 4×3 → 8×4 | 6 → 31 | 4 → 12 |
| 50〜74 | 1タイル(60px) | 1 | 1920×1080 | 10×6 → 16×9 | 28 → 139 | 12 → 16 |
| 75〜100 | 1タイル(60px) | 2 | 3840×1080 | 18×9 → 32×9 | 140 → 288 | 16 → 20 |

**カーブ設計:** 各層で `cw / ch` が連続的に増え、`targetCells` (実効セル数) が滑らかに 6 → 288 まで上昇。層境界 (d=49→50, d=74→75) は実効セル数を一致させ、変化点は「道幅」「パネル数」だけにする (難易度ジャンプを避ける)。`deadEndPrune` は `totalCells - targetCells` で自動算出 (枝を間引いて目標まで縮める)。
