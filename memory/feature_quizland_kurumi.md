...
4. 13 variants 分繰り返し（共通の slot で済むなら 1 回でも OK、 ポーズによってはみ出す場合は variant ごとに微調整）
5. 「📋 JSON のみクリップボード」 で `pono-op-layout-v1` schema をコピー → AI チャットに貼付け
6. orchestrator が `saved-layout.json` の `__op_layout.{B,C,D}.kurumi.perVariant.{variant}` をマージ + commit + auto push
7. develop → staging → 全端末反映

### シナリオ speaker 編集 (v925 追加)

シナリオモード (各 panel の dialogue 行を編集する画面) で、 各 line の speaker ラジオに **「くるみ」が選択肢として追加**:
...
- 立ち絵切替は別途 `kurumiImg` フィールドで指定 (現状 UI からの指定は未実装、 直接 OP_PANELS を編集するか scenario JSON Export → 手動編集で対応)
...
- **v925 (現行)**: editor 左ペインに **Kurumi バリエーションサムネ一覧 (13 ポーズ、 makeKurumiThumb)** + **シナリオ行 speaker に「くるみ」追加 (ポノ / はかせ / くるみ の 3 way ラジオ)**。 kurumi user-added 画像のドラッグ&ドロップは未対応 (info 文で明示)。 editor の「くるみ側」タブは「対話 (P2-6)」モードでのみ表示される仕様 (CSS line 661/667 で `is-narration` / `is-scenario` 時に tab-bar 全体が `display: none`) を memory 文書に明文化
