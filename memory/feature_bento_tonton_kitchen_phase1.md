...
`bento/kitchen.html`（自己完結 HTML + inline CSS/JS、IIFE 構成、約 3198 行、sw v423 時点）。 ユーザーが既存 `bento/index.html` を開く前にここを通る想定。
...
- editor 対象 (`editableSelectors`、**v410 で 7 要素**): `.app-header`, `.knife`, `.chopY-marker`（[ツール] 包丁の最下点）, `.ingredient-bbox-marker`（[ツール] 食材の位置とサイズ）, **`.startX-marker`（[ツール] 包丁の切り始め X、v410 新規）**, **`.endX-marker`（[ツール] 包丁の切り終わり X、v410 新規）**, `.bowl`
- per-ingredient 対象 (`perQuestionSelectors`、**v410 で 5 要素**): `.knife`, `.chopY-marker`, `.ingredient-bbox-marker`, `.startX-marker`, `.endX-marker`（食材切替時に座標も切替）
...
**applyIngredientPlacement とのレイヤリング** (loadIngredient 内の順序、v408 で seedIngredientBboxFromOnBoard 追加):
1. `applyIngredientPlacement(ing)`: chopMechanics の `ingredientLeft/Top/Width_pct`（コード値）を inline 反映
2. **`seedIngredientBboxFromOnBoard()`**（v408 新規）: `.ingredient-on-board` の現在位置を読んで `.ingredient-bbox-marker` の inline style を同期 → 緑矩形がにんじん本体と同じ位置に表示
3. `syncMechanicsFromMarkers(ing)` → `syncIngredientBboxFromMarker(ing)`:
   - marker 未編集 → no-op で 1. の値が効く（Claude がコード値調整する経路を温存）
   - marker 編集済み → marker の `getBoundingClientRect` から inline を再上書き（editor 値が勝つ）
4. requestAnimationFrame × 2 後に **`seedIngredientBboxFromOnBoard()` 再走** + `syncMechanicsFromMarkers` 再走（img onload 後の精度確保）

これで「コード値で設定」「editor で WYSIWYG 調整」の両経路が共存可能。
...
#### [chop アニメの 3 フェーズ直角リニアモーション (v419)]

```
t=0ms     ▼ knife-inner.transform = translate(0, chopY%)  ← 真下
t=100ms   ▲ knife-inner.transform = translate(0, liftY%)  ← 真上
t=200ms   ◀ --knife-x = nextX% (次タップ位置)              ← 左へ
t=300ms     完了、次タップ待ち
```

CSS transition は全て `0.1s linear`（旧 `0.18s ease-out` のベジェ風挙動を廃止）。最終タップは Phase 2/3 skip で最下点静止。ユーザー直感「真下→真上→左の直角サイクル」を実装。
...
- editor と通常プレイで .knife 位置が食い違うバグ修正 (`.knife` に `data-le-keep-position="1"` 付与 + `body.layout-editor-on .knife { position: absolute !important; }`、v405 の chopY-marker と同型バグ): **v423（現行）**
