...
`bento/kitchen.html`（自己完結 HTML + inline CSS/JS、IIFE 構成、約 2730 行、sw v403 時点）。 ユーザーが既存 `bento/index.html` を開く前にここを通る想定。
...
- saved-layout.json から chopY-marker の `tx:0, ty:0` エントリを削除 (CSS の translate(-50%,-50%) 上書きで画面外に消えてた): **v403（現行）**
