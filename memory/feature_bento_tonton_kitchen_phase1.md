...
`bento/kitchen.html`（自己完結 HTML + inline CSS/JS、IIFE 構成、約 3165 行、sw v419 時点）。 ユーザーが既存 `bento/index.html` を開く前にここを通る想定。
...
- chop アニメを直角リニアモーション化 (knife wrapper + inner 2 層化、3 フェーズ「下→上→左」分割、editor 位置と chop アニメを完全分離): **v419（現行）**
