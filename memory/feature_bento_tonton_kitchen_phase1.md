...
`bento/kitchen.html`（自己完結 HTML + inline CSS/JS、IIFE 構成、約 2656 行、sw v394 時点）。 ユーザーが既存 `bento/index.html` を開く前にここを通る想定。
...
- editor 対象 (`editableSelectors`、v394 で 4 要素に縮減): `.app-header`, `.knife`, `.chopY-marker`（ラベル「[ツール] 包丁の最下点」）, `.bowl`
- per-ingredient 対象 (`perQuestionSelectors`、v394 で 2 要素に縮減): `.knife`, `.chopY-marker`（食材切替時に座標も切替）
- **`.ingredient-on-board` と `.cutting-board` は editor 対象から外した（v394）**: CSS の `width:74%; aspect-ratio` 依存で、editor の px 固定化と相容れない（極小化バグの根本対処）。にんじん本体の位置/サイズは INGREDIENTS の `chopMechanics`（startX_pct/endX_pct/bladeTipOffsetX_pct）でコード制御する設計
- **`.cutting-board` を外した理由（v394）**: `.knife` / `.chopY-marker` / `.ingredient-on-board` の親なので、editor で `.cutting-board` を drag すると `parent.contains(child)` チェックで子要素もグループ追従してしまうバグの根本対処。まな板自体の位置/サイズは CSS 固定で背景画像と整合済み
...
**Why editor 対象を縮減（v394）**: `.ingredient-on-board` / `.cutting-board` を editor 対象に含めるとそれぞれ別の致命的バグが出た。前者は CSS % が px 固定化されてにんじんが極小化（14×163px）、後者は親子追従でドラッグ時に内部要素全部がついてくる。editor を `.knife` / `.chopY-marker` の 2 要素 + `.app-header` / `.bowl` の合計 4 要素に絞り、それ以外は CSS と chopMechanics（コード）で制御する設計に変更。
...
- editor 対象縮減 (にんじん極小化 + 親子追従バグ解消、editor 4 要素のみに): **v394（現行）**
