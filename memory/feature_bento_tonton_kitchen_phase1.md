...
`bento/kitchen.html`（自己完結 HTML + inline CSS/JS、IIFE 構成、約 2779 行、sw v398 時点）。 ユーザーが既存 `bento/index.html` を開く前にここを通る想定。
...
- editor 対象 (`editableSelectors`、v398 で 7 要素に拡張): `.app-header`, `.knife`, **`.ingredient-bbox-marker`（ラベル「[ツール] 食材の位置とサイズ」）**, **`.startX-marker`（[ツール] 包丁の開始 X）**, **`.endX-marker`（[ツール] 包丁の終了 X）**, `.chopY-marker`（[ツール] 包丁の最下点 Y）, `.bowl`
- per-ingredient 対象 (`perQuestionSelectors`、v398 で 5 要素): `.knife`, `.ingredient-bbox-marker`, `.startX-marker`, `.endX-marker`, `.chopY-marker`（食材切替時に座標も切替）
- **`.ingredient-on-board` と `.cutting-board` は editor 対象に直接含めない（v394 で外し、v398 でも維持）**: CSS の `width:74%; aspect-ratio` 依存で editor の px 固定化と相容れない（極小化バグ）+ 親要素 drag で子要素グループ追従バグ。代わりに `.ingredient-bbox-marker`（dummy DOM）を経由して位置/サイズを編集する設計
...
**Why editor 対象を縮減（v394）→ マーカー 4 種に再拡張（v398）**: v394 で `.ingredient-on-board` / `.cutting-board` を editor 対象に含めた時にバグが出たのは「**editor の px 固定化哲学**」と「**% + aspect-ratio で組まれた相対レイアウト**」が構造的に衝突するため。一旦 editor を 4 要素に縮減したが、ユーザーから「全食材を editor で動かしたい」「包丁の縦/横移動位置も editor で設定したい」要望を受けて v398 で **4 マーカー（ingredient-bbox + startX + endX + chopY）パターン** に拡張。**「editor 対象 = 常に位置だけ持つマーカー、サイズも marker 自身で持つ。実体 `.ingredient-on-board` は editor から外したまま」** という責務分離を徹底することで、両立を実現。

#### chop メカニクスエディタ用 4 マーカー（v398）

| マーカー | 形・色 | 編集対象 | 反映先 | スコープ |
|---|---|---|---|---|
| `.ingredient-bbox-marker` | 緑 #4CAF50 dashed rect | 位置 + サイズ | `.ingredient-on-board` の left/top/width（% 指定で inline 上書き） | per-ingredient |
| `.startX-marker` | 青 #2196F3 縦線 8×80px | X 位置のみ | `chopMechanics.startX_pct` | per-ingredient |
| `.endX-marker` | 赤 #E53935 縦線 8×80px | X 位置のみ | `chopMechanics.endX_pct` | per-ingredient |
| `.chopY-marker` (既存) | 橙 #D86E2F dashed 円 28×28 | Y 位置のみ | `chopMechanics.chopY_pct` | per-ingredient |

実装:
- 全マーカーは `body.layout-editor-on` 時のみ `display: block`、通常時は完全非表示
- 集約関数 `syncMechanicsFromMarkers(ing)` で 4 sync を一括実行、`loadIngredient` 末尾 + `requestAnimationFrame` 2 回で img onload 後の bbox 再計算を担保
- 各 sync は「**editor 由来 transform / width が無ければ no-op**」のガードで、通常プレイ時に CSS デフォルトを破壊しない設計
- editor 由来 transform 検出: `/translate\([^,]+,/.test(transform) && transform.indexOf('-50%') === -1`
- bbox の width 編集は `!!style.width` の OR 条件で検出

**残課題**: editor でマーカーを動かしても即時 preview されない（`loadIngredient` 再走でしか反映されない）。drag end イベント購読は Phase 2 で対応。img の disk cache miss 時、bbox 計算が 2 frame では足りない可能性 → `img.addEventListener('load', ...)` で 3 度目の sync 追加余地あり。
...
- editor マーカー 4 種 (ingredient-bbox/startX/endX/chopY) で食材位置+包丁chop範囲を WYSIWYG 化: **v398（現行）**
