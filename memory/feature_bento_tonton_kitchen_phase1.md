...
`bento/kitchen.html`（自己完結 HTML + inline CSS/JS、IIFE 構成、約 2800 行、sw v406 時点）。 ユーザーが既存 `bento/index.html` を開く前にここを通る想定。
...
**applyIngredientPlacement とのレイヤリング** (loadIngredient 内の順序):
1. `applyIngredientPlacement(ing)`: chopMechanics の `ingredientLeft/Top/Width_pct`（コード値）を inline 反映
2. `syncMechanicsFromMarkers(ing)` → `syncIngredientBboxFromMarker(ing)`:
...
- 食材の位置・サイズ editor 化: .ingredient-bbox-marker (緑 dashed rect) を v405 知見ベースで再導入 (data-le-keep-position 必須、 applyIngredientPlacement とのレイヤリング): **v406（現行）**
