...
`bento/kitchen.html`（自己完結 HTML + inline CSS/JS、IIFE 構成、約 3250 行、sw v426 時点）。 ユーザーが既存 `bento/index.html` を開く前にここを通る想定。
