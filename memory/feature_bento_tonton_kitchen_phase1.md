...
- v764 デプロイ後のユーザー報告 6 件 (塩シェイク機構刷新 + フライパン蓋 salmon 対応 + flip 軸変更 等) への一括対応:
  ...
  6. **フライパンの蓋を salmon にも追加** (egg 専用設計を拡張、 既存 egg 制御は完全温存): ユーザー繰り返し「フライパンの蓋どこ？ また勝手に削除した？」 と訴えるが、 git 履歴で `.egg-glass-lid` (HTML `<div id="egg-glass-lid">` + CSS L3038- + JS `onEggLidPointerDown` 等) は **削除されていない** ことを確認 (誤認)。 ただし salmon でも蓋を期待されているため、 **`.egg-glass-lid.show-for-salmon` 新クラス** で salmon でも蓋を auto-show: `cookFrontSubStep`/`cookBackSubStep` 冒頭で `state.current && state.current.id === 'salmon'` ガード付きで `add`、 `runFlipSubStep`/`runDoneSubStep` 冒頭 + `resetCookSubStepUI` で `remove`。 CSS は `.show-for-salmon { display: block; opacity: 0.92; left: var(--grill-food-x, 58%); top: var(--grill-food-y, 52%); width: 32%; z-index: 6 }` で独立定義。 egg 用の `.egg-lid-ready` / `.egg-lid-closed` クラス系統には一切触らない (egg ステージ起動時に `show-for-salmon` 残留は `resetCookSubStepUI` で事前除去、 副作用ゼロ)。 「ユーザー要望と現実料理ロジックが乖離する場合、 ユーザー要望優先で別系統クラスで拡張する」 知見を記録。
