...
`bento/kitchen.html`（自己完結 HTML + inline CSS/JS、IIFE 構成、約 3119 行、sw v417 時点）。 ユーザーが既存 `bento/index.html` を開く前にここを通る想定。
...
- 包丁 Y 移動が動かない真因 (CSS var-in-transform で transition 不発) を style.transform 直接代入で解消: **v417（現行）**
