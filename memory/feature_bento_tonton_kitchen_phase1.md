...
`bento/kitchen.html`（自己完結 HTML + inline CSS/JS、IIFE 構成、約 2867 行、sw v408 時点）。 ユーザーが既存 `bento/index.html` を開く前にここを通る想定。
...
- editor 対象 (`editableSelectors`、**v406 で 5 要素**): `.app-header`, `.knife`, `.chopY-marker`（[ツール] 包丁の最下点）, **`.ingredient-bbox-marker`（[ツール] 食材の位置とサイズ、v406 で再導入）**, `.bowl`
- per-ingredient 対象 (`perQuestionSelectors`、**v406 で 3 要素**): `.knife`, `.chopY-marker`, `.ingredient-bbox-marker`（食材切替時に座標も切替）
...
#### chop メカニクスエディタ用 マーカー（v406 で 2 種に）
...
- bbox-marker 初期位置ずれ修正 (seedIngredientBboxFromOnBoard) + MutationObserver でリアルタイム drag/resize 同期 (bbox + chopY 両方): **v408（現行）**
