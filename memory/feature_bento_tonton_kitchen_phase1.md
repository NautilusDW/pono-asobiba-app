...
- v762 デプロイ後のユーザー報告 6 件 (タイミングゲージ復活 + 5 仕様 fix) への一括対応:
  0. **【最優先・指示違反の撤回】 cookFront/cookBack で X/N タップカウンター (v761) を完全撤回 → 既存タイミングゲージに戻す**: ユーザー強い不満「ゲージがなくなってるからひっくり返せない」「タップを3回するようにしたの？ そんな指示は出してない」「今まで見た草下にゲージを作って、 ちょうどいいところで押すのに戻して」。 v761 では subStep モードで `stopCookTimingGauge()` を呼ぶ仕様を踏まえて X/N バナーで代替したが、 ユーザーの真意は **既存の Timing-At-Right-Spot ゲージ** をそのまま流用することだった。 修正: `cookFrontSubStep`/`cookBackSubStep` の中身を「`startCookTimingGauge('grill')` を呼ぶ + バナー『タイミングよく タップ！』 / 『うらも タイミングよく タップ！』」 に再実装。 `handleSubStepGrillTap` は `recordCookTiming('grill')` (内蔵 stopCookTimingGauge) + 420ms 後 `advanceSubStep` で flip → 1 タップで done = 計 2 タップ。 既存 `onGrillTap` の subSteps ガード経由でのみ呼ばれる設計のため、 egg / meatball / chicken / shrimp / korokke の従来 path 完全互換。 dead state (`cookFrontProgress`/`cookBackProgress`/`cookFrontMaxTaps`/`cookBackMaxTaps`) は次回整理で削除予定。
  1. **chop 散布クラスター解消の強化** (v761 0.42 では不十分): 少数 piece (2-3 個) では乱数で偶然中央集中する確率が高い。 修正: `bowlEl._placedPieces` で配置済み piece の座標を追跡し、 `addSingleCutPieceToBowl` の else 枝で **12 候補をランダム生成 → 既存 piece との最小距離 + ボウル境界マージン** をスコアにベスト位置を選択 (`score = minDist * 1.5 + edgeMargin * 0.5`)。 中心バイアスの `Math.sqrt(Math.random())` を `0.15 + 0.75 * Math.random()` の uniform 化で削除。 `clearBowl` で `_placedPieces.length = 0` リセット。 isLeafy / isParticle / isChicken の独自分岐は非干渉。
  2. **intro 演出簡略化**: 鮭が先に出て皿が左から slide-in する 2 段アニメをユーザーが「演出がおかしい」 と却下。 修正: CSS `salmonIntroFade` で **鮭と皿が同時に center で fade-in** する 1 段アニメに変更、 plate slide-in を撤去。
  3. **塩位置を鮭の上 → お皿の右側に**: ユーザー「塩が鮭の上に乗っかってるのもおかしい、 塩とテキストは右側 (お皿の右側) に移動して」。 修正: `showSaltSubStep` で塩シェイカー home を `left: 75%, top: 40%` (お皿右側上方) に変更、 `.cook-substep-salt-mode` クラスで `.cook-substep-banner { left: 75% }` 右寄せ。
  4. **塩 3 振り → 4 振り + OK 合図**: ユーザー「油に移動するのが早すぎる、 3-4 振りぐらいさせて、 塩 OK 合図を出してから油へ」。 修正: INGREDIENTS salmon の `subSteps[salt].taps` を `4` に変更、 `finishSaltSubStep` で `rest-lower-left` 追加直後に **「しお バッチリ！」 バナー** + `.done-check` クラスで **緑 ✓ チェックマーク** (`::after` で `background: #4CAF50` 円 + `doneCheckPop` 0.6s scale 0→1.2→1.0 アニメ) を 1100ms 表示後に advance。
  5. **油容器の登場経路 + 注ぎ口反転 + layer 不透明度**: ユーザー「中央じゃなくて右下に小さく現れて、 右側から左にドラッグするので注ぎ口を反転、 油の不透明度が目玉焼きみたいに濃すぎ」。 修正:
     - **中央経由スキップ**: `showOilSubStep` で `rest-lower-right` + `show` を直接付与 (中央 900ms 経由を廃止)
     - **注ぎ口水平反転**: `.cook-substep-oil.tilted` に `scaleX(-1)` 追加で右→左ドラッグ方向に注ぎ口が左下を向く
     - **油 layer 薄化**: `.cook-substep-oil-layer.show` の opacity `0.92 → 0.45` (目玉焼き感を消して薄い油膜に)、 brightness/saturate ブーストは維持
  キャッシュバスター同期: `cookingAssetPath` `?v=762 → ?v=763 → ?v=764` (2 段階)、 CSS hardcoded `oil_layer.png?v=764`、 sw.js v763 → v764。 `?v=761/762/763` 残存 grep 0 件: v763/v764 (5 fix を v763 で先行実装、 gauge 復活を v764 で上乗せ)
- v764 デプロイ後のユーザー報告 6 件 (塩シェイク機構刷新 + フライパン蓋 salmon 対応 + flip 軸変更 等) への一括対応:
  1. **塩シェイカーの傾き方向反転**: `.cook-substep-shaker.tilted` の rotate 符号を反転 (右傾き → 左傾き、 キャップが鮭側に倒れる方向)、 右手でシェイク動作する自然な姿勢と整合。
  2. **塩シェイク検出を「上下シェイク 1 回 = パラパラ 1 burst」 に全面刷新**: v763 で導入した 260ms throttle 連続発火は「ドバドバ」 感が強く、 ユーザー仕様 「1 回上下に振ったらパラパラ落ちて、 それ以上は出ない。 もう 1 回振ったらまた出る。 3 回繰り返す」 を満たさなかった。 修正: pointermove 内で **Y 軸速度符号変化検出** (`state.lastSaltDy * dy < 0` + ±8px 閾値) で「上下シェイク反転」 を 1 イベントとして捕捉、 **400ms cooldown** 中は次の振りを受け付けない、 鮭外に出たら `lastSaltDy = null` で振りカウンタ累積をリセット (毎回ゼロから)、 3 振り完了で `finishSaltSubStep`。 `taps: 4 → 3` に戻す (ユーザー「3 振りくらいさせて」 指示優先)。 `showSaltSubStep` 冒頭で `lastSaltY`/`lastSaltDy`/`lastSaltShakeTime` の 3 つを null/0 リセット (前 session の蓄積を排除)。 初回 dy 計算は `lastSaltY == null` ガード + sign check は `lastSaltDy != null` ガードの 2 段でクラッシュ防止。
  3. **油びんの一瞬逆回転バグ解消**: v763 で `.cook-substep-oil.tilted` に `scaleX(-1)` 追加した際、 base CSS に `scaleX(-1)` が無く、 drag 開始の瞬間や `.tilted` add/remove 切替で transform 差分が transition 経由でない一瞬ジャンプを起こしていた。 修正: base `.cook-substep-oil` と `.tilted:not(.pouring)` で `scaleX(-1)` を **一貫保持** (base からずっと反転)、 `transition: transform 0.22s ease-out` を base に追加してアニメ滑らか化、 `.tilted` add は **over-pan 検出 + pour 開始時のみ** に限定 (drag 開始の瞬間には付与されない)。
  4. **「ドラッグして フライパンへ」 の対象明示**: バナーテキストだけでは「何を」 ドラッグすべきか分からないユーザー報告。 修正: `.cook-substep-oil.hint-target::before` で **油びん周囲を点滅ダッシュリング** で囲み、 `.cook-substep-oil-arrow` 新規要素で **左向き ◀ 矢印が pan center に向かってスライド** するアニメ (`oilArrowSlide` 1.4s ループ + `oilSourcePulse` リング 1.4s ループ) を追加。 drag 開始 / `triggerOilPour` 成功 / `resetCookSubStepUI` の 3 経路で `hint-target` / arrow クラスを `remove`。 矢印は `::after { transform: scaleX(-1) }` で base flip + keyframe にも `scaleX(-1)` を含む暗黙依存あり (将来 keyframe 編集時の保守注意)。
  5. **鮭 flip の回転軸変更 (rotateY → rotateX)**: v761 で `@keyframes foodFlipEnhanced` を rotateY (水平軸で奥行き回転) で実装したが、 「水平軸で前後に裏返る = 自分の方を向いてから裏向く」 動作は料理動作 (フライ返しで持ち上げて前後にひっくり返す) と合わず、 ユーザー「鮭の回転もおかしい」 と却下。 修正: **rotateY → rotateX** (縦軸タンブル = 手前から奥に倒れて裏面を見せる) に変更、 duration 1100ms → 900ms に短縮で「もたつかない一連動作」 に。 backStages の裏面 PNG 切替タイミング (mid-flip) は据え置き。
  6. **フライパンの蓋を salmon にも追加** (egg 専用設計を拡張、 既存 egg 制御は完全温存): ユーザー繰り返し「フライパンの蓋どこ？ また勝手に削除した？」 と訴えるが、 git 履歴で `.egg-glass-lid` (HTML `<div id="egg-glass-lid">` + CSS L3038- + JS `onEggLidPointerDown` 等) は **削除されていない** ことを確認 (誤認)。 ただし salmon でも蓋を期待されているため、 **`.egg-glass-lid.show-for-salmon` 新クラス** で salmon でも蓋を auto-show する仕組みを追加した。 ただし v765 時点では **位置/サイズを CSS でハードコード** + **cookFront から表示** で 2 重に間違えていて、 v766 で訂正 (下記 v766 セクション参照)。
  キャッシュバスター: `cookingAssetPath` `?v=764 → ?v=765` + CSS `oil_layer.png?v=765` + sw.js v765 同期、 `?v=764` 残存 grep 0 件: v765
- v765 蓋実装の 2 つの間違いを v766 で訂正 (ユーザー指摘「エディターで大きさを変えたの何のためにやったの？ 蓋を閉めるのは裏にした時からだったじゃん」):
  1. **editor 編集対象要素にハードコード上書きしてはいけない原則違反**: v765 で `.egg-glass-lid.show-for-salmon` に `left: var(--grill-food-x); top: var(--grill-food-y); width: 32%; transform: translate(-50%, -50%)` を直接書き込んでいたため、 editor (`?edit=1`) で調整 → saved-layout.json に保存された `.egg-glass-lid|0` の `w/h/tx/ty` 値が **CSS cascade に負けて反映されない** 状態だった。 修正 (v766): `.show-for-salmon` から `left/top/width/transform` をすべて削除し、 `display: block; opacity: 0.92; z-index: 6; pointer-events: none; transition: opacity 0.45s ease` の 5 プロパティのみに縮約。 これで base CSS の `.egg-glass-lid { left: var(--grill-lid-home-left, 78%); top: var(--grill-lid-home-top, 8%); width: var(--grill-lid-home-w, 21%) }` 変数経路 + LayoutApplier 経由の **inline style** (`style.width = "370.57px"` 等、 cascade 最上位) が正しく効くようになり、 editor 調整値 + saved-layout 値が反映される。
  2. **蓋表示のタイミング契約違反**: v765 で `cookFrontSubStep` 冒頭でも `classList.add('show-for-salmon')` していたが、 ユーザー仕様は「**裏返してから** 蓋を閉める」 = `cookBackSubStep` 開始時のみ蓋を closed 位置に。 修正 (v766): `cookFrontSubStep` 内の add 行を削除 (1 行コメント化)、 `cookBackSubStep` 冒頭の add (state.current.id === 'salmon' ガード付き) は維持、 `runFlipSubStep`/`runDoneSubStep`/`resetCookSubStepUI` の remove も維持。 これで「表焼き = 蓋なし開放、 裏返し → 裏焼き = 蓋を閉めて蒸し焼き、 完成 = 蓋を取って盛り付け」 の自然な料理動作に整合 (現実の魚調理でも厚みのある身に火を通す時に蓋をする動作)。
  
  **知見 (恒久ルール)**: (a) **editor 編集対象要素 (`.egg-glass-lid`/`.knife`/`.bowl` 等の `editableSelectors` 登録要素) には絶対に CSS で位置・サイズをハードコードしない**。 LayoutApplier が inline style で上書きする前提 (v403/v405/v423 で学んだ data-le-keep-position + position absolute の規律と対をなす)。 状態クラス (`.show-for-salmon` 等) で **visibility / opacity / z-index / transition のみ** を切替え、 位置・サイズは base CSS の CSS 変数 + saved-layout inline style に委ねる。 (b) **既存メカニクスを別食材に流用する時は、 タイミング契約も確認する**: egg の蓋は「user が drag → closed」 が UI 契約、 salmon に流用する時は「裏返してから (cookBack で) user が drag → closed」 が正しい契約だった (v766 で「auto closed」 と誤って解釈、 v767 で訂正)。 単に CSS クラスを ON にするだけでなく、 ON/OFF タイミング + インタラクション形式 (auto vs manual) を subStep のどのフェーズに紐付けるかを user spec で確定してから実装する。 sw v766。: v766
- v766 蓋の **インタラクション形式の誤解釈** を v767 で訂正 (ユーザー報告「蓋が閉められません」):
  v766 で `.show-for-salmon` に `pointer-events: none` + auto-close を実装したが、 ユーザーの真意は「裏返してから **手動で** 蓋をドラッグして閉める」 = **egg と同じ drag-to-close メカニクス** だった (= 上記 (b) ルールの教訓元)。 v766 では蓋が触れない/閉められない状態。 v767 で **salmon 専用 drag handler を新設** して egg flow と完全独立に実装:
  - **CSS**: `.egg-glass-lid.show-for-salmon` を `pointer-events: auto` + `cursor: grab` に変更、 新規 `.salmon-lid-dragging` / `.salmon-lid-closed` クラスで drag 中の追従 + snap 後の closed 姿勢を CSS で定義 (salmon 用 closed pose は egg の CSS 変数 `--grill-lid-home-*` を流用、 egg scoped セレクタは使わない)
  - **state**: `cookSalmonLidActive` / `cookSalmonLidClosed` / `cookSalmonLidDragging` / `cookSalmonLidPointerId` を新規追加 (egg の `cookEggLidReady` / `cookEggLidClosed` は **完全非破壊**、 1 ビットも触らない)
  - **handler**: 新規 `onSalmonLidPointerDown` / `Move` / `End` + helper `attachSalmonLidHandlers` / `detachSalmonLidHandlers` / `moveSalmonLidToPoint` / `resetSalmonLidToHome` / `isSalmonLidDraggable`。 pan center hit-test は egg の `getEggLidDropTarget` を流用 (gating は `state.current.id === 'salmon'` + salmon フラグ)、 drag 座標変換は egg と同じ `getCookDragPoint('grill', evt)` 経由で `getBoundingClientRect.width/height` 除算 → **v429 stage scale バグ再発なし**
  - **cookBackSubStep**: salmon の場合は (i) `attachSalmonLidHandlers()` で listener 追加、 (ii) バナー「ふたを しめよう！」 表示、 (iii) `state.cookSalmonLidActive = true`、 (iv) `startCookTimingGauge('grill')` を **起動しない** (蓋 close 待ち)。 非 salmon は従来通り即 gauge 起動 (fall-through)
  - **`onSalmonLidPointerEnd` snap 処理**: drop 位置が pan center 半径内 (egg と同じ hit-test) なら `.salmon-lid-closed` 追加 + `state.cookSalmonLidClosed = true` → ここで初めて `startCookTimingGauge('grill')` 起動 + バナー「タイミングよく タップ！」 切替
  - **`handleSubStepGrillTap` ガード**: `sub.action === 'cookBack' && state.cookSalmonLidActive && !state.cookSalmonLidClosed` で **タップ無視**、 蓋を閉める前に gauge をタップで進めない安全策
  - **cleanup**: `runFlipSubStep` / `runDoneSubStep` / `resetCookSubStepUI` で `detachSalmonLidHandlers` + 4 state リセット + `.salmon-lid-dragging` / `.salmon-lid-closed` / `.drop-ready` クラス除去。 cross-review で `runFlipSubStep` の `drop-ready` 除去漏れ (LOW) を検出 → 同 commit で 1 行追加修正済み
  - **完全独立性**: 既存 `onEggLidPointerDown/Move/End` + `cookEggLidReady/Closed` + `.egg-lid-ready/.egg-lid-closed` クラスは 1 行も改造していない (git diff で確認、 review でも非破壊性確認済み)
  
  **追加知見**: 「インタラクション形式 (auto vs manual)」 は subStep のタイミング契約とは別軸で確認すべき項目。 v766 で「タイミング契約」 (= cookBack 開始時) は正しく直したが、 「インタラクション形式」 (= auto vs manual) を user spec で確認せず auto を選んでしまった。 新メカニクス導入時はこの 2 軸 (タイミング × インタラクション) を必ず user 確認して確定する: v767
- v767 蓋 snap 後の **closed pose 位置・サイズが editor 設定と一致しない** バグを v768 で訂正 (ユーザー報告「エディターではちゃんと大きさと位置を指定してるんだけど、 なぜ同じにならないのか」):
  v767 で `.salmon-lid-closed` CSS に egg 用 closed 位置を流用するつもりで `left: var(--grill-lid-left, 43%); top: var(--grill-lid-top, 32%); width: var(--grill-lid-w, 53%)` を hardcode していた。 LayoutApplier は editor の `.egg-glass-lid|0` 値 (`w/h/tx/ty`) を **inline `width / transform: translate(tx, ty)` には書き込むが、 `left/top` は inline 化しない** ため、 `.salmon-lid-closed` 付与時に CSS の `left: 43%` 等が cascade で勝ち editor 設定が無視されていた。
  修正 (v768): (1) `.salmon-lid-closed` CSS から `left/top/width/transform` をすべて削除し、 `opacity: 1; pointer-events: none; cursor: default` の 3 プロパティのみに縮約 (v766 の `.show-for-salmon` 縮約と同じ原則を closed pose にも適用)。 (2) `onSalmonLidPointerEnd` の snap 成功分岐で `resetSalmonLidToHome()` で drag inline (left/top/width/height/transform) をクリアした **直後** に `reapplyKnifeLayout()` (= `LayoutApplier.apply(null)` で whole-document re-apply、 v425 から knife / chop で使われている既存 helper) を呼んで editor 値を inline style に**復活**。 これで `.salmon-lid-closed` クラスが付いた状態でも editor 設定値 (`.egg-glass-lid|0` の `w: 376.212px, h: 311.313px, tx: -445.85, ty: -6.56` 等) がそのまま反映される。 (3) 既存 egg `.egg-lid-closed` / `cookEggLidClosed` / `onEggLidPointerEnd` / `--grill-lid-*` 変数は 1 行も触らず非破壊 (salmon と egg は selector レベル + state flag レベルで完全分離)。
  
  **v766 ルールの拡張 (恒久ルール強化)**: 「editor 編集対象要素には CSS で位置・サイズをハードコードしない」 を **全ての state クラス** に適用する必要がある。 v766 では `.show-for-salmon` (home pose) には適用したが `.salmon-lid-closed` (closed pose) には適用し忘れていたのが今回のバグ。 状態クラスが複数ある (home / dragging / closed / ...) 時は **全クラスを横断的にチェック**して位置・サイズ hardcode が無いことを確認する。 また、 inline style を JS で clear した後は **LayoutApplier 再適用** (`reapplyKnifeLayout` 経由) を必ず呼ぶ (v425 knife の rest pose 復帰パターンと同じ責務)。: v768
- v768 を v769 で **完全 rollback** (私の設計ミス、 ユーザー報告「フライパンが閉められません」): v768 で `.salmon-lid-closed` から hardcode を削除し `reapplyKnifeLayout()` で editor 値を再適用する設計を採用したが、 致命的な見落としがあった。 **`saved-layout.json` の `.egg-glass-lid|0` entry は egg と salmon の両方で 共有** されており、 保存値 `tx=-445.85px, ty=-6.56px` は **egg 用にチューンされた値** だった。 salmon の snap 後に `reapplyKnifeLayout` で適用すると salmon の base 座標系 (HOME 78%/8% スタート) に egg 用 translate が掛けられ、 **蓋が画面外に飛んで** ユーザーには「閉まらない」 と見えた。
  
  **v769 修正**: (1) `.salmon-lid-closed` に v767 同等の hardcoded position (`left: 43%; top: 32%; width: 53%`) を CSS で復活、 (2) `onSalmonLidPointerEnd` snap 分岐の `reapplyKnifeLayout()` 呼出を削除。 v767 の挙動に完全 rollback。 editor 値は home pose の見た目だけに影響する仕様に戻る。 既存 egg 蓋制御は引き続き完全非破壊。
  
  **学んだ恒久ルール (3 つ目)**: 「**1 つの DOM 要素 (`.egg-glass-lid`) が複数食材で使われる場合、 saved-layout の単一 entry を複数食材で共有してはいけない**」。 食材ごとの座標系・運用が違う以上、 共有 entry を一方の食材の `LayoutApplier.apply` で復活させると他方の座標系を破壊する。 解決には saved-layout に `.egg-glass-lid-salmon-closed|0` のような **食材別 entry** を追加する設計が必要 (将来課題、 現在は salmon 側を hardcode で固定して妥協)。 v429/v403/v423 で学んだ「editor と独自アニメの transform 奪い合いを避ける」 ルールに加え、 「**saved-layout entry のスコープ管理**」 も食材を跨ぐ要素では必須チェック項目とする: v769
- v770-v772 中間バンプ (sw.js のみ、 アセット差分なしの cache-bust 等の自動 commit 経由)、 v773 で **コロッケ / エビフライ / じゃがいもの下ごしらえチェーン本格化** (ユーザー Plan A 承認: 「エビフライ + コロッケ即対応 + ポテト mash 追加」)。 既存 INGREDIENTS で `shrimp_breaded` / `korokke_raw` の `prepActions` フィールドが初版から未接続だった (PREP_CONFIGS には `bread_shrimp` / `bread_korokke` の 3 段階 (小麦粉→卵→パン粉) 衣つけ実装が完備 + アセット計 30 枚あったが、 食材定義側で接続漏れ) + `potato` は `chop` のみで「すりつぶす」 工程が未実装だった問題への対処:
  
  **PREP_CONFIGS 新規 2 件**:
  - `mash_potato` (`mix_mince` 雛形): `dragPrep: true` + 3 段階 brushSrcs (`potato/potato_boiled_001` → `potato_mash_002` → `potato_mash_003`)、 ボウルの上でぐにぐにドラッグして塊から滑らかへ
  - `shape_korokke` (`shape_menchi` 雛形): `shapePrep: true, shapeType: 'patty', shapeDistanceScale: 1.08` + 3 段階 brushSrcs (`korokke/prep/korokke_dough_001` → `korokke_shape_002/003`)、 小判形成
  既存 PREP_CONFIGS (`mix_mince` / `mix_mince_onion` / `shape_meatball` / `shape_menchi` / `shape_tsukune` / `shape_patty` / `bread_shrimp` / `bread_korokke` / `marinate_chicken` / `coat_chicken`) は 1 行も変更なし。
  
  **INGREDIENTS 3 食材接続**:
  - `potato.prepActions = [chop, mash_potato, mix_mince, shape_korokke, bread_korokke]`、 `cookingActions = [...prepActions, fry]` (6 段階フルチェーン) + `cookStartActionIndex: 0` (chop から再実行、 既存 mince_patty/meatball と同パターン)
  - `shrimp_breaded.prepActions = [bread_shrimp]`、 `cookingActions = [bread_shrimp, fry]` + `cookStartActionIndex: 0`
  - `korokke_raw.prepActions = [bread_korokke]`、 `cookingActions = [bread_korokke, fry]` + `cookStartActionIndex: 0`
  
  **アセット placeholder 7 枚** を `scripts/gen_potato_korokke_placeholders.py` (新規スクリプト、 既存 `gen_cooking_placeholders.py` の出力には触らない) で Pillow 生成:
  - `potato/potato_boiled_001.png` + `potato_mash_002.png` + `potato_mash_003.png` (380×220)
  - `potato/masher.png` (220×260、 現状未接続 — 次イテレーション課題)
  - `korokke/prep/korokke_dough_001.png` + `korokke_shape_002.png` + `korokke_shape_003.png` (380×220)
  
  **既知の将来課題 (MEDIUM、 placeholder 期間中の許容範囲)**:
  - `mix_mince` の brushSrcs (`knead_mince_stage_1.webp` 等) は **ひき肉専用画像** で、 potato で流用するとビジュアル不整合 (じゃがいも → ひき肉混合 のアニメで「ひき肉だけ」 が描画)。 placeholder→本画像差替時に `mix_mince_potato` 専用 config (じゃがいも+ひき肉合成画像) を別途定義予定
  - `masher.png` は `mash_potato.steps[]` から未参照、 次イテレーションで `toolSrc` 等のフィールドに wiring 予定
  - `cookStartActionIndex: 0` が chop から再実行を強制するが、 子供向け UX として「お弁当箱から既に下ごしらえ済 potato を選んだ時に chop からやり直し」 が正しいか UX 確認推奨 (mince_patty/meatball も同じ挙動なので統一性は OK): v773
