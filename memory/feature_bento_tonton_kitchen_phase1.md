...
`bento/kitchen.html`（自己完結 HTML + inline CSS/JS、IIFE 構成、約 3350 行、sw v427 時点）。 ユーザーが既存 `bento/index.html` を開く前にここを通る想定。
