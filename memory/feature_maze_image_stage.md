...
**Phase 3 (2026-05-14)**: ラフ → エディタ シームレス handoff + start/goal radius 可変 + edges 一括削除 + gh proxy 日本語ファイル名対応 を実装済 → 詳細は [[feature_maze_rough_to_editor_workflow]] (sw v1000)
...
- ✅ **お邪魔虫ミニゲーム拡充 Phase B (4 種追加)**: じゃんけんに加えて以下 4 種を実装。`_currentCreature` / `_currentGame` グローバルと `_GAME_STARTERS` ディスパッチ表で `_showEncounterStart()` から分岐。stage def の `creatures[].minigame` で個別オーバーライド可能 (ホワイトリスト検証)
  ...
  - **遭遇セリフを統一 (2026-05-15)**: ミニゲーム種別 / お邪魔虫 kind に関わらず、遭遇モーダルのセリフは固定で「ここは とおさないぞ！\nぼくに かったら、とおしてあげる！」に統一 (じゃんけん + `_resolveQuiz` 系の truefalse / silhouette / oddone / simon すべて共通)。`_GAME_DIALOGS` マップ参照は呼び出し側から削除済 (定数自体は将来用に残置)。`c.dialog` (ステージ JSON 作者オーバーライド) があれば従来通り最優先
  - **勝利後セリフを追加 (2026-05-15)**: `_closeEncounter(true)` の前に新ステップを挟み、「うーん、まけちゃった！\nとおって いいよ。」をお邪魔虫側のセリフとしてモーダルに表示 → 1.6s 後に `_creatureDefeatTimer` が `_closeEncounter` を自動発火して歩行再開。じゃんけん経路 + `_resolveQuiz` 経路 (truefalse / silhouette / oddone / simon) の双方をカバー。タイマーは `_triggerEncounter` / `imgLoadStage` / `_closeEncounter` の各冒頭でキャンセルして二重発火・stale fire を防止。敗北フロー (くやしい〜！ で直前ノードへバウンス) は無改変
...
- ✅ **お邪魔虫マーカーの PNG スプライト対応 (2026-05-15 → 2026-05-16 全種画像化完了)**: 全 `kind` を PNG イラストで描画する方式に移行。画像ファイル: `assets/images/maze/creatures/{mayoi,odoke,pyon,kabuto,hachi}.png` (コード参照は `../assets/...` の相対パス)。実装: `CREATURE_KIND_DATA` (ランタイム) / `CREATURE_VIS` (エディタ) / `CREATURE_IMAGE_URL` (ラフ) に `imageUrl` フィールドを追加し、`Image` をレイジー load + キャッシュ。読込完了後はアスペクト比保持で描画、読込前は `emoji` フィールド (`🐛 🐞 🦗 🪲 🐝`) を一時フォールバックとして利用 (絵文字専用 kind は無くなった)。修正ファイル: `maze/index.html`, `tools/maze-editor.html`, `tools/maze-rough.html` + 新規 PNG (mayoi/odoke で初出、2026-05-16 に pyon/kabuto/hachi を追加)
- ✅ **お邪魔虫 kind 集合の改修 (2026-05-16, commit b3cdba6 + b24cb3c)**: 4 種 (mayoi/odoke/nemuri/pyon) → **5 種 (mayoi/odoke/pyon/kabuto/hachi) に再編、全 PNG 画像ベース**。変更点: (a) `nemuri` (😴) を廃止、(b) `pyon` を 🦗 絵文字から実画像 (バッタ PNG) に昇格、(c) `kabuto` (カブトムシ=コツコツムシ) / `hachi` (ハチ=ブンブンムシ) を新規追加。**後方互換**: 既存ステージ JSON が `kind:'nemuri'` を参照していても、ローダー側 (`_creatureData` in `maze/index.html`, `loadDef` + rough-import normalize in `tools/maze-editor.html`, `validCKinds` path in `tools/maze-rough.html`) が静かに `'mayoi'` にエイリアスするので壊れない。エディタで再保存すれば自動的に mayoi に正規化される。エディタツールバーは 5 個の kind ボタン (img サムネ 22x22 / 1.6em) を表示。遭遇セリフは引き続き `CREATURE_ENCOUNTER_DIALOG` で統一 (kabuto/hachi 専用セリフは無し)、ミニゲーム抽選は `_GAME_LABELS` から kind 非依存ランダムのまま動作。`hachi.png` は LANCZOS で 836x850 にリサイズして 1.47MB → 0.97MB に最適化 (alpha 保持)
...
## 救出シーン セリフ集 (確定版, 2026-04-28)

ステージ進行のアーチ: **明るい森の浅い迷子 (1-3) → 不安が深まる (4-5) → 夜の心細さ (6-7) → ランタンに照らされた希望の合流 (8)**

各ステージ JSON の `story` フィールドに `rescueStory` (動物の独白) と `ponoComment` (ポノからの一言) として格納。`imgShowRescueScene` が 0.8s/2.6s 遅延で順番に吹き出し表示する。両者を持たないステージは旧仕様 (キャプション 1 行) にフォールバック。

### 設計ポリシー
- 迷子理由は全種別違うパターンに分散: 追いかけ (1,2,7) / はぐれ (3,6,8) / 隠れ (4) / 居眠り (5)
- ポノの結びの言葉も毎回違う表現: 「いっしょに いってあげる」「あっち だね」「いっしょに さがそう」「みつけたよ」「すぐに おうち」「ごしゅじん の とこ」「ランタンが あるから」「すぐ そこ だよ」
- ステージ 8 はしっとりめに落としてエンディング感、「みんな」を強調して全クリオーバーレイの「まいごの どうぶつたちを ぜんぶ たすけたよ」と回収

### Slot 1 — 森の入り口 / リス (活発・好奇心旺盛) — **JSON 反映済 ✓**
- **リス**: 「ちょうちょを おいかけてたら\nもりの おくに きちゃったの…\nおうちに かえれなくて こわかった！」
- **ポノ**: 「もう だいじょうぶ だよ！\nポノが おうちまで\nいっしょに いってあげる！」

### Slot 2 — 花のこみち / ネコ (自由気まま・気まぐれ)
- **ネコ**: 「いい かおりが して つい…\nおはなを たどって あるいてたら\nかえりみち が わからない の。」
- **ポノ**: 「ポノが いるから\nもう さびしくない よ。\nおうち は あっち だね！」

### Slot 3 — しずくの池 / 子ジカ (おっとり・親っ子)
- **子ジカ**: 「おかあさん と\nみず を のんでたら\nいつのまに ひとりに…\nぐすん。」
- **ポノ**: 「ポノが いっしょに\nおかあさん を さがそう。\nきっと みつかる よ！」

### Slot 4 — きのこの小道 / ウサギ (慎重・こわがり)
- **ウサギ**: 「かくれんぼ してたら\nだれも みつけて くれなくて…\nふしぎな きのこ ばかりで\nこわく なっちゃった。」
- **ポノ**: 「もう ひとりじゃ ないよ。\nポノ が みつけた からね、\nいっしょに かえろう。」

### Slot 5 — こもれびの森 / レッサーパンダ (のんびり・ちょっと天然)
- **レッサーパンダ**: 「きの うえで うとうと してたら…\nおりた ところが\nしらない もり だった の。\nねぼすけ で ごめんね…」
- **ポノ**: 「びっくり したね。\nポノ と あるけば\nすぐに おうちだよ！」

### Slot 6 — つる草の小径 / イヌ (元気でやんちゃ・反省気味)
- **イヌ**: 「ぼく 走るの はやくて…\nきづいたら ごしゅじん が\nみえなく なっちゃったんだ。\nさみしい よ…」
- **ポノ**: 「えらかったね、まってて くれて。\nポノ と いっしょに\nごしゅじん の とこ いこう！」

### Slot 7 — ほたるの湿り道 / アヒル (のほほん・夜が苦手)
- **アヒル**: 「ほたる の ひかり が\nきれいで…\nおいかけてたら\nよる に なって\nこころぼそく なっちゃった。」
- **ポノ**: 「ランタン が あるから\nもう くらく ない よ。\nポノ が ずっと いっしょ だよ！」

### Slot 8 — ひかりの広場へ / ハリネズミ (シャイ・最終ステージしっとり)
- **ハリネズミ**: 「みんな と はぐれて\nずっと さがして いた の…\nもう あえない かもって\nおもって いた…」
- **ポノ**: 「みんな まってる よ。\nさあ かえろう、\nひかりの ひろば は\nすぐ そこ だよ！」
...
```json
{
  "story": {
    ...
    "animalLabel": "<label>",
    "intro": "...",
    "cryingIconUrl": "imageStages/<id>_crying.png",
    "reliefIconUrl": "imageStages/<id>_relief.png",
    "rescueStory": "<動物の独白>",
    "ponoComment": "<ポノの一言>"
  }
}
...
各ステージの JSON が画像と一緒に届いたら、上の表からコピペで rescueStory / ponoComment を埋めてください。
