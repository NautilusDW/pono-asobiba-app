...
- 2026-07-23 - by Codex - [batch:1432-hyokkori-mask-alignment-retention] - hyokkori-hightouch/**, assets/images/hyokkori-hightouch/** (調査のみ。必要時は対象限定), tests/e2e/hyokkori/**, tests/hyokkori_hightouch_regression.cjs, tmp/alpha_pending/1418-hyokkori-hightouch-art-refresh/GAMEPLAY-IDEAS.md, sw.js, play.html, HANDOFF.md, AGENTS_CLAIMS.md - ユーザー実画面指摘「各茂みが背景接地面からずれる／出現中に下から透ける／停止時の絵の下端がクロップ」を、キャラ画像alpha bbox・窓mask・前縁・出現transformの同一座標系化で修正。主操作はawake友だちへのハイタッチ、fallbackはsleepingを待つ。4 viewport・全awake/sleeping・出現途中／停止状態・タップ面不変を回帰し、急かしすぎない繰り返し要素案も既案と比較して追記する。[overlap: batch:1428-hyokkori-central-combo-lane (同機能のfollow-up), batch:1429-guragura-weight-redesign-e2e (sw.js/play.html)]
