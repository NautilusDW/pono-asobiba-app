...
- 2026-07-24 - by Codex - [batch:1450-hyokkori-grounded-journey] - hyokkori-hightouch/**, assets/images/hyokkori-hightouch/hideout_world_*_v2_20260724.png, docs/HYOKKORI_FOREST_WALK_PLAN_2026-07-23.md, tests/hyokkori_hightouch_regression.cjs, tests/e2e/hyokkori/**, play.html, sw.js, AGENTS_CLAIMS.md, HANDOFF.md - 宙に浮く穴と水辺外装の泡状生成不良をGPT Image 2で差し替え、全3面の接地点・マスク・配置を実画面で再検証。3面以降を目的のある森の旅として背景／動物／到達点まで企画更新し、App stagingまで確認する [overlap: batch:1447-tsuri-fishing-phase0, batch:1437-machizukuri-new-feature, batch:1440-guragura-doll-weight-v3-ui, batch:1441-guragura-round-review-fixes (play.html/sw.js共通)]
- 2026-07-24 - by Claude - [batch:1449-tsuri-unity-water-spike] - unity/PonoNativeGames/Assets/Pono/Games/KawaGlint/** (新規パッケージのみ・AquaLumina/HideSeekCreatures/ColorWaterDelivery/AppShellは不変更), docs/unity-migration/kawa-glint/DESIGN.md, AGENTS_CLAIMS.md, HANDOFF.md - **DONE (Editor batchmode build-verified, screenshot未取得)**: つりゲーム(tsuri-kawa)の川面をUnity URP水シェーダーで検証する技術スパイク「KawaGlint」を新規実装(4モジュール: refraction/surface/stage/actors + 統合)。専用シーン`91_KawaGlint.unity`・コピーしたRenderer2D(index2として追加、AquaLuminaのindex1は無変更)・専用VolumeProfileを構築、SetupFromCommandLine 2回実行でidempotent(UniversalRP.assetの差分は追加1行のみで2回目はゼロ差分)、macOSビルド成功、HideSeekの出荷ビルドも再検証しshipping無事を確認。**重要な発見**: KawaGlintのビルドだけでなくHideSeekの出荷ビルドを走らせただけでも、Unity内部の自動同期挙動によりプロジェクト内の全VolumeComponentサブクラス(Aqua*2種+Kawa*1種)が共有`Assets/DefaultVolumeProfile.asset`へ都度混入することを確認(AquaLumina単体の事故ではなく、このリポジトリで複数のUnityスパイクが同居する限り、どのビルドコマンドを実行しても再発しうるプロジェクト全体のハザード)。都度AquaLumina/KawaGlint双方のCleanSharedDefaultVolumeProfileメニューで除去し、最終的にgit diffが空であることを確認済み。QAスクリーンショット取得はbatch:1446と同一理由(本セッションのサンドボックス環境がGUIアプリへ実ディスプレイ/GPUコンテキストを渡せない)で未達、-nographicsでは全黒画像・-nographicsなしではハングを対照実験で確認しKawaGlint側の欠陥ではないと切り分け済み。設計資料は`docs/unity-migration/kawa-glint/DESIGN.md`。commit/push未実施、ユーザー承認待ち [overlap: batch:1446-aquarium-unity-water-spike (同じunity/PonoNativeGamesプロジェクト、AquaLuminaのRenderer2D/VolumeProfile追加パターンを参照するのみで同ファイルは編集しない)]
